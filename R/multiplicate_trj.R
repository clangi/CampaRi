...
#' @param spacing_window  A spacing window between the snapshots took.
#'
...
multiplicate_trj <- function(trj, window = NULL, overlapping_reduction = NULL, method = NULL, spacing_window = NULL){

  available_methods <- c("sincos")
  # Checking input variables (again - it is also a stand alone function)
  if(!is.null(spacing_window) && (!.isSingleInteger(spacing_window) || spacing_window <= 0 || spacing_window > nrow(trj)/2))
    stop('The used spacing_window is too small or too big (must be less than half to have sense) or it is simply an erroneus insertion.')
  if(!is.null(window) && (length(window) != 1 || !is.numeric(window) || window <= 0 || window > nrow(trj)/2))
    stop('The used window is too small or too big (must be less than half to have sense) or it is simply an erroneus insertion.')
  ...
  if(((round(window*spacing_window)-1)/2)%%1 == 0) {
    window_r <- window_l <- (round(window*spacing_window)-1)/2
  }else{
    window_r <- floor((round(window*spacing_window)-1)/2)
    window_l <- ceiling((round(window*spacing_window)-1)/2)
  }
  ...
  for(i in 1:dim(trj)[1]){
    if((i - window_l) <= 0){
      sel <- seq(i, i + window_r, spacing_window)
      trj_out[i,] <- as.vector(t(trj[rep(sel, ceiling((window - .lt(sel))/.lt(sel) + 1))[1:window], ]))
    }else if((window_r + i) > nrow(trj)){
      sel <- seq(i, dim(trj)[1], spacing_window)
      trj_out[i,] <- as.vector(t(trj[rep(sel, ceiling((window - .lt(sel))/.lt(sel) + 1))[1:window],]))
      
    }else{
      trj_out[i,] <- as.vector(t(trj[seq((i - window_l), (i + window_r), spacing_window),]))
    }
  }
  return(trj_out)
}
