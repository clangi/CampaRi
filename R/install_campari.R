...
install_campari <- function(campari_source = NULL, installation_location = NULL, install_ncminer = FALSE, install_threads = FALSE, 
  ...
  if(!is.null(campari_source)){
    ...
    camp_src <- paste0(system.file('extdata/', package = "CampaRi"), '/')
    camp_src <- paste0(camp_src, '/for_campari/')
    if(!dir.exists(camp_src)) dir.create(camp_src)
    if(!file.exists(paste0(camp_src, '/source/chainsaw.f90'))) {
      ...
      system(paste0('unzip -o -qq -d', camp_src, ' ', camp_src, "/debcampari-master.zip "))
