...
plot_traces <- function(voltage_traces_mat, return_plot = F, sub_sampling_factor = NULL, title = "", 
  ...
  gg <- ggplot() +
    xlab("Snapshots") + ylab("Neuron voltages") + theme_minimal() 
  ...
  for(i in 1:n_traces){
    ...
    grps <- c(grps, rep(i,n_snaps))
  }
  gg <- gg + geom_line(aes(x = x_multilines,
                              y = y_multilines, group = grps),
                          size = 0.1*size_line)
