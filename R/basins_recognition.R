...
################################################################################
        ...
        ## It works in pathlogical cases (i.e. NULL vecto)
        if(!silent) print(paste("Number of bigg is", .lt(bigg.brk)))
        breaks.tot <- unique(sort(c(breaks.tot, bigg.brk)))
        if(!silent) print(paste("Number of matched is", .lt(bigg.brk)))
...
########################################################################
            ...
            distHell <- rep(-1, .lt(breaks.tot)-2)
            for(j in 1:(.lt(breaks.tot)-2)) distHell[j] <- myHell(dens[,j], dens[,j+1])
...
########################################################################################
    ...
    ## source("./Rfunctions/SBR_images_functions.R")
    ## probdist.comparison(progind$Time, rawset, hist, joinx, breaks.tot, distHell, brk.dyn)
