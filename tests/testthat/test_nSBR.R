...
test_that('new trials for SBR', {
  ...
  expect_error(a1 <- CampaRi::nSBR(data = file.pi, n.cluster = 3, 
                                   ...
                                   silent = silent, dbg_nSBR = F, return_plot = F), NA)
  # ------------------------------------------------------- shuffles test
  expect_error(a1 <- CampaRi::nSBR(data = file.pi, n.cluster = 15, shuffles = T, 
                                   comb_met = c('MIC'),
                                   unif.splits = seq(5, 100, 8),  
                                   pk_span = 500, ny = 20, plot = T, random_picks = 100, ann = ann,
                                   silent = silent, dbg_nSBR = T, return_plot = F), NA)
  ...
  ######################### evaluating an automatic way to select the number of cluster -> grid search ##########################
    ...
    rownames(sco.mat) <- colnames(sco.mat) <- rownames(sco.rnd.mat) <- colnames(sco.rnd.mat) <- rownames(h.split.mat) <- colnames(h.split.mat) <- paste0('ncl.', ncl.vec)
    rownames(span.mat) <- colnames(span.mat) <- rownames(ny.mat) <- colnames(ny.mat) <- paste0('ncl.', ncl.vec)
    for(ncl.i in ncl.vec){
      ...
      for(ncl.j in ncl.vec){
        ...
        ref_sc <- CampaRi::score_sapphire(the_sap = file.pi, ann = ref.vec, manual_barriers = temp2$tmp$barriers[1:(ncl.j-1)], silent = T)
    ...
    sco.mat.save <- sco.mat
    rownames(sco.mat) <- paste0(rownames(sco.mat), '.i')
    colnames(sco.mat) <- paste0(colnames(sco.mat), '.j')
    ...
    vec.from.barriers <- function(bar.vec){
      bar.vec <- sort(c(bar.vec, 3000))
      bar.vec <- c(bar.vec[1], diff(bar.vec))
      c(unlist(sapply(1:length(bar.vec), function(x) rep(x, bar.vec[x]))))
    }
