...
test_that('Test for basin recognition with ext files', {
    ...
    expect_error(tmp <- basins_recognition(data=file.pi, nx=parspace$nxbb[1], ny=parspace$nybb[1], ny.aut=T, local.cut=F, match=T, dyn.check=2, avg.opt="SG", plot=T, out.file=T, silent=F, pol.degree=3), NA)
    ## list.files()
