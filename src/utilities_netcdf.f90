...
!
  ...
  if (exists.EQV..true.) then !manual delete of the file if it exists
    write(ilog,*) 'Dumping file "MST_DUMPLING.nc" already existent. It will be overwritten'
    ncid = freeunit()
...
! ! close
  ...
  write(ilog,*) 'Saved a total number of snapshots of: ', sum(approxmst(1:n_snaps)%deg)
!
end
!
!-------------------------------------------------------------------------------
...
!
 ...
 &from NetCDF (',dumpfile,') in setup_netcdftraj().'
    call fexit()
  end if
! open
...
!
  if (nframes.lt.1) then
    write(ilog,*) 'Fatal. NetCDF-file (',dumpfile,') has no neighbor &
...
!   do nothing
 ...
 &dumpfile,'). Use NetCDFs ncdump utility to check file.'
    call fexit()
  else
...
!
  ...
  write(ilog,*) 'Read a total number of snapshots of: ', sum(approxmst(1:n_snaps)%deg)
  deallocate(vsnp)
  ! deallocate(vnbs)
  deallocate(vdist)
!
