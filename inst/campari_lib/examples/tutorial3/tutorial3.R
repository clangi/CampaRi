...
# load data
if (doread == TRUE) {
  fn <- sprintf("%s/N_000_BB_SEGMENTS.dat",example_directory);
  dat <- read.table(fn,header=FALSE);
  a <- dim(dat);
  bbcols = a[2];
  bbs <- array(0.0,c(reps,nseq,bbcols));
  fn <- sprintf("%s/N_000_DSSP.dat",example_directory);
  dat <- read.table(fn,header=FALSE);
  a <- dim(dat);
  dsspcols = a[2];
  ...
  for (rep in 1:reps) {
    fn <- sprintf("%s/N_%03d_BB_SEGMENTS.dat",example_directory,rep-1);
    dat <- read.table(fn,header=FALSE);
    a <- dim(dat);
    entrs = a[1];
    for (i in 1:entrs) {
      bbs[rep,i,] = as.integer(dat[i,]);
    }
  }
  for (rep in 1:reps) {
    fn <- sprintf("%s/N_%03d_DSSP.dat",example_directory,rep-1);
    dat <- read.table(fn,header=FALSE);
    a <- dim(dat);
    entrs = a[1];
    for (i in 1:entrs) {
      dssps[rep,i,] = as.integer(dat[i,]);
    }
  }
  dssps = dssps / (1.0*totcnt);
  bbs = bbs / (1.0*totcnt);
