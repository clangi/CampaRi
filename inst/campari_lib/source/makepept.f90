...
!
  ...
  logical done,startmol,foundN,foundC,foundM,foundit,laststartmol
!
...
!
! first built all missing tails onto existing partial internal conformation read from PDB
...
!   and RANDOMIZE was not set to 2 or 3
    if (doclnk(1).EQV..true.) then
      imol2 = molofrs(crosslink(lkcnt)%rsnrs(1))
      jmol = molofrs(crosslink(lkcnt)%rsnrs(2))
    end if
    if (do_ids(2).EQV..true.) then
!
!     we have an outgoing crosslink connection into imol (= jmol)
      if (doclnk(1).EQV..true.) then
!
        if (imol2.le.pdbrsize) then
...
!       if both were read in parts, do not randomize RB 
        if ((imol2.le.pdbrsize).AND.(jmol.le.pdbrsize)) cycle
...
!
        do while (etval.gt.globrdmthresh)
          ...
          if (cnt.gt.globrdmatts) then
            write(ilog,67) imol,rsmol(imol,1),amino(seqtyp(rsmol(imol,1))),rsmol(imol,2),amino(seqtyp(rsmol(imol,2)))
...
!         make sure intermolecular energies (only backward) are sane
          ...
          if (use_IPP.EQV..false.) iacnt = rsmol(jmol,2)-rsmol(jmol,1)+1
...
!
  implicit none
!
  RTYPE et(MAXENERGYTERMS),dis,dis2,etbu,etpen,eto,etval
...
!
  ...
  RTYPE evec(MAXENERGYTERMS),random,picky,etbu,ettmp,oth,doth
...
!
  ...
  do while ((rs.ge.rsi).AND.(rs.le.rsf))
    ...
    evec(:) = 0.
    call randomize_energy_short(rs,evec,use_cutoffs,align_C,singlemol,pdbrsz,pdb_didread,curl) 
    etbu = sum(evec)/curl
!
    needone = fone
    do while ((etbu.gt.globrdmthresh).OR.(needone.EQV..true.))
      cnt = cnt + 1
      if (cnt.gt.globrdmatts) exit
      dofs = 0
...
!
  ...
  call randomize_energy_short(sta,evec,use_cutoffs,align_C,singlemol,pdbrsz,pdb_didread,curl) 
  etbu = sum(evec)/curl
