...
!
      ...
      if ((choice.lt.0).OR.(choice.gt.3)) then
        write(ilog,*) 'Invalid spec. for FMCSC_GUESS_BONDED. Turned off by default.'
        guess_bonded = 0
      else
        guess_bonded = choice
      end if
...
!
      ...
      if ((choice.lt.0).OR.(choice.gt.2)) then
        par_IMPSOLV2(1) = 1
        write(ilog,*) 'Invalid spec. for FMCSC_SAVMODE. Defaulting to linear approximation (mode ',par_IMPSOLV2(1),').'
      else
...
!
      ...
      if ((choice.gt.2).OR.(choice.lt.1)) then
        par_IMPSOLV2(2) = 1
        write(ilog,*) 'Invalid spec. for FMCSC_FOSFUNC. Defaulting to ',par_IMPSOLV2(2),'.'
      else
...
!
      ...
      if ((par_IMPSOLV(3).gt.100.0).OR.(par_IMPSOLV(3).le.0.0)) then
        par_IMPSOLV(3) = 0.25
        write(ilog,*) 'Invalid spec. for FMCSC_FOSTAU. Defaulting to ',par_IMPSOLV(3),'.'
      end if
!
...
!
      ...
      if ((par_IMPSOLV(6).gt.1.0).OR.(par_IMPSOLV(6).lt.0.0)) then
        par_IMPSOLV(6) = 0.1
        write(ilog,*) 'Invalid spec. for FMCSC_FOSMID. Defaulting to ',par_IMPSOLV(6),'.'
      end if
!
...
!
      ...
      if (par_IMPSOLV(12).le.0.0) then
        ...
        write(ilog,*) 'Invalid spec. for FMCSC_FOSGRANULE. Defaulting to ',par_IMPSOLV(12),' A^3.'
      end if
!
...
!
      ...
      if ((par_IMPSOLV(13).ge.1.0).OR.(par_IMPSOLV(13).lt.0.0)) then
        par_IMPSOLV(13) = 0.0
        write(ilog,*) 'Invalid spec. for FMCSC_FOSTIGHT. Defaulting to ',par_IMPSOLV(13),'.'
      end if
!
...
!
      ...
      if ((par_IMPSOLV(14).gt.1.0).OR.(par_IMPSOLV(14).lt.0.0)) then
        par_IMPSOLV(14) = 0.0
        write(ilog,*) 'Invalid spec. for FMCSC_FOSSHIFT. Defaulting to ',par_IMPSOLV(14),'.'
      end if
!
...
!
        ...
        if ((choice.gt.2).OR.(choice.lt.1)) then
          par_IMPSOLV2(3) = 1
          write(ilog,*) 'Invalid spec. for FMCSC_SCRFUNC. Defaulting to ',par_IMPSOLV2(3),'.'
        else
...
!
        ...
        if (par_IMPSOLV(15).le.0.0) then
          ...
          write(ilog,*) 'Invalid spec. for FMCSC_SCRGRANULE. Defaulting to ',par_IMPSOLV(15),' A^3.'
        end if
!
!     for stair step function, how steep the steps are 
!
        ...
        if ((par_IMPSOLV(16).ge.1.0).OR.(par_IMPSOLV(16).lt.0.0)) then
          par_IMPSOLV(16) = 0.0
          write(ilog,*) 'Invalid spec. for FMCSC_SCRTIGHT. Defaulting to ',par_IMPSOLV(16),'.'
        end if
!
!     for stair step function, where the centers of steps are placed
!
        ...
        if ((par_IMPSOLV(17).gt.1.0).OR.(par_IMPSOLV(17).lt.0.0)) then
          par_IMPSOLV(17) = 0.0
          write(ilog,*) 'Invalid spec. for FMCSC_SCRSHIFT. Defaulting to ',par_IMPSOLV(17),'.'
        end if
!
...
!
        ...
        if ((dum.le.0.0).OR.(dum.gt.1.0e-4)) then
          ewerfctol = 1.0e-8
          write(ilog,*) 'Invalid spec. for FMCSC_EWERFTOL. Defaulting to ',ewerfctol,'.'
        else
          ewerfctol = dum
        end if
!
...
!
        ...
        if ((choice.lt.1).OR.(choice.gt.5)) then
          write(ilog,*) 'Invalid spec. for FMCSC_EWFFTPLANNER. Defaulting to fast but measurement-based plan optimization &
 &(FFTW_MEASURE).'
          fftlevel = 2
        else
          fftlevel = choice
...
!     in case of periodic boundary conditions
 ...
 & boundary condition of type ',bnd_type,'. Defaulting to a rectangular cuboid bo&
 &x (mode ',bnd_shape,').'
        else
          bnd_shape = choice
        end if
...
!
    if (((bnd_type.eq.1).AND.(bnd_shape.ne.1)).OR.(bnd_type.eq.3).OR.(bnd_type.eq.4)) then
!
...
!
        ...
        if ((choice.lt.2).OR.(choice.gt.16)) then
          lincs_order = 8
          write(ilog,*) 'Invalid spec. for FMCSC_LINCSORDER. Defaulting to ',lincs_order,'.'
        else
          lincs_order = choice
        end if
!
...
!
        ...
        if ((choice.lt.1).OR.(choice.gt.10)) then
          lincs_iter = 2
          write(ilog,*) 'Invalid spec. for FMCSC_LINCSITER. Defaulting to ',lincs_iter,'.'
        else
          lincs_iter = choice
        end if
!
...
!
      ...
      if (embuf.lt.1.0) then
        embuf = 1.5
        write(ilog,*) 'Invalid spec. for FMCSC_EMBUFFER. Defaulting to ',embuf,'.'
      end if
!
...
!
      ...
      if ((mccutmode.lt.1).OR.(mccutmode.gt.2)) then
        mccutmode = 1
        write(ilog,*) 'Invalid spec. for FMCSC_MCCUTMODE. Defaulting to exact interatomic SR cutoffs for MC.'
      end if
!
...
!
      ...
      if (time_flush.le.0.0) then
        time_flush = 5.
        write(ilog,*) 'Invalid spec. for FMCSC_FLUSHTIME. Defaulting to ',time_flush,' minutes.'
      end if
!
!   output frequency for energy values
!
...
!
      ...
      if (choice.lt.0) then
        pdb_rmol = 0
        write(ilog,*) 'Invalid spec. for FMCSC_XYZ_REFMOL. Any possible image adjustments for molecules &
...
!
      ...
      if (choice.lt.1) then
        cbirchbatch = 1
        write(ilog,*) 'Invalid spec. for FMCSC_BIRCHCHUNKSZ. Defaulting to ',cbirchbatch,'.'
      else
        cbirchbatch = choice
      end if
#endif
...
!
      ...
      if (cmergecrit.lt.0.) then
        cmergecrit = 1.0
        write(ilog,*) 'Invalid spec. for FMCSC_CMERGEDIAM. Defaulting to ',cmergecrit,'.'
      end if
!
...
!
      ...
      if (choice.ge.0) then
        ...
        cprogrdepth = 0
        write(ilog,*) 'Invalid spec. for FMCSC_CPROGRDEPTH. Defaulting to ',cprogrdepth,'.'
      end if
!
...
!
      ...
      if (choice.ge.0) then
        ...
        cprogbatchsz = 1
        write(ilog,*) 'Invalid spec. for FMCSC_CPROGRDBTSZ. Defaulting to ',cprogbatchsz,'.'
      end if
!
...
!
      ...
      if (choice.lt.0) then
        c_multires = 0
        write(ilog,*) 'Invalid spec. for FMCSC_BIRCHMULTI. Defaulting to ',c_multires,'.'
      else
        c_multires = choice
      end if
!
...
!
      ...
      if (choice.le.0) then
        clufcol = 0
        write(ilog,*) 'Invalid spec. for FMCSC_CLUFILECOL. Defaulting to ',clufcol,'.'
      else
        clufcol = choice
      end if
!
...
!     we'll check for existence in the reader
      ...
      if ((choice.ge.0).AND.(choice.le.2)) then
        ...
        brklnk_report = 0
         write(ilog,*) 'Invalid spec. for FMCSC_BRKLNKREPORT. Turned off by default.'
      end if
!
    end if
!
...
!
        ...
        if ((choice.ge.0).AND.(choice.le.2)) then
          ...
          write(ilog,*) 'Invalid spec. for FMCSC_CREWEIGHT. Disabling (default).'
          cequil = 0
        end if
!
!     how to augment network
        ...
        if ((choice.ge.0).AND.(choice.le.5)) then
          ...
          caddlkmd = 0
          write(ilog,*) 'Invalid spec. for FMCSC_CADDLINKMODE. Defaulting to ',caddlkmd,'.'
        end if
!
!     weight to use for added links
        ...
        if (dum.ge.0.0) then
          ...
          caddlinkwt = 1.0
          write(ilog,*) 'Invalid spec. for FMCSC_CLINKWEIGHT. Defaulting to ',caddlinkwt,'.'
        end if
!
!     how to augment network
        ...
        if (dum.ge.0.0) then
          ...
          caddlinks = 1.0
          write(ilog,*) 'Invalid spec. for FMCSC_CADDLINKS. Defaulting to ',caddlinks,'.'
        end if
!
!     weight buffer
        ...
        if (dum.ge.0.0) then
          ...
          cequilbuf = 0.0
          write(ilog,*) 'Invalid spec. for FMCSC_CREWEIGHTBUF. Defaulting to ',cequilbuf,'.'
        end if
!
...
!     how much time to spend maximally in iterative algorithms
        ...
        if (dum.le.0) then
          ...
          write(ilog,*) 'Invalid spec. for FMCSC_MAXTIME_ITERS. Defaulting to unlimited execution time.'
        end if
...
!
        ...
        if ((choice.lt.0).OR.(choice.gt.3)) then
          synmode = 0
          write(ilog,*) 'Invalid spec. for FMCSC_SYNTRAJ_MD. Disabling (default).'
        else
          synmode = choice
        end if
!
...
!
        ...
        if ((choice.eq.1).OR.(choice.eq.2).OR.(choice.eq.3)) then
          ...
          which_timeflow = 1
          write(ilog,*) 'Invalid spec. for FMCSC_TMAT_MD. Defaulting to to forward time only (mode ',which_timeflow,'.)'
        end if
!
...
!
        ...
        if ((choice.lt.-1).or.(choice.gt.nsim)) then
          inissnap_usrslct = 0
          write(ilog,*) 'Invalid spec. for FMCSC_INISYNSNAP. Defaulting to centroid of largest cluster.'
        else
...
!
        ...
        if ((choice.lt.0).OR.(choice.gt.nsim)) then
          endssnap_usrslct = 0
          write(ilog,*) 'Invalid spec. for FMCSC_ENDSYNSNAP. Defaulting to the use of the last saved snapshot.'
        else
...
!
        ...
        if ( (choice.ge.1).AND.(choice.lt.(nsim/cstorecalc)) ) then
          ...
          clagt_msm = 1
          write(ilog,*) 'Invalid spec. for FMCSC_CLAGT_MSM. Defaulting to ',clagt_msm
        end if
!
#ifdef LINK_LAPACK
!
...
!
        ...
        if ((choice.lt.0).or.(choice.gt.6)) then
          hsl_verbosity = 0
          write(ilog,*) 'Invalid spec. for FMCSC_HSL_VERBOSITY. Disabling (default).'
        else
...
!
        ...
        if ((choice.lt.0).or.(choice.gt.3)) then
          eigvalmd = 0
          write(ilog,*) 'Invalid spec. for FMCSC_EIGVAL_MD. Disabling (default).'
        else
          eigvalmd = choice
        end if
!
...
!
        ...
        if ((choice.lt.1).OR.(choice.gt.nsim)) then
          arnblks = -1 !numeig + 2
          write(ilog,*) 'Invalid spec. for FMCSC_NEIGBLOCKS. Defaulting to number of eigenvalues + 2'
        else
          arnblks = choice
        end if
!
...
!
        ...
        if ((choice.lt.2).OR.(choice.gt.nsim)) then
          ...
          write(ilog,*) 'Invalid spec. for FMCSC_NEIGSTEPS. Defaulting to ceiling((8.*numb. of eigv.)/numb. of blocks)'
        else
          arnstps = choice
        end if
!
...
!
        ...
        if ((choice.lt.0).OR.(choice.gt.1000)) then
          arnmaxitr = 10
          write(ilog,*) 'Invalid spec. for FMCSC_NEIGRST (< 0 or > 1000). Defaulting to ',arnmaxitr,'.'
        else
          arnmaxitr = choice
        end if
!
...
!
        ...
        if ((arntol.lt.0).OR.(arntol.gt.1.0E-03)) then
          arntol = 0
          write(ilog,*) 'Invalid spec. for FMCSC_EIGTOL. Defaulting to (10^3)*(machine precision).'
        end if
#endif
!
      end if
...
!
      if (keyword(1:16) .eq. 'FMCSC_NSYNTRAJS ') then
        ...
        if ((choice.le.0).or.(choice.gt.10000)) then
          nstraj = 10
          write(ilog,*) 'Invalid spec. for FMCSC_NSYNTRAJS (max 9999). Defaulting to ',nstraj,'.'
        else
          nstraj = choice
        end if
!
...
!
        ...
        if ((choice.le.0).or.(choice.gt.1000000000)) then !max one billion steps
          nssnap = 1000000
          write(ilog,*) 'Invalid spec. for FMCSC_NSYNSNAPS (no less than one, no more tha one billion). &
...
!
        ...
        if (choice.lt.0) then
          nsskip = 100
          write(ilog,*) 'Invalid spec. for FMCSC_NSYNTRAJOUT. Defaulting to ',nsskip,'.'
        else
          nsskip = choice
        end if
      end if
...
!
      if (keyword(1:12) .eq. 'FMCSC_NEIGV ') then
        ...
        if ((choice.lt.1).OR.(choice.gt.nsim)) then
          numeig = 1
          write(ilog,*) 'Invalid spec. for FMCSC_NEIGV. Defaulting to ',numeig,'.'
...
!
    if (keyword(1:16) .eq. 'FMCSC_RANDOMIZE ') then
      ...
      read (wo1(wt11:wt12),*) dum
      call dum2int(dum,choice)
      if ((choice.ge.0).AND.(choice.le.4))  then
...
!
      ...
      if ((choice.lt.0).OR.(choice.gt.3)) then
        pdb_force_box = 0
        write(ilog,*) 'Invalid spec. for FMCSC_XYZ_FORCEBOX. Turned off by defaulting (setting ',pdb_force_box,').'
      else
...
!
      ...
      if ((choice.lt.0).OR.(choice.gt.4)) then
        thrdat%verbosity = 1
        write(ilog,*) 'Invalid spec. for FMCSC_THREADS_VERBOSE. Defaulting to ',thrdat%verbosity,'.'
      else
...
!
      ...
      if (choice.lt.10) then
        ...
        write(ilog,*) 'Invalid spec. for FMCSC_THREADS_DLB_FREQ. Defaulting to ',thrdat%dlbfreq,'.'
      else
...
!
      ...
      if (choice.le.0) then
        thrdat%dlbscale = 1
        write(ilog,*) 'Invalid spec. for FMCSC_THREADS_DLB_EXT. Defaulting to ',thrdat%dlbscale,'.'
      else
...
!
      ...
      if (choice.le.10) then
        thrdat%dlblen = 1000
        write(ilog,*) 'Invalid spec. for FMCSC_THREADS_DLB_STOP. Defaulting to ',thrdat%dlblen,'.'
      else
...
!    
      ...
      if (choice.le.0) then
        ...
        write(ilog,*) 'Invalid spec. for FMCSC_RE_TRAJTOTAL. Defaulting to ',re_aux(8),'.'
      else
        re_aux(8) = choice
      end if
!
...
!
    if ((ncdm_donc.EQV..true.).OR.(ncdm_doasconv.EQV..true.)) then  !net-cdf data mining mode has been activated
!
...
!
    end if !if ((ncdm_donc.EQV..true.).OR.(ncdm_doasconv.EQV..true.)) 
!
  else if (which.eq.3) then
!
...
!
!     Number of snapshots: needed only for conversion
!
        ...
        if (ncdm_nframes_ini .le. 3) then
          write(ilog,*) 'Fatal. Invalid spec. for FMCSC_NCDM_NRFRMS ( <= 3 ??? ).'
          ncdm_donc = .false.
          ncdm_doasconv = .false.
          call fexit()
        end if
!
!     Number of features: needed only for conversion
!
        ...
        if (ncdm_nfeats_ini .le. 0) then
          write(ilog,*) 'Fatal. Invalid spec. for FMCSC_NCDM_NRFEATS ( <= 0 ??? ).'
          ncdm_donc = .false.
          ncdm_doasconv = .false.
          call fexit()
        end if
!      
!     Values for possible periodicity range: needed only for conversion to put as an attribute 
!
        ...
        if (ncdm_periodic_rng(1).ne.ncdm_periodic_rng(2)) then 
          ncdm_is_periodic = .true.
        else   
          write(ilog,*) 'Fatal. Invalid spec. for FMCSC_NCDM_PRDCRNG.'
          call fexit()
        end if
!       
      end if
...
!
    if (ncdm_donc.EQV..true.) then
!
!     to print out check file after reading, mainly for debug. Needed only in case we have an input net-cdf file
!
      if (keyword(1:23) .eq. 'FMCSC_NCDM_DOINPTCHECK ') then
        have_legit_key = .true.
        read (wo1(wt11:wt12),*) dum
        call dum2int(dum,choice)
        if (choice.eq.1) then
