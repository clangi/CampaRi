...
!
subroutine randomize_energy_short(rs,evec,cut,ct,molonly,nmr,mrdat,iacnt)
!
  use iounit
  use sequen
  use cutoffs
...
!
  implicit none
!
  integer, INTENT(IN):: rs,nmr,mrdat(nmr,2)
...
!
 ...
 &          ((rs.lt.mrdat(imol,1)).AND.(sta.gt.mrdat(imol,2)))) then
          cycle
        end if
        call dis_bound(refat(j),refat(sta),dis2)
        dis = sqrt(dis2)
