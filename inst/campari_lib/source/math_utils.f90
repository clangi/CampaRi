...
!
  logical norm !where or not the cumsum should have the largest value to one
!
  ...
  if (length.gt.1) then
    ...
    if (norm.EQV..true.) then
      ...
      if (err.gt.1.0E-09) then
        write(ilog,*) 'Fatal. Too much mismatch in cumulative normalized sum (1).',j,cumsumvls(j),err
        call fexit()
      end if
    else
      do j=2,length
