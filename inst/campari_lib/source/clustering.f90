...
!$OMP BARRIER
    ...
    call birch_clustering_old(cmode,nstruccls,tpi)
!$OMP BARRIER
#else
    !call birch_clustering(cmode,nstruccls)
    call birch_clustering_old(cmode,nstruccls,azero)
#endif
    call System_Clock(t2)
    if (tpi.le.1) write(ilog,*) 'Time elapsed for clustering and graph-based analyses: ',(t2-t1)/thrdat%rate, ' [s]'
...
!
  ...
  use iounit
  use interfaces
  use threads, ONLY: thr_hlper,thr_rhlper,thrdat,thr_limits
!
  implicit none
!
...
!$OMP BARRIER
  ...
  if (tpi.le.1) then
    ...
    write(*,*) (tmvars(2)-tmvars(1))/thrdat%rate,' s for Clustering #1'
    tmvars(1) = tmvars(2)
  end if
!
  write(*,*) tpi,' so far did ',cnt1
...
!$OMP END DO
!
  write(*,*) tpi,' now did ',cnt1
  if (tpi.le.1) then
    call System_Clock(count=tmvars(2))
    write(*,*) (tmvars(2)-tmvars(1))/thrdat%rate,' s for Clustering #2'
    tmvars(1) = tmvars(2)
  end if
!
...
!$OMP DO SCHEDULE(DYNAMIC,1)
  do bla2=1,bla
    ...
    mm = csnap2clus(mm,ii-1)
    if (maxval(approxmst(bla2)%adj(1:approxmst(bla2)%deg)).eq.62318) write(*,*)  approxmst(bla2)%adj(1:approxmst(bla2)%deg)
...
!$OMP BARRIER
  if (tpi.le.1) then
    call System_Clock(count=tmvars(2))
    write(*,*) (tmvars(2)-tmvars(1))/thrdat%rate,' s for Clustering #3'
    tmvars(1) = tmvars(2)
  end if
...
!$OMP BARRIER
    if (tpi.le.1) then
      call System_Clock(count=tmvars(2))
      write(*,*) (tmvars(2)-tmvars(1))/thrdat%rate,' s for Clustering Refinement'
    end if
  end if
!
  if (tpi.gt.0) then
...
!$OMP BARRIER
  if (tpi.le.1) then
    call System_Clock(count=tmvars(2))
    write(*,*) (tmvars(2)-tmvars(1))/thrdat%rate,' s for Clustering Post-Processing'
  end if
!
  if (modei.ge.0) then
!$OMP SINGLE
...
!
  if (tpi.le.1) then
    call System_Clock(count=tmvars(2))
    write(*,*) (tmvars(2)-tmvars(1))/thrdat%rate,' s for Clustering Rest'
  end if
!
end
!
