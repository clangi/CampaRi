...
!
  integer mode,i,ii,j
  RTYPE dis2,dis,evec(MAXENERGYTERMS),dvec(3),ddd,ca_f(3,n)
!
...
!
  ...
  azero = 0
  afalse = .false.
  rspmatthere = allocated(grid%tmpl)
!
...
! make sure energy3 doesn't seg-fault
  if ((use_cutoffs.EQV..true.).AND.(use_mcgrid.EQV..true.).AND.(rspmatthere.EQV..false.)) then
...
! exit if Cartesian
  if (fycxyz.ne.1) then
    if ((use_cutoffs.EQV..true.).AND.(use_mcgrid.EQV..true.).AND.(rspmatthere.EQV..false.)) deallocate(grid%tmpl)
