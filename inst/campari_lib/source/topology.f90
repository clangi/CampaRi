...
!
  ...
  use params
  use sequen
  use polypep, ONLY: oi,ni,hni,cai,ci
!
  implicit none
!
...
! certain dihedral angles to be 
  do rs=1,nrebuild
    ...
    if ((i1.eq.oi(atmres(i1))).AND.(atmres(i1).lt.rsmol(molofrs(atmres(i1)),2))) then
      ...
      if (fndem(6).EQV..false.) then
        ztor(hni(atmres(i1))) = ztor(ci(atmres(i1))) - 180.
      end if
      if (fndem(7).EQV..false.) then
...
!
 ...
 &could not be read.'/'Atoms in supported residues in immediate sequence context must all be present and readable.')
...
!        
 ...
 &                       xyzdum(:,pdbmap(iz(1,ji))),xyzdum(:,j),ztor(ji))
              ...
              do k=1,j-1
                if (invpdbmap(k).le.0) cycle
                ki = invpdbmap(k)
                if ((iz(1,ki).eq.iz(2,ji)).AND.(ki.ne.iz(1,ji))) then
                  iz(3,ji) = ki
                  call dihed(xyzdum(:,pdbmap(iz(3,ji))),xyzdum(:,pdbmap(iz(2,ji))),&
 &                           xyzdum(:,pdbmap(iz(1,ji))),xyzdum(:,j),ztor(ji))
                  ztor(ji) = ztor(ji)*RADIAN
 ...
 &                           xyzdum(:,pdbmap(iz(1,ji))),xyzdum(:,j),ztor(ji))
                  iz(4,ji) = -1
                  if (ztor(ji).lt.0.0) iz(4,ji) = 1
                  call bondang2(xyzdum(:,k),xyzdum(:,pdbmap(iz(1,ji))),xyzdum(:,j),ztor(ji))
                  ztor(ji) = ztor(ji)*RADIAN
                  exit
                end if
              end do
            ...
            if ((iz(1,ji).eq.iz(1,dihk)).AND.(iz(2,ji).eq.(iz(2,dihk))).AND.(iz(3,dihk).gt.0)) then
              ...
              iz(4,ji) = 0
!             basically an improper (more stable)
              call dihed(xyzdum(:,pdbmap(iz(3,ji))),xyzdum(:,pdbmap(iz(2,ji))),xyzdum(:,pdbmap(iz(1,ji))),&
 &                      xyzdum(:,j),ztor(ji))
              ztor(ji) = ztor(ji)*RADIAN
            end if
            do k=1,j-1
 ...
 &                         xyzdum(:,pdbmap(iz(1,ji))),xyzdum(:,j),ztor(ji))
                if ((iz(1,ki).eq.iz(1,ji))) then
                  ...
                  ztor(ji) = -1
                  if (ztor(ji).lt.0.0) iz(4,ji) = 1
                  call bondang2(xyzdum(:,k),xyzdum(:,pdbmap(iz(1,ji))),xyzdum(:,pdbmap(j)),ztor(ji))
                end if
                ztor(ji) = ztor(ji)*RADIAN
                exit
              end if
...
!     sanity, should be handled by the +1-offset
 ...
 &a Z matrix inferred from an input file with corrupt atom order (unsupported residue).'
        if (n12(at).gt.MAXVLNC) write(ilog,*) '(encountered for atom ',at,')'
        if (n12(at2).gt.MAXVLNC) write(ilog,*) '(encountered for atom ',at2,')'
        call fexit()
      end if
      i12(n12(at),at) = at2
      i12(n12(at2),at2) = at
