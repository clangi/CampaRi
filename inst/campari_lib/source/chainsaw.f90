...
!
  ...
  integer(KIND=8) ee2,ee1,t1,t2,hlp1
...
! This block is a complete shortcut that circumvents the execution of the rest of the program
! Please ignore this when trying to understand what CAMPARI does in the general case
#ifdef LINK_NETCDF
!
  if ((ncdm_donc.EQV..true.).OR.(ncdm_doasconv.EQV..true.)) then 
!
...
!
    ...
    if (ncdm_donc.EQV..true.) then  !mutually exclusive with ncdm_doasconv (see ncdm_sanity_checks_1)
      call ncdm_read_ncfl()
    end if
!
!   --- data mining ---
!
    if ((ncdm_donc.EQV..true.).OR.(ncdm_isdmonas.EQV..true.)) then    !otherwise only conversion was requested
      call ncdm_fill_cludata()    !fill cludata related quantities
      ...
      if ((ncdm_is_periodic.EQV..true.).AND.((cdis_crit.eq.1).OR.(cdis_crit.eq.2))) then
        call ncdm_scale_periodicity()
      end if
!
#ifdef ENABLE_THREADS
...
!
 ...
 &' on ',rc(1)(5:6),'/',rc(1)(7:8),'/',rc(1)(1:4),'.'
    write(*,*) '--------------------------------------------------------------------------'
    stop 0
  end if !if ((ncdm_donc.EQV..true.).OR.(ncdm_doasconv.EQV..true.)) then
#endif
! --------------------------------------------------------------------------------------------
!
  logbuff = use_trajidx
...
!$OMP PARALLEL DEFAULT(SHARED) PRIVATE(istep,tpi,tpn,rs,logdummy,modstep)
  ...
  thr_timings(16,tpi) = firststep+99
!$OMP SINGLE
  curframe = 0
!$OMP END SINGLE
#else
...
!$OMP END SINGLE
    ...
    do rs=1,framecnt
#ifdef ENABLE_THREADS
      if ((rs.eq.thr_timings(16,tpi)).AND.(tpi.eq.1)) then
#else
      if (rs.eq.thr_timings(16,1)) then
        tpi = 1
#endif
        thr_timings(8,tpi) = thr_timings(7,tpi)
        call System_Clock(thr_timings(7,tpi))
        if ((thr_timings(7,tpi).gt.thr_timings(8,tpi)).AND.(thr_timings(16,tpi).gt.thr_timings(15,tpi))) then
          tt = thrdat%rate*86.4e3*1.0*(thr_timings(16,tpi)-thr_timings(15,tpi))/&
   &        (1.0*(thr_timings(7,tpi)-thr_timings(8,tpi))) ! in steps per day
...
#endif
          thr_timings(15,tpi) = thr_timings(16,tpi)
          thr_timings(16,tpi) = thr_timings(16,tpi) + max(1,nint(time_flush*tt/1440.))
        else if (thr_timings(16,tpi).gt.thr_timings(15,tpi)) then
          hlp1 = thr_timings(16,tpi) - thr_timings(15,tpi)
          thr_timings(15,tpi) = thr_timings(16,tpi)
          thr_timings(16,tpi) = thr_timings(16,tpi) + hlp1
        else
          thr_timings(15,tpi) = thr_timings(16,tpi)
          thr_timings(16,tpi) = thr_timings(16,tpi) + 100
        end if
#ifdef ENABLE_THREADS
#else
        tpi = 0
...
!$OMP END SINGLE
...
#ifdef ENABLE_THREADS
    if ((istep.eq.thr_timings(16,tpi)).AND.(tpi.eq.1)) then
#else
    if (istep.eq.thr_timings(16,1)) then
      tpi = 1
#endif
      thr_timings(8,tpi) = thr_timings(7,tpi)
      call System_Clock(thr_timings(7,tpi))
 ...
 &        (1.0*(thr_timings(7,tpi)-thr_timings(8,tpi))) ! in steps per day
        ...
        thr_timings(16,tpi) = thr_timings(16,tpi) + max(1_8,nint(time_flush*tt/1440.,KIND=8))
#else
        thr_timings(16,tpi) = thr_timings(16,tpi) + max(1_INT64,nint(time_flush*tt/1440.,KIND=8))
#endif
      else if (thr_timings(16,tpi).gt.thr_timings(15,tpi)) then
        hlp1 = thr_timings(16,tpi) - thr_timings(15,tpi)
        thr_timings(15,tpi) = thr_timings(16,tpi)
