...
MDEPS3=`grep -H -m 1 "#include \"macros.i\"" ${MODOS} | awk '{print "${LIB_DIR}/${ARCH}/threads/" $1}' | sed s/.i:#include/.o/`
MDEPS4=`grep -H -m 1 "#include \"macros.i\"" ${MODOS} | awk '{print "${LIB_DIR}/${ARCH}/mpi_threads/" $1}' | sed s/.i:#include/.o/`
echo ${MDEPS} ${MDEPS2} ${MDEPS3} ${MDEPS4}: \${SRC_DIR}/macros.i >> ${SRC_DIR}/DEPENDENCIES
