...
!
  implicit none
!
  integer i,k,lastval(100),tpi,tpn,tpn2,shf2,stx(4),aone,afive,kk
!
  aone = 1
  afive = 5
  tpn = thrdat%maxn
...
!   1-4 are static limits in the two most important hierarchy levels: atoms and residues
    thr_limits(1,i) = min(n,nint((i-1)*(1.0*n/(1.0*tpn)))) + 1
    thr_limits(2,i) = max(1,nint(i*(1.0*n/(1.0*tpn))))
    thr_limits(3,i) = min(nseq,nint((i-1)*(1.0*nseq/(1.0*tpn)))) + 1
    thr_limits(4,i) = max(1,nint(i*(1.0*nseq/(1.0*tpn))))
!   5,6 are dynamic bounds for grid-based NBL search
    thr_limits(5,i) = min(grid%pnts,nint((i-1)*(1.0*grid%pnts/(1.0*tpn)))) + 1
    thr_limits(6,i) = max(1,nint(i*(1.0*grid%pnts/(1.0*tpn))))
    if (grid%pnts.le.0) thr_limits(6,i) = 0
!   7,8 are dynamic limits for NBL generation
    thr_limits(7,i) = min(nseq,nint((i-1)*(1.0*nseq/(1.0*tpn)))) + 1
    thr_limits(8,i) = max(1,nint(i*(1.0*nseq/(1.0*tpn))))
!   9,10 are dynamic limits for interactions (global or just IPP/ATTLJ/WCA/SAV) when no twin-range is considered
    ...
    if (i.le.tpn2) then
      thr_limits(9,i) = min(nseq,nint((i-1)*(1.0*nseq/(1.0*tpn2)))) + 1
      thr_limits(10,i) = max(1,nint(i*(1.0*nseq/(1.0*tpn2))))
    else
      thr_limits(9,i) = 1
      thr_limits(10,i) = 0
    end if
    thr_limits(9,i) = min(nseq,nint((i-1)*(1.0*nseq/(1.0*tpn)))) + 1
    thr_limits(10,i) = max(1,nint(i*(1.0*nseq/(1.0*tpn))))
!   11-26 are various limits for holonomic constraint solvers
    if ((settle_tip3ps.gt.0).AND.(i.gt.shf2)) then
      thr_limits(11,i) = min(settle_tip3ps,nint((i-1-shf2)*(1.0*settle_tip3ps/(1.0*tpn2)))) + 1
      thr_limits(12,i) = max(1,nint((i-shf2)*(1.0*settle_tip3ps/(1.0*tpn2)))) 
    else
      thr_limits(11,i) = 1
      thr_limits(12,i) = 0
    end if
    if ((settle_spcs.gt.0).AND.(i.gt.shf2)) then
      thr_limits(13,i) = min(settle_spcs,nint((i-1-shf2)*(1.0*settle_spcs/(1.0*tpn2)))) + 1
      thr_limits(14,i) = max(1,nint((i-shf2)*(1.0*settle_spcs/(1.0*tpn2))))
    else
      thr_limits(13,i) = 1
      thr_limits(14,i) = 0
    end if
    if ((settle_tip4ps.gt.0).AND.(i.gt.shf2)) then
      thr_limits(15,i) = min(settle_tip4ps,nint((i-1-shf2)*(1.0*settle_tip4ps/(1.0*tpn2)))) + 1
      thr_limits(16,i) = max(1,nint((i-shf2)*(1.0*settle_tip4ps/(1.0*tpn2))))
    else
      thr_limits(15,i) = 1
      thr_limits(16,i) = 0
    end if
    if ((settle_tip5ps.gt.0).AND.(i.gt.shf2)) then
      thr_limits(17,i) = min(settle_tip5ps,nint((i-1-shf2)*(1.0*settle_tip5ps/(1.0*tpn2)))) + 1
      thr_limits(18,i) = max(1,nint((i-shf2)*(1.0*settle_tip5ps/(1.0*tpn2))))
    else
      thr_limits(17,i) = 1
      thr_limits(18,i) = 0
    end if
    if ((settle_tip4pes.gt.0).AND.(i.gt.shf2)) then
      thr_limits(19,i) = min(settle_tip4pes,nint((i-1-shf2)*(1.0*settle_tip4pes/(1.0*tpn2)))) + 1
      thr_limits(20,i) = max(1,nint((i-shf2)*(1.0*settle_tip4pes/(1.0*tpn2))))
    else
      thr_limits(19,i) = 1
      thr_limits(20,i) = 0
    end if
    if ((settle_rest.gt.0).AND.(i.gt.shf2)) then
      thr_limits(21,i) = min(settle_rest,nint((i-1-shf2)*(1.0*settle_rest/(1.0*tpn2)))) + 1
      thr_limits(22,i) = max(1,nint((i-shf2)*(1.0*settle_rest/(1.0*tpn2))))
    else
      thr_limits(21,i) = 1
      thr_limits(22,i) = 0
    end if
    thr_limits(23,i) = min(shake_cnt,nint((i-1)*(1.0*shake_cnt/(1.0*tpn)))) + 1
    thr_limits(24,i) = max(1,nint(i*(1.0*shake_cnt/(1.0*tpn))))
    if (shake_cnt.le.0) thr_limits(24,i) = 0
    thr_limits(25,i) = min(cart_cons_grps,nint((i-1)*(1.0*cart_cons_grps/(1.0*tpn)))) + 1
    thr_limits(26,i) = max(1,nint(i*(1.0*cart_cons_grps/(1.0*tpn))))
...
!   27-30 are various bounds for LR electrostatics treatments
    if ((lrel_md.eq.1).OR.(lrel_md.eq.3)) then
      ...
      if (thrdat%subnrs(1).gt.0) then
        ...
        if (i.le.tpn2) then
          thr_limits(27,i+tpn-thrdat%subnrs(1)) = min(nseq,nint((i-1)*(1.0*nseq/(1.0*tpn2)))) + 1
          thr_limits(28,i+tpn-thrdat%subnrs(1)) = max(1,nint(i*(1.0*nseq/(1.0*tpn2))))
        else
          thr_limits(27,i-thrdat%subnrs(1)) = 1
          thr_limits(28,i-thrdat%subnrs(1)) = 0
        end if
      else 
        thr_limits(27,i) = min(nseq,nint((i-1)*(1.0*nseq/(1.0*tpn)))) + 1
        thr_limits(28,i) = max(1,nint(i*(1.0*nseq/(1.0*tpn))))
      end if
      ...
      if (thrdat%subnrs(1).gt.0) then
        ...
        if ((i.le.tpn2).AND.(cglst%ncrs.gt.0)) then
          thr_limits(27,i+tpn-thrdat%subnrs(1)) = min(cglst%ncrs,nint((i-1)*(1.0*cglst%ncrs/(1.0*tpn2)))) + 1
          thr_limits(28,i+tpn-thrdat%subnrs(1)) = max(1,nint(i*(1.0*cglst%ncrs/(1.0*tpn2))))
        else
          thr_limits(27,i-thrdat%subnrs(1)) = 1
          thr_limits(28,i-thrdat%subnrs(1)) = 0
        end if
      else if (cglst%ncrs.gt.0) then
        thr_limits(27,i) = min(cglst%ncrs,nint((i-1)*(1.0*cglst%ncrs/(1.0*tpn)))) + 1
        thr_limits(28,i) = max(1,nint(i*(1.0*cglst%ncrs/(1.0*tpn))))
      else
        thr_limits(27,i) = 1
        thr_limits(28,i) = 0
      end if
 ...
 &having a number of mesh points in the z-dimension that is an integer multiple of the number of threads.'
      ...
      if (thrdat%subnrs(1).gt.0) then 
        tpn2 = thrdat%subnrs(1)
        if (i.le.tpn2) then
          thr_limits(29,i+tpn-thrdat%subnrs(1)) = min(k,nint((i-1)*(1.0*k/(1.0*tpn2)))) + 1
          thr_limits(30,i+tpn-thrdat%subnrs(1)) = max(1,nint(i*(1.0*k/(1.0*tpn2))))
        else
          thr_limits(29,i-thrdat%subnrs(1)) = 1
          thr_limits(30,i-thrdat%subnrs(1)) = 0
        end if
      else
        thr_limits(29,i) = min(k,nint((i-1)*(1.0*k/(1.0*tpn)))) + 1
        thr_limits(30,i) = max(1,nint(i*(1.0*k/(1.0*tpn))))
      end if
    else
      thr_limits(29,i) = 1
      thr_limits(30,i) = 0
...
!   31,32 are dynamic limits for interactions (global or just IPP/ATTLJ/WCA/SAV) when twin-range IS considered
    thr_limits(31,i) = min(nseq,nint((i-1)*(1.0*nseq/(1.0*tpn)))) + 1
    thr_limits(32,i) = max(1,nint(i*(1.0*nseq/(1.0*tpn))))
!   33,34 are dynamic limits for interactions (just POLAR/TABUL) when twin-range IS considered
    thr_limits(33,i) = min(nseq,nint((i-1)*(1.0*nseq/(1.0*tpn)))) + 1
    thr_limits(34,i) = max(1,nint(i*(1.0*nseq/(1.0*tpn))))
!   35,36 are dynamic limits for molecule-based nontrivial operations
    thr_limits(35,i) = min(nmol,nint((i-1)*(1.0*nmol/(1.0*tpn)))) + 1
    thr_limits(36,i) = max(1,nint(i*(1.0*nmol/(1.0*tpn))))
!   37-40 are dynamic limits for residue-based O(N) interactions (bonded and so on)
    ...
    thr_limits(39,i) = min(n,nint((i-1)*(1.0*n/(1.0*tpn)))) + 1
    thr_limits(40,i) = max(1,nint(i*(1.0*n/(1.0*tpn))))
!   41,42 are dynamic limits for interactions (just POLAR/TABUL) when twin-range is not considered
    thr_limits(41,i) = min(nseq,nint((i-1)*(1.0*nseq/(1.0*tpn)))) + 1
    thr_limits(42,i) = max(1,nint(i*(1.0*nseq/(1.0*tpn))))
...
!   45,46 are dynamic limits for residue operations scaling with the number of internal d.o.f.
    thr_limits(45,i) = min(nseq,nint((i-1)*(1.0*nseq/(1.0*tpn)))) + 1
    thr_limits(46,i) = max(1,nint(i*(1.0*nseq/(1.0*tpn))))
!   47-58 are various bounds for SC_DREST and SC_EMICRO
    if (ndrest.gt.0) then
      thr_limits(47,i) = min(ndrest,nint((i-1)*(1.0*ndrest/(1.0*tpn)))) + 1
      thr_limits(48,i) = max(1,nint(i*(1.0*ndrest/(1.0*tpn))))
    else
      thr_limits(47,i) = 1
      thr_limits(48,i) = 0
    end if
    thr_limits(49,i) = min(emgrid%dim(3),nint((i-1)*(1.0*emgrid%dim(3)/(1.0*tpn)))) + 1
    thr_limits(50,i) = max(1,nint(i*(1.0*emgrid%dim(3)/(1.0*tpn))))
    if ((use_EMICRO.EQV..true.).OR.(emcalc.le.nsim)) then
      ...
      thr_limits(51,i) = min(k,nint((i-1)*(1.0*k/(1.0*tpn)))) + 1
      thr_limits(52,i) = max(1,nint(i*(1.0*k/(1.0*tpn))))
    else
      thr_limits(51,i) = 1
      thr_limits(52,i) = 0
    end if
    thr_limits(53,i) = min(n,nint((i-1)*(1.0*n/(1.0*tpn)))) + 1
    thr_limits(54,i) = max(1,nint(i*(1.0*n/(1.0*tpn))))
    if ((use_EMICRO.EQV..true.).OR.(emcalc.le.nsim)) then
      thr_limits(55,i) = min(emgrid%dim(3),nint((i-1)*(1.0*emgrid%dim(3)/(1.0*tpn)))) + 1
      thr_limits(56,i) = max(1,nint(i*(1.0*emgrid%dim(3)/(1.0*tpn))))
    else
      thr_limits(55,i) = 1
      thr_limits(56,i) = 0
    end if
    if (use_EMICRO.EQV..true.) then
      thr_limits(57,i) = min(emcoarsegrid%dim(3),nint((i-1)*(1.0*emcoarsegrid%dim(3)/(1.0*tpn)))) + 1
      thr_limits(58,i) = max(1,nint(i*(1.0*emcoarsegrid%dim(3)/(1.0*tpn))))
    else
      thr_limits(57,i) = 1
      thr_limits(58,i) = 0
    end if
    thr_limits(59,i) = min(nmol,nint((i-1)*(1.0*nmol/(1.0*tpn)))) + 1 ! for fixed cost per mol
    thr_limits(60,i) = max(1,nint(i*(1.0*nmol/(1.0*tpn))))
    thr_limits(61,i) = min(nmol,nint((i-1)*(1.0*nmol/(1.0*tpn)))) + 1 ! for cost ~dof per mol with DLB-12
    thr_limits(62,i) = max(1,nint(i*(1.0*nmol/(1.0*tpn))))
    if (unslst%nr.gt.0) then
      thr_limits(63,i) = min(unslst%nr,nint((i-1)*(1.0*unslst%nr/(1.0*tpn)))) + 1 ! fixed schedule in unslst%nr
      thr_limits(64,i) = max(1,nint(i*(1.0*unslst%nr/(1.0*tpn))))
    else
      thr_limits(63,i) = 1
      thr_limits(64,i) = 0
    end if
    if (unklst%nr.gt.0) then
      thr_limits(65,i) = min(unklst%nr,nint((i-1)*(1.0*unklst%nr/(1.0*tpn)))) + 1 ! fixed schedule in unklst%nr
      thr_limits(66,i) = max(1,nint(i*(1.0*unklst%nr/(1.0*tpn))))
    else
      thr_limits(65,i) = 1
      thr_limits(66,i) = 0
    end if
...
!   85,86 are dynamic limits for the fxn Vforce_dsavdr
    thr_limits(85,i) = min(n,nint((i-1)*(1.0*n/(1.0*tpn)))) + 1
    thr_limits(86,i) = max(1,nint(i*(1.0*n/(1.0*tpn))))
...
!   89,90 are static weights for complete N^2 interactions
    thr_limits(89,i) = -min(n*(n-1)/2,nint((i-1)*(0.5*n*(n-1)/(1.0*tpn)))) + 1
    thr_limits(90,i) = -max(1,nint(i*(0.5*n*(n-1)/(1.0*tpn))))
    kk = 0
    do k=1,nseq
      kk = kk + at(k)%na*(n-(at(k)%bb(1)+at(k)%na-1)) + at(k)%na*(at(k)%na-1)/2
      if (thr_limits(89,i).lt.0) then
        if (kk.ge.-thr_limits(89,i)) thr_limits(89,i) = k + 1
      end if
      if (thr_limits(90,i).lt.0) then
        if (kk.gt.-thr_limits(90,i)) thr_limits(90,i) = k 
      end if
    end do
...
! cluster list
    ...
    if (mode.eq.1) then
      do i=1,nstruccls
        allia = allia + scluster(i)%nb ! sum(scluster(1:nstruccls)%nb) -> cannot respect KIND=8 for allia as written
      end do
    ...
    do i=1,nstruccls
      if (mode.eq.1) then
        if (scluster(i)%nb.le.0) cycle
        laf = i
        if (fif.le.0) fif = i
        allia = allia + scluster(i)%nb
      else if (mode.eq.2) then
...
!
  ...
  integer i,j,k,fiia,laia,stbu(2),topos
  RTYPE myia
!
  laia = 0
  fiia = 0
...
!     this is a legitimate empty specification -> don't fix further
    ...
    do while ((thr_limits(ixes(1),i).eq.thr_limits(ixes(1),i-1)).AND.(thr_limits(ixes(2),i).eq.thr_limits(ixes(2),i-1)).AND.&
 &      (thr_limits(ixes(3),i).eq.thr_limits(ixes(3),i-1)).AND.(thr_limits(ixes(4),i).eq.thr_limits(ixes(4),i-1)).AND.&
 &      (thr_limits(ixes(5),i).eq.thr_limits(ixes(5),i-1)).AND.(thr_limits(ixes(6),i).eq.thr_limits(ixes(6),i-1)))
      if ((thr_limits(ixes(3),k).eq.1).AND.(thr_limits(ixes(4),k).eq.1)) then
        if (fnbl.EQV..true.) then
          thr_limits(ixes(3:4),i) = 2
        else
          thr_limits(ixes(3:4),i) = 5
        end if
      end if
      thr_limits(ixes(5),i) = 1
      thr_limits(ixes(6),i) = 0
      i = i + 1
    end do
    k = k + 1
  end do
...
!
  ...
  integer tpn,i,k,ixx,klo,khi,centeri
...
!
  ...
  do i=1,tpn
    ...
    fixlims(3,i) = min(nsnps,nint((i-1)*(1.0*nsnps/(1.0*tpn)))) + 1
    fixlims(4,i) = max(1,nint(i*(1.0*nsnps/(1.0*tpn))))
  end do
  do k=1,2
    ixx = 1
    do i=2,tpn
...
!$OMP END SINGLE
  if (cmode.ne.7) then
    do i=fixlims(1,tpi),fixlims(2,tpi)
      if (it(i)%nmbrs.eq.1) it(i)%center = it(i)%snaps(1)
...
!$OMP SINGLE
    do i=1,thrdat%maxn
      if (thr_rutil(19,i).lt.it(nint(thr_rutil(20,i)))%nodewt(2)) then
        it(nint(thr_rutil(20,i)))%nodewt(2) = thr_rutil(19,i)
        it(nint(thr_rutil(20,i)))%center = nint(thr_rutil(21,i))
      end if
    end do
!$OMP END SINGLE NOWAIT
  else
...
#endif
#endif
  integer azero,tpi,tpn,i,j,k,OMP_GET_THREAD_NUM,imol,mm,tpnbu,bu1,lo,hi
...
!
  allocate(thr_rhlper(thrdat%maxn*10001,3))
  allocate(refvals(thrdat%maxn*10001,thrdat%maxn+1))
  do j=1,thrdat%maxn
    do i=1,thrdat%maxn*10001
      refvals(i,j) = random()
    end do 
  end do
...
!$OMP END PARALLEL
  ...
  write(ilog,55) 'thr_combinevec','a small FP vector',fr1,' a.u.',fr2,' a.u'
!
  thr_rhlper(:,2) = 0.0
!$OMP PARALLEL DEFAULT(SHARED) PRIVATE(tpi,mm,imol,i,j,k)
  mm = thrdat%maxn*10001
  i = 67
  j = 1
  k = 2
  tpi = OMP_GET_THREAD_NUM()+1
...
!
  allocate(thr_hlper(thrdat%maxn*10001,3))
  allocate(refivals(thrdat%maxn*10001,thrdat%maxn+1))
  do j=1,thrdat%maxn
    do i=1,thrdat%maxn*10001
      refivals(i,j) = random()
    end do 
