...
!
 30    format (a6,i5,1x,a4,a1,a3,1x,a1,i4,4x,3f8.3,1x,f5.2,'  0.00') ! default PDB format, overridden
...
!       write to file but watch for formatting errors (correct crudely)
        ...
        if ((pdb_force_box.gt.1).AND.(bnd_type.eq.1)) then
          ...
          write (ipdb,fpdbstr) tstr,attiwr,ana,extra,resnameprt,chch,rswr,outv(1),outv(2),outv(3)+shf,occup
        else       
          write (ipdb,fpdbstr) tstr,attiwr,ana,extra,resnameprt,chch,rswr,x(ii)+shf3(1),y(ii)+shf3(2),z(ii)+shf3(3),occup
        end if
!
...
!             identify the biotype for C3*
 ...
 &                (b_type(kk).eq.626).OR.(b_type(kk).eq.938)) then
                  if ((pdb_force_box.gt.1).AND.(bnd_type.eq.1)) then
                    ...
                    write (ipdb,fpdbstr) tstr,atti,ana,extra,resnameprt,chch,rs,outv(1),outv(2),outv(3)+shf,occup
                  else       
                    write (ipdb,fpdbstr) tstr,atti,ana,extra,resnameprt,chch,rs,x(kk),y(kk),z(kk)+shf,occup
                  end if
                else
