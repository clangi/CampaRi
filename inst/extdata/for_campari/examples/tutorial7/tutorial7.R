...
# Lennard-Jones correction
nwatertypes <- 1
nsolutetypes <- 3
watereps <- c(0.1521) # OW
soluteeps <- c(0.066,0.17,0.03) # C, O, aliphatic H
watersig <- c(3.15061)
solutesig <- c(3.5,3.12,2.50)
nperwat <- c(1)
npersolute <- c(1,1,3)
waterdens <- c(1085)/(32^3)
cutoff <- 12.0
waterrad <- 1.1
soluterad <- 2.0
sigcombrule <- 2
epscombrule <- 2
ljlr <- 0.0
for (i in 1:nwatertypes) {
  for (j in 1:nsolutetypes) {
    if (sigcombrule == 1) {meansig <- 0.5*(watersig[i]+solutesig[j])}
    else if (sigcombrule == 2) {meansig <- sqrt(watersig[i]*solutesig[j])}
    else {meansig <- 0.5*(watersig[i]+solutesig[j])}
    if (epscombrule == 1) {meaneps <- 0.5*(watereps[i]+soluteeps[j])}
    else if (epscombrule == 2) {meaneps <- sqrt(watereps[i]*soluteeps[j])}
    else {meaneps <- 0.5*(watereps[i]+soluteeps[j])}
    ljlr <- ljlr - nperwat[i]*waterdens*npersolute[j]*(16./3.)*pi*meaneps*((meansig^6)*(1.0/(cutoff+waterrad+soluterad))^3 - (meansig^12)*(1.0/(cutoff+waterrad+soluterad))^9)
  }
}
