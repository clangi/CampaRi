...
!
  ...
  character(3), INTENT(IN):: iname
!
  integer i,seqt_from_name
!
  do i=1,MAXAMINO
    if (i.eq.26) cycle
...
!
  ...
  integer stat,strs,jupp,firs,rsshft,ii,jj,currs,lastrs,alcsz
...
! allocation first
  ...
  alcsz = 3*sum(natres(1:nseq))+100
  allocate(pdbnm(alcsz))
  allocate(rsnam(alcsz))
  allocate(chnam(alcsz))
  allocate(kw(alcsz))
  allocate(rsnmb(alcsz))
  allocate(rsshftl(alcsz))
  allocate(xr(alcsz))
  allocate(yr(alcsz))
  allocate(zr(alcsz))
  allocate(lix(alcsz))
...
!
      call readpdb1_collect(stat,i,pdbnm,xr,yr,zr,pdbxyzs,ok,alcsz)
!
    end do
...
!----------------------------------------------------------------------------------
!
subroutine readpdb1_collect(stat,i,pdbnm,xr,yr,zr,xyzs,ok,sz)
!
  use iounit
  use pdb
  use atoms
...
!
  implicit none
!
  integer, INTENT(IN):: sz,stat,i
  RTYPE, INTENT(IN):: xr(sz),yr(sz),zr(sz)
  character(4), INTENT(IN):: pdbnm(sz)
!
  integer ok(120)
  type(t_pdbxyzs) xyzs
!
