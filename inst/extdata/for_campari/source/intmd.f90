...
!
!   apply fixed fudge factors
    if (fudge_mass.gt.0) then
      j = size(dc_di(imol)%im(:))
      dc_di(imol)%im(4:j) = dc_di(imol)%fudge(4:j)*dc_di(imol)%im(4:j)
    end if
!
...
!   apply fixed fudge factors
    if (fudge_mass.gt.0) then
      ...
      if (mode.eq.1) then
        ...
        dc_di(imol)%olddat(4:j,2) = dc_di(imol)%fudge(4:j)*dc_di(imol)%olddat(4:j,2)
      end if
    end if
!$OMP END SINGLE NOWAIT
...
!
  implicit none
!
  integer imol,j,jm,i,ttc
  RTYPE tmpm
  logical havw(2)
!
  if (fycxyz.ne.1) then
    write(ilog,*) 'Fatal. Called set_mass_fudges() while choice of degrees of freedom&
 & is not equivalent to torsional / rigid-body ones. This is a bug.'
    call fexit()
  end if
!
  if (fudge_mass.eq.0) return
! 
  havw(:) = .false.
  if (fudge_mass.eq.1) then
    do imol=1,nmol
      if (molmass(moltypid(imol)).le.6.0) then
        if (havw(1).EQV..false.) then
          havw(1) = .true.
          write(ilog,*) 'Warning. At least one molecule has such a small mass that fudging was deemed inappropriate &
 &(via FMCSC_FUDGE_DYN). Consider using mass patches (first seen for molecule ',imol,').'
        end if
        cycle
      end if
      if (atmol(imol,1).eq.atmol(imol,2)) cycle
!     treatment of rotation missing for diatomic molecules or other linear molecules
      if ((atmol(imol,2)-atmol(imol,1)).le.1) then
        write(ilog,*) 'Fatal. Diatomic (or other colinear) molecules are not yet supported in internal coordinate &
 &space dynamics.'
        call fexit()
      end if
      jm = size(dc_di(imol)%im)
!     it is important that alignment choices and atomic mass patches are complete
      do j=1,dc_di(imol)%maxntor
        i = dc_di(imol)%recurs(j,1)
        ttc = dc_di(imol)%recurs(izrot(i)%treevs(4),3)
        if ((izrot(i)%treevs(5).eq.0).AND.(dc_di(imol)%im(ttc).le.6.0)) then
          dc_di(imol)%fudge(ttc) = 6.0/dc_di(imol)%im(ttc)
          dc_di(imol)%im(ttc) =  dc_di(imol)%fudge(ttc)*dc_di(imol)%im(ttc)
        else if ((dc_di(imol)%im(ttc).le.6.0).AND.(havw(2).EQV..false.)) then
          write(ilog,*) 'Warning. At least one dihedral angle that is not terminal (and thus has no fixed inertial mass) &
 &has such a small inertial mass in the current configuration that it would be eligible for patching (FMCSC_FUDGE_DYN). &
 &This could lead to instabilities.'
          havw(2) = .true.
        end if
      end do
      tmpm = minval(dc_di(imol)%im(4:6))
      if (tmpm.le.6.0) then
        dc_di(imol)%fudge(4:6) = 6.0/tmpm
        dc_di(imol)%im(4:6) = dc_di(imol)%im(4:6)*dc_di(imol)%fudge(4:6)
      end if
    end do
  else
    write(ilog,*) 'Fatal. Called set_mass_fudges() with an unsupported mode (got ',fudge_mass,'). This is a bug.'
    call fexit()
  end if
!
end
...
!
 30 format('  Mol.  # Ref.   Atoms   Mass')
 31 format('  Mol.  # Ref.   Atoms   Mass      Frozen')
 32 format('  Mol.  # Ref.   Atoms   Mass      Frozen  Rot. Fudge')
 33 format('   Atom # Rotat.  Mass    Unique Rank Order  Parent Ele. Frozen  Dih. Fudge')
 34 format(2x,i7,1x,i6,1x,g10.4,i6,1x,i3,1x,i5,1x,i7,1x,a1,a1,a1,a1,2x,l1,6x,g10.4)
 35 format(2x,i7,1x,i6,1x,g10.4,i6,1x,i3,1x,i5,1x,i7,1x,a1,a1,a1,a1)
 36 format(i7,1x,i7,1x,i7,1x,g10.4,1x,6(l1),2x,g10.4)
 37 format('   Atom # Rotat.  Mass    Unique Rank Order  Parent Ele.')
 38 format('   Atom # Rotat.  Mass    Unique Rank Order  Parent Ele. Frozen')
!
  write(ilog,*)
 ...
 &dc_di(imol)%frz(1:5)
      ...
      if (have_tmd.EQV..true.) then
        if (fudge_mass.gt.0) then
          write(ilog,36) imol,atmol(imol,1),atmol(imol,2)-atmol(imol,1)+1,sum(mass(atmol(imol,1):atmol(imol,2))),&
 &dc_di(imol)%frz(1:6),dc_di(imol)%fudge(4)
        else
          write(ilog,36) imol,atmol(imol,1),atmol(imol,2)-atmol(imol,1)+1,sum(mass(atmol(imol,1):atmol(imol,2))),&
 &dc_di(imol)%frz(1:6)
        end if
...
!    format(i7,1x,i6,1x,
    if (maxval(izrot((atmol(imol,1)+1):atmol(imol,2))%alsz).gt.0) then
      if (have_tmd.EQV..true.) then
        if (fudge_mass.gt.0) then
          write(ilog,33)
        else
    ...
    do i=atmol(imol,1)+1,atmol(imol,2)
      if (izrot(i)%alsz.gt.0) then
        ...
        if (have_tmd.EQV..true.) then
          if (fudge_mass.gt.0) then
            write(ilog,34) i,ixx,tmpm,ixx2,izrot(i)%treevs(5),izrot(i)%treevs(4),izrot(i)%treevs(1),&
 &bio_code(b_type(iz(3,i)))(1:1),bio_code(b_type(iz(2,i)))(1:1),bio_code(b_type(iz(1,i)))(1:1),bio_code(b_type(i))(1:1),&
 &dc_di(imol)%frz(dc_di(imol)%recurs(izrot(i)%treevs(4),3)),dc_di(imol)%fudge(dc_di(imol)%recurs(izrot(i)%treevs(4),3))
          else
            write(ilog,34) i,ixx,tmpm,ixx2,izrot(i)%treevs(5),izrot(i)%treevs(4),izrot(i)%treevs(1),&
 &bio_code(b_type(iz(3,i)))(1:1),bio_code(b_type(iz(2,i)))(1:1),bio_code(b_type(iz(1,i)))(1:1),bio_code(b_type(i))(1:1),&
