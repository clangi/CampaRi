...
!
  do i=1,hmjam%nlst
    if ((hmjam%lst(i).le.1).OR.(hmjam%lst(i).gt.curmaxets).OR.(hmjam%lst(i).eq.4)) then
    ...
    if ((hmjam%lst(i).eq.13).AND.(use_OSMO.EQV..false.)) hmjam%lst(i) = 0
    ...
    if (hmjam%lst(i).eq.13) hmjam%lst(i) = 4 ! because of subsequent hijack, hijack code 4 (becomes 2) for OSMO 
...
!
 ...
 &                                                hmjam%alpha(6,1),hmjam%alpha(6,2)
  ...
  if (hmjam%isin(2).EQV..true.) write(ilog,77) 'OSMO ',hmjam%thresh(2,1),hmjam%thresh(2,2),hmjam%alpha(2,1),hmjam%alpha(2,2)
...
!     generate the required composite terms (otherwise inconsistent Hamiltonian relative to MD)
    ...
    if (eref.lt.hmjam%thresh(dimi,1)) then
      ...
      if (hmjam%alpha(dimi,1).le.0.0) then
        hmjam%boosts(bodimi,mode) = dum
      else
        hmjam%boosts(bodimi,mode) = dum*dum/(hmjam%alpha(dimi,1) + dum)
      end if
      ...
      if (hmjam%alpha(dimi,2).le.0.0) then
        ...
        hmjam%boosts(bodimi,mode) = dum*dum/(dum - hmjam%alpha(dimi,2))
      end if
    end if
  end do
!
...
!
  if (ebias.lt.hmjam%thresh(dimi,1)) then
    ...
    if (hmjam%alpha(dimi,1).le.0.0) then
      hmjam%boosts(bodimi,1) = dum
    else
      hmjam%boosts(bodimi,1) = dum*dum/(hmjam%alpha(dimi,1) + dum)
    end if
    ...
    if (hmjam%alpha(dimi,2).le.0.0) then
      ...
      hmjam%boosts(bodimi,1) = dum*dum/(dum - hmjam%alpha(dimi,2))
    end if
...
!
  ...
  ebias = ebias + hmjam%boosts(bodimi,1)
!
end
!
!----------------------------------------------------------------------------------
...
!
  ...
  eraw = sum(hmjam%evec_thr(bodimi,:))
!
  if (tpi.le.1) hmjam%boosts(bodimi,1) = 0.0
!
  if (hmjam%isin(dimi).EQV..false.) return
!
! energy bias is computed by every thread, but increment of %boosts is restricted
  if (eraw.lt.hmjam%thresh(dimi,1)) then
    ...
    if (hmjam%alpha(dimi,1).le.0.0) then
      if (tpi.le.1) hmjam%boosts(bodimi,1) = dum
    else
      if (tpi.le.1) hmjam%boosts(bodimi,1) = dum*dum/(hmjam%alpha(dimi,1) + dum)
    end if
    dbdU = 1.0 - 2.0*dum/(hmjam%alpha(dimi,1) + dum) + dum*dum/((hmjam%alpha(dimi,1) + dum)**2)
    ...
    if (hmjam%alpha(dimi,2).le.0.0) then
      ...
      if (tpi.le.1) hmjam%boosts(bodimi,1) = dum*dum/(dum - hmjam%alpha(dimi,2))
    end if
    dbdU = 1.0 - 2.0*dum/(dum - hmjam%alpha(dimi,2)) + dum*dum/((dum - hmjam%alpha(dimi,2))**2)
  else
    dbdU = 1.0
...
!
  if (tpi.le.1) then
    if (mode.eq.1) then
      eref = eraw + hmjam%boosts(bodimi,1)
    else if (mode.eq.2) then
      eref = hmjam%boosts(bodimi,1)
