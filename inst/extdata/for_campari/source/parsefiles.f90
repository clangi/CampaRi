...
!
    ...
    if (checkbounds.EQV..true.) then
      if ((mn.gt.n).OR.(mn.le.0)) then
        write(ilog,*) 'Warning. Requested set member ',&
...
!
    ...
    if ((ii.ne.1).AND.(checkbounds.EQV..true.)) then
      if ((mn.gt.n).OR.(mn.le.0)) then
        write(ilog,*) 'Warning. Requested set member ',mn,' for (atom) pair index list, which does not exist. Ignored ...'
        cycle
      end if
    end if
    ...
    if ((ii.ne.1).AND.(checkbounds.EQV..true.)) then
      if ((mn2.gt.n).OR.(mn2.le.0)) then
        write(ilog,*) 'Warning. Requested set member ',mn2,' for (atom) pair index list, which does not exist. Ignored ...'
        cycle
      end if
      if (mn.eq.mn2) then
        write(ilog,*) 'Warning. Requested identical indices (',mn,') for (atom) pair index list. Ignored ...'
        cycle
      end if
    end if
    if (ii.eq.1) then
...
! get the unique, sorted set of atom indices
  ...
  if (algstruc%nr.lt.3) then
    write(ilog,*) 'Warning. Set of atom indices is too small (',algstruc%nr,') for successful alignment. &
 ...
 &an integer multiple of the reseeding interval. Consider changing this.'
  ...
  if (.NOT.((use_MPIAVG.EQV..true.).AND.(use_MPIMultiSeed.EQV..true.))) then
    if (cmaxsnaps.le.3) then
      write(ilog,*) 'Warning. Disabling structural clustering due to insufficient number &
 &of snapshots being analyzed.'
      cstorecalc = nsim + 1
      if (cmode.eq.7) then
        write(ilog,*) 'This is fatal in pure file-based clustering (FMCSC_CMODE is 7).'
        call fexit()
      end if
      return
    end if
   ...
   &        ((unslst%nr.gt.0).OR.(unklst%nr.gt.0))) then
      ...
      if (ttc.le.0) then
        ...
        if (pdb_analyze.EQV..true.) then
          ...
          write(ilog,*) 'Warning. Ignoring request to perform structural clustering.'
          cstorecalc = nsim + 1
          return
        end if
      end if
...
! get the unique, sorted set of atom indices
  ...
  if (pdbeffn.lt.1) then
    write(ilog,*) 'Warning. Set of atom indices for trajectory output has zero size. &
...
!
    ...
    if ((mn.gt.nsim).OR.(mn.le.0)) then
      write(ilog,*) 'Warning. Requested frame number ',mn,' for trajectory frame list,&
...
! get the unique, sorted set of atom indices
  ...
  if (savreq%nats.lt.1) then
    write(ilog,*) 'Warning. Set of atom indices for individual solvent-accessibility histograms has zero size. &
...
!
      do i=1,MAXTORRES
        if ((par_TOR(1,i).gt.180.0).OR.(par_TOR(1,i).lt.-180.0)) then
          ...
          scale_TOR = 0.0
          use_TOR = .false.
          write(ilog,*) 'Warning. Bad minimum position for torsional bias poten&
...
!
        ...
        do i=1,MAXTORRES
          if ((par_TOR(k,i).gt.180.0).OR.(par_TOR(k,i).lt.-180.0)) then
            par_TOR2(k) = 0
            write(ilog,*) 'Warning. Bad minimum position for torsional bias poten&
 &tial for residue ',k,'  (',par_TOR(k,i),'). Turning bias term off.'
            badflag = .true.
          end if
        end do
...
!
        ...
        do i=1,MAXTORRES
          if ((par_TOR(k,i).gt.180.0).OR.(par_TOR(k,i).lt.-180.0)) then
            par_TOR2(k) = 0
            write(ilog,*) 'Warning. Bad minimum position for torsion&
...
!
    ...
    str3 = 'N_'//nod(1:re_aux(10))//'_SAMPLE_TORFILE.dat'
#else
...
!
 ...
 &     (par_POLY(mn,2).gt.1.0).OR.(par_POLY(mn,2).lt.0.0)) then
        par_POLY2(mn) = 0
        write(ilog,*) 'Warning. Bad minimum position for polymeric bias poten&
 &tial (',par_POLY(mn,1),',',par_POLY(mn,2),').'
        write(ilog,*) 'Turning bias term off for mol. #',mn,'.'
      end if
...
!
 ...
 &tial (',par_POLY(mt,1),',',par_POLY(mt,2),').'
        write(ilog,*) 'Turning bias term off for mol. type #',mt,'.'
        cycle
      end if
...
!
    ...
    if (ii.eq.1) cycle
    if ((mn.gt.n).OR.(mn.le.0)) then
      write(ilog,*) 'Warning. Requested atom number ',mn,' for applying a patch to its ',str4(t3:t4),'.&
 ...
 &((mnrt.lt.0.0).AND.(mode.eq.4))) then
      write(ilog,*) 'Warning. Requested an illegal value for patching ',str4(t3:t4),' of atom ',mn,' &
...
!
    ...
    if (ii.eq.1) cycle
    if ((mn.gt.n).OR.(mn.le.0)) then
      write(ilog,*) 'Warning. Requested atom number ',&
...
!
    ...
    if ((mn.gt.nseq).OR.(mn.le.0)) then
      write(ilog,*) 'Warning. Requested residue number ',mn,' for applying a patch to its net charge group parsing.&
 & It does not exist, however, and the request will be ignored ...'
      cycle
    end if
...
!
    ...
    if ((mnrt.ne.1).AND.(mnrt.ne.0)) then
      write(ilog,*) 'Warning. Encountered uninterpretable entry in net charge patch input (',mnrt,'). First number after residue &
...
!
    if ((mn.gt.n).OR.(mn.le.0)) then
      write(ilog,*) 'Warning. Requested atom number ',mn,' for applying a patch to biotype assignment.&
 & It does not exist, however, and the request will be ignored ...'
      cycle
    end if
!
    if (mode.eq.2) then
      if (atmres(mn).ne.rstlst(seqvec(atmres(mn)))) then
        write(ilog,*) 'Warning. Requested atom number ',mn,' for applying a patch to residue type-based biotype assignment.&
...
!
    ...
    if ((mnrt.gt.n_biotyp).OR.(mnrt.le.0)) then
      write(ilog,*) 'Warning. Requested biotype number ',mnrt,' for an atom to be patched, but the request is illegal &
...
!
    ...
    if (ii.eq.1) cycle
    if ((mn.gt.n).OR.(mn.le.0)) then
      write(ilog,*) 'Warning. Requested atom number ',mn,' for applying a patch to steric and dispersion parameters.&
 & It does not exist, however, and the request will be ignored ...'
      cycle
    end if
    str3(1:MAXSTRLEN-ii+1) = str2(ii:MAXSTRLEN)
    ...
    if ((mnrt.gt.n_ljtyp).OR.(mnrt.le.0)) then
      write(ilog,*) 'Warning. Requested LJ type number ',mnrt,' for an atom to be patched, but the request is illegal &
 &(corrupt entry or lack of support in parameter file). Request will be ignored ...'
      cycle
    end if
...
!
  if (nlst.gt.0) then
    ...
    write(ilog,'(1x,a)') 'Warning. Failed to read in any eligible patches (check input).'
    write(ilog,*)
  end if
!
end
...
!
  ...
  use params
  use ISO_FORTRAN_ENV, ONLY: IOSTAT_END
  use system, ONLY: bnd_type,bnd_shape,bnd_params
!
  implicit none
!
  integer iunit,freeunit,iomess,t1,t2,i,ii,datt(2),aone,atwo,daty
  character(MAXSTRLEN) str2,str3
  logical exists,havew(5)
  RTYPE daprm(2)
!
  ndrest = 0
!
...
!
    ...
    if ((datt(1).le.0).OR.(datt(2).eq.0).OR.(datt(2).lt.-3).OR.&
 &      (datt(1).gt.n).OR.(datt(2).gt.n)) then
      write(ilog,*) 'Warning. Illegal entry in distance restraint input (atom&
 & numbers out of range in ',drestfile(t1:t2),').'
      cycle
    else if ((daty.lt.1).OR.(daty.gt.3)) then
      write(ilog,*) 'Warning. Illegal entry in distance restraint input (unsupported&
 & restraint type in ',drestfile(t1:t2),').'
      cycle
    else if ((daprm(1).lt.0.0).AND.(datt(2).gt.0)) then
      write(ilog,*) 'Warning. Illegal entry in distance restraint input (nega&
 &tive minimum distance in ',drestfile(t1:t2),').'
      cycle
    else if ((daty.le.3).AND.(daprm(2).lt.0.0)) then
      write(ilog,*) 'Warning. Got negative force constant for (half-)harmonic restraint in distance restrai&
...
!
 23   format('Atom  #',i7,' (',a4,' in ',a3,') in ',a,'-dimension.')
 24   format('Atoms #',i7,' (',a4,' in ',a3,') and '&
 &                 ,i7,' (',a4,' in ',a3,'))')
 25   format(8x,'Type: ',i2,' REF: ',f7.2,' K_d: ',f7.2)
 55   format(1x,'Further warnings of this type are ommitted.')
!
  havew(:) = .false.
  if (bnd_type.eq.1) then
    do i=1,ndrest
      if (dresat(i,2).gt.0) then
        if (havew(1).EQV..false.) then
          write(ilog,*) 'Warning. Distance restraints in any periodic boundary condition can imply force discontinuities &
 &due to the minimum image convention.'
          write(ilog,55)
          havew(1) = .true.
        end if
        if ((havew(3).EQV..false.)) then
          if (((bnd_shape.eq.1).AND.(dresprm(i,1).ge.minval(bnd_params(1:3))/2.)).OR.&
 &            ((bnd_shape.eq.3).AND.(dresprm(i,1).ge.bnd_params(6)/2.0))) then
            write(ilog,*) 'Warning. At least one distance restraint has a reference distance that exceeds half of the shortest &
 &periodic dimension in the system. This virtually guarantees force discontinuities.'
            write(ilog,55)
            havew(3) = .true.
          end if
        end if
      else if ((bnd_shape.eq.1).OR.((bnd_shape.eq.3).AND.(dresat(i,2).eq.-3))) then
        if (havew(4).EQV..false.) then
          write(ilog,*) 'Warning. Position restraints in any periodic boundary dimension can imply force and even energy&
 & discontinuities due to the potential being replicated periodically as well (minimum image convention).'
          write(ilog,55)
          havew(4) = .true.
        end if
        if ((havew(2).EQV..false.).AND.(drestyp(i).gt.1)) then
          write(ilog,*) 'Warning. One-sided (half-harmonic) position restraints are guaranteed to create an energy &
 &and force discontinuity along a periodic dimension (this can only be masked by a suitably chosen one-sided potential &
 & of the opposite type acting on the same atom).'
          write(ilog,55)
          havew(2) = .true.
        end if
      end if
  ...
  if (drest_report.EQV..true.) then
    write(ilog,*)
    write(ilog,*) '-- Summary of Position and/or Distance Restraint Terms in the System --'
    write(ilog,*)
    do i=1,ndrest
      if (dresat(i,2).eq.-1) then
        write(ilog,23) dresat(i,1),bio_code(b_type(dresat(i,1))),&
 &                   amino(seqtyp(atmres(dresat(i,1)))),'x'
      else if (dresat(i,2).eq.-2) then
        write(ilog,23) dresat(i,1),bio_code(b_type(dresat(i,1))),&
 &                   amino(seqtyp(atmres(dresat(i,1)))),'y'
      else if (dresat(i,2).eq.-3) then
        write(ilog,23) dresat(i,1),bio_code(b_type(dresat(i,1))),&
 &                   amino(seqtyp(atmres(dresat(i,1)))),'z'
      else
...
!       the atom-specific format is simple enough
 ...
 &          (dum(2).le.0).OR.(dum(2).gt.n).OR.(dum(3).le.0)) then
          write(ilog,*) 'Warning. Range exception occurred while reading GPC &
 &input:'
...
!       this has to be the first mol. of its type
        if (imol.eq.jmol) then
          if (imol.ne.molangr(an_grp_mol(imol),1)) then
            write(ilog,*) 'Warning. Atoms for intramolecular terms have to co&
...
!       both mol.s have to be the first occurrence of their respective group
 ...
 &            (jmol.ne.molangr(an_grp_mol(jmol),1))) then
            write(ilog,*) 'Warning. Atoms for intermolecular terms have&
...
!       the first mol has to be the first in its group, the second the second
          ...
          if (imol.ne.molangr(an_grp_mol(imol),1)) then
            write(ilog,*) 'Warning. Atoms for group-internal intermolecular terms have &
...
!     intermolecular type-wise requests
 ...
 &          (typ2.le.0).OR.(typ2.gt.nangrps)) then
          write(ilog,*) 'Warning. Range exception occurred while processing GPC input:'
          write(ilog,74) abc(1:1),typ1,typ2
          cycle
        end if
        if (typ1.eq.typ2) then
          if (molangr(typ1,2).eq.1) then
            write(ilog,*) 'Warning. Intermolecular term for analysis group ',&
...
!
    ...
    write(ilog,*) 'Warning. Unknown mode for general PC analysis (got: ',gpc%mode,'). Turning feature off.'
    gpc%nos = 0
    close(unit=iunit)
    return
  end if
...
!
  ...
  if (gpc%nos.eq.0) then
    write(ilog,*) 'Warning. No general PCs requested in ',pccodefile(t1:t2),'? Turning feature off.'
    deallocate(pclst)
    call allocate_gpcpre(atwo)
    return
  end if
...
!
  if (gpc_report.EQV..true.) then
    ...
    if ((use_REMC.EQV..true.).AND.(myrank.eq.0)) then
      call int2str(myrank,nod,re_aux(10))
      str3 = 'N_'//nod(1:re_aux(10))//'_GENERAL_PC.idx'
...
!
  if (tabul_report.EQV..true.) then
    ...
    if ((use_REMC.EQV..true.).AND.(myrank.eq.0)) then
      call int2str(myrank,nod,re_aux(10))
      str3 = 'N_'//nod(1:re_aux(10))//'_TABULATED_POT.idx'
    else if ((use_MPIAVG.EQV..true.).AND.(myrank.eq.0)) then
...
! first bail-out for Cartesian dynamics -> such constraints are not supported via this path
 ...
 &would be the presence of additional, holonomic constraints (-> FMCSC_SHAKESET) to match the MC constraint set exactly.'
    ...
    if ((abc(1:1).eq.'a').OR.(abc(1:1).eq.'A')) then
      write(ilog,*) 'Warning. In hybrid internal coordinate space simulations, atom-specific constraint requests (mode "A") will &
 &only be active for the dynamics portion (use a different mode to create a consistent set of constraints).'
    end if
  end if
!
  ncons(:) = 0 
...
!
      ...
      if ((mn.le.0).OR.(mn.gt.nmol)) then
        write(ilog,*) 'Warning. Requested constraints for molecule ',mn,', which does not exist. Skipping ...'
        cycle
      end if
      if (ii.eq.1) exit
...
!
 ...
 &grees of freedom, which could be constrained -> ignored.'
        ...
        write(ilog,*) 'Warning. Unknown selection for constraint request for &
...
!
      ...
      if ((mt.le.0).OR.(mt.gt.nmoltyp)) then
        write(ilog,*) 'Warning. Requested constraints for molecule type ',mt,', which does not exist. Skipping ...'
        cycle
      end if
      if (ii.eq.1) exit
      call extract_abc(str2,dofs,ii)
...
!
      ...
      if ((rs.le.0).OR.(rs.gt.nseq)) then
        write(ilog,*) 'Warning. Requested constraints for residue ',rs,', which does not exist. Skipping ...'
        cycle
      end if
      if (ii.eq.1) exit
      call extract_abc(str2,dofs,ii)
...
!
      ...
      if ((rs.le.0).OR.(rs.gt.n)) then
        write(ilog,*) 'Warning. Requested constraints for atom ',rs,', which does not exist. Skipping ...'
        cycle
      end if
      if (ii.eq.1) exit
      call extract_abc(str2,dofs,ii)
...
!
      if (fycxyz.eq.2) then
        if (dofs.eq.'XYZ') then
          ...
          write(ilog,*) 'Warning. Unknown selection for Cartesian space constraint request for &
 ...
 &ed in torsional/rigid-body dynamics. Check back later.'
          ...
          write(ilog,*) 'Warning. Unknown selection for internal coordinate space constraint request for &
...
!
      ...
      call extract_int(str2,mn,ii)
      if ((mn.gt.nmol).OR.(mn.le.0)) then
        write(ilog,*) 'Warning. Requested ghosting for molecule ',mn,', which does not exist. Ignored ...'
        cycle
      end if
      if (ii.eq.1) exit
!
...
!     attempt to read flag for full decoupling overwrite
      ...
      if (decfl.eq.0) then
       do rs=rsmol(mn,1),rsmol(mn,2)
        ...
        write(ilog,*) 'Warning. Incomprehensible input for extra flag in ghost particle input. Ignored ...'
      end if
!
    end do
!
...
!
      ...
      call extract_int(str2,mt,ii)
      if ((mt.gt.nmoltyp).OR.(mt.le.0)) then
        write(ilog,*) 'Warning. Requested ghosting for molecule type',mt,', which does not exist. Ignored ...'
        cycle
      end if
      if (ii.eq.1) exit
!
...
!
      ...
      if ((rs.gt.nseq).OR.(rs.le.0)) then
        write(ilog,*) 'Warning. Requested ghosting for residue',rs,', which does not exist. Ignored ...'
        cycle
      end if
      if (ii.eq.1) exit
!
...
!
      ...
      if ((kk.gt.n).OR.(kk.le.0)) then
        write(ilog,*) 'Warning. Requested constraint for atom ',kk,', which does not exist. Ignored ...'
        cycle
      end if
 ...
 & provided only for the instances of atoms in the first molecule of each type.'
      ...
      if ((jj.gt.n).OR.(jj.le.0)) then
        write(ilog,*) 'Warning. Requested constraint for atom ',jj,', which does not exist. Ignored ...'
        cycle
      end if
 ...
 & provided only for the instances of atoms in the first molecule of each type.'
      ...
      if (add_settle.EQV..true.) then
        if ((atmres(kk).eq.atmres(jj)).AND.((seqtyp(atmres(kk)).eq.39).OR.(seqtyp(atmres(kk)).eq.40))) then
          write(ilog,*) 'Warning. Requested constraint for (exemplary) atoms ',&
 ...
 !
    ...
    do while (1.eq.1)
      ...
      if (iomess.eq.IOSTAT_END) then
        ...
        write(ilog,*) 'Fatal. File I/O error while processing SHAKE/LINCS input (got: ',cart_cons_file(t1:t2),').'
        call fexit()
      end if
!
      ii = 1
      ...
      if (add_settle.EQV..true.) then
        if ((atmres(kk).eq.atmres(jj)).AND.((seqtyp(atmres(kk)).eq.39).OR.(seqtyp(atmres(kk)).eq.40))) then
          write(ilog,*) 'Warning. Requested constraint for atoms ',kk,' and ',jj,' which will be covered by SETTLE. Ignored ...'
          cycle
        end if
      end if
      nidx = nidx + 1
...
! Initialize data structures
  ...
  do typid = 1, nmoltyp
    thermalvolume(typid) = (planckconstant * sqrt(invtemp/2/PI/molmass(typid))) ** 3
  end do
  do imol = 1, nmol
    call insertmol(imol)
  end do
...
! &initialnumber, ' mu: ', chempot(typid), 'kcal/mol'
    ...
    do while (cardinality(typesets(typid,1)).gt.initialnumber)
      call deletemol(getmember(typesets(typid,1), cardinality(typesets(typid,1)) ))
    end do
  end do
!
...
!
 ...
 &otential is ',g12.5,' kcal/mol. The starting concentration is (atmost) ',g14.6,'mM.')
 75   format('Its expected average as well as starting number is ',i7,' molecules and its excess chemical p&
 &otential is ',g12.5,' kcal/mol. The implied bulk concentration is (atmost) ',g14.6,'mM.')
!
 ...
 &chempot(fluctypes%array(i)),cardinality(typesets(fluctypes%array(i),1))/(ens%insV*avogadro*1.0e-30)
...
!
  ...
  use polypep, ONLY: at
  use system, ONLY: ens
  use units, ONLY: avogadro
  use ISO_FORTRAN_ENV, ONLY: IOSTAT_END
!
  implicit none
!
  integer iunit,freeunit,iomess,t1,t2,mm,mt,rs,ii,compi
  character(10) abc
  character(MAXSTRLEN) str2,str3
  logical exists,havem
  RTYPE netdens(8)
!
  call strlims(osmofile,t1,t2)
  inquire(file=osmofile(t1:t2),exist=exists)
  if (exists.EQV..false.) then
    write(ilog,*) 'WARNING: Cannot open spec.d input file (',osmofile(t1:t2),') for FMCSC_OSMOFILE. Disabled.'
    use_OSMO = .false.
    return
  end if
!
...
!
  iunit = freeunit()
  open (unit=iunit,file=osmofile(t1:t2),status='old')
!
  read(iunit,79,iostat=iomess) str2
  if (iomess.eq.IOSTAT_END) then
    write(ilog,*) 'Warning. Got empty/incomplete file for compartmentalization potential input (',osmofile(t1:t2),'). &
 &Turning off term.'
    use_OSMO = .false.
    close(unit=iunit)
    return
  else if (iomess.eq.2) then
    write(ilog,*) 'Fatal. File I/O error while processing compartmentalization potential input (got: ',osmofile(t1:t2),').'
    call fexit()
  end if
!
  ii = 1
...
!
      ...
      mt = -1
      call extract_int(str2,mt,ii)
      if ((mt.gt.nmol).OR.(mt.le.0)) then
        write(ilog,*) 'Warning. Requested compartmentalization potential for molecule type',mt,', which does not exist. &
 &Ignored ...'
        cycle
      end if
      if (ii.eq.1) exit
!
      ...
      ii = 1
      compi = -1
      call extract_int(str3,compi,ii)
      if ((compi.lt.0).OR.((compi.gt.2).AND.(par_OSMO(1).le.1)).OR.((compi.gt.4).AND.(par_OSMO(1).le.2)).OR.&
 &                        ((compi.gt.8).AND.(par_OSMO(1).le.3))) then
        write(ilog,*) 'Warning. Incomprehensible input for compartmentalization potential for molecule typ ',mt,'. Ignored ...'
      else
        do mm=1,nmol
          if (moltypid(mm).eq.mt) then
            do rs=rsmol(mm,1),rsmol(mm,2)
              par_OSMO2(rs) = compi
            end do
          end if
        end do
  ...
  else if ((abc(1:1).eq.'M').OR.(abc(1:1).eq.'m')) then
    allocate(par_OSMO2(nseq))
    par_OSMO2(:) = 0
    do while(.true.)
      read(iunit,79,iostat=iomess) str2
...
!
      ii = 1
      mm = -1
      call extract_int(str2,mm,ii)
      if ((mm.gt.nmol).OR.(mm.le.0)) then
        write(ilog,*) 'Warning. Requested compartmentalization potential for molecule',mm,', which does not exist. Ignored ...'
        cycle
      end if
      if (ii.eq.1) exit
!
      ...
      if ((compi.lt.0).OR.((compi.gt.2).AND.(par_OSMO(1).le.1)).OR.((compi.gt.4).AND.(par_OSMO(1).le.2)).OR.&
 &                        ((compi.gt.8).AND.(par_OSMO(1).le.3))) then
        write(ilog,*) 'Warning. Incomprehensible input for compartmentalization potential for molecule ',mm,'. Ignored ...'
      else
        do rs=rsmol(mm,1),rsmol(mm,2)
          par_OSMO2(rs) = compi
        end do
...
!
      ...
      call extract_int(str2,rs,ii)
      if ((rs.gt.nseq).OR.(rs.le.0)) then
        write(ilog,*) 'Warning. Requested compartmentalization potential for residue',rs,', which does not exist. Ignored ...'
        cycle
      end if
      if (ii.eq.1) exit
!
      ...
      if ((compi.lt.0).OR.((compi.gt.2).AND.(par_OSMO(1).le.1)).OR.((compi.gt.4).AND.(par_OSMO(1).le.2)).OR.&
 &                        ((compi.gt.8).AND.(par_OSMO(1).le.3))) then
        write(ilog,*) 'Warning. Incomprehensible input for compartmentalization potential for residue ',rs,'. Ignored ...'
      else
        par_OSMO2(rs) = compi
      end if
    end do
!
    ...
    write(ilog,*) 'Unknown mode identifier while reading first line of file ',osmofile(t1:t2),': ',abc(1:1),'! This is fatal.'
    close(unit=iunit)
    call fexit()
  end if
  close(unit=iunit)
...
!
  if (havem.EQV..false.) then
    write(ilog,*) 'Warning. No eligible molecules or residues requested in ',osmofile(t1:t2),'? Turning off compartmentalization &
 &potential ...'
    use_OSMO = .false.
    return 
  end if
!
 45   format(4x,'Residue #',i7,' (',a3,'): free')
 46   format(4x,'Residue #',i7,' (',a3,'): ',i3)
 47   format(4x,'Compartment #',i3,'  holds a formal, expected bulk density of ',g12.5,'g/cm^3 (includes any possible patches).')
!
  if (osmo_report.EQV..true.) then
    write(ilog,*)
    write(ilog,'(a,a,a)') '-- List of residues and their target compartments (from ',osmofile(t1:t2),') --'
    do rs=1,nseq
      if (par_OSMO2(rs).gt.0) then
        write(ilog,46) rs,amino(seqtyp(rs)),par_OSMO2(rs)
      else
        write(ilog,45) rs,amino(seqtyp(rs))
      end if
    end do
    ...
    havem = .false.
    do rs=1,nseq
      if (par_OSMO2(rs).eq.0) then
        netdens(:) = netdens(:) + sum(mass(rsinfo(rs,1):(rsinfo(rs,1)+at(rs)%na-1)))/(2.0**par_OSMO(1))
        havem = .true.
      else
        netdens(par_OSMO2(rs)) = netdens(par_OSMO2(rs)) + sum(mass(rsinfo(rs,1):(rsinfo(rs,1)+at(rs)%na-1)))
      end if
    end do
    do rs=1,2**par_OSMO(1)
      write(ilog,47) rs,netdens(rs)*(2**par_OSMO(1))/(ens%insV*avogadro*1.0e-24)
    end do
    if (havem.EQV..true.) write(ilog,'(4x,a)') 'This assumes that free particles are distributed homogeneously across partitions.'
    write(ilog,'(a)') '-----------------------------------------------------------------------------------------'
    write(ilog,*)
  end if
!
