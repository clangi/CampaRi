...
!
  ...
  use paircorr
  use clusters
  use grandensembles, ONLY: particlenumcalc,particlenuminst
...
!
  implicit none
!
  integer freeunit,lext,mode,mt,imol,lext2
  logical exists,doperser
  character(int(log10(1.0*nangrps) + 1)) mtstr
  character(60) dihedfile,polmrfile
  character(60) holesfile,accfile,trcvfile
  character(60) persfile,sasafile,enfile,dsspfile
  character(60) phtfile,ensfile,elsfile,pcfile,rmsdfile,numfile
!  character(60) bprtnrfile
  logical fycopen,polymopen,ensopen,dsspopen,&
 &accopen,enopen,persopen,holesopen,sasaopen,phtopen,elsopen,pcopen,rmsdopen,numopen!,bpopen
#ifdef ENABLE_MPI
...
! including initialization (once(!)) 
 ...
 &holesopen/.false./,sasaopen/.false./,phtopen/.false./,elsopen/.false./,pcopen/.false./,rmsdopen/.false./,numopen/.false./
!,bpopen/.false./
#ifdef ENABLE_MPI
...
!
 ...
 &phtopen,dsspopen,elsopen,pcopen,rmsdopen,numopen!,bpopen
#ifdef ENABLE_MPI
...
!   in this case we have to open the output files for each node individually
    if (mode .eq. 1) then
      ...
      numfile = 'N_'//xpont(1:lext)//'_PARTICLENOS.dat'
...
!
! setup filehandle for on-the-fly particle numbers output
      inquire(file=numfile,exist=exists)
      if (exists.EQV..true.) then
        idihed_ring = freeunit()
        open (unit=idihed_ring,file=numfile,status='old')
        close(unit=idihed_ring,status='delete')
      end if
      if ((particlenumcalc.le.nsim).AND.(particlenuminst.gt.0).AND.(particlenuminst.le.(nsim/particlenumcalc))) then
        idihed_ring = freeunit()
        open (unit=idihed_ring,file=numfile,status='new')
        numopen = .true.
      end if
!
...
! setup filehandle for temporary TRCV (covariance of torsions) file
      if (covcalc.le.nsim) then
        do mt=1,nangrps
          call int2str(mt,mtstr,lext2)
          trcvfile = 'N_'//xpont(1:lext)//'_TRCV_'//mtstr(1:lext2)//'.tmp'
...
!   close files and free up filehandles
      ...
      if (numopen) close(unit=idihed_ring)
...
! setup filehandle for temporary TRCV (covariance of torsions) file
      if ((covcalc.le.nsim).AND.(myrank.eq.masterrank)) then
        do mt=1,nangrps
          call int2str(mt,mtstr,lext2)
          trcvfile = 'TRCV_'//mtstr(1:lext2)//'.tmp'
          inquire(file=trcvfile,exist=exists)
          if (exists.EQV..true.) then
            itrcv(mt) = freeunit()
            open (unit=itrcv(mt),file=trcvfile,status='old')
