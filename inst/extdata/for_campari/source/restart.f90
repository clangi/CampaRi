...
!
  ...
  if (isone.EQV..true.) then
    dumpfile = 'N_'//nod(1:re_aux(10))//'_'//basename(1:bleng)//'_1.rst'
    isone = .false.
  else
    dumpfile = 'N_'//nod(1:re_aux(10))//'_'//basename(1:bleng)//'_2.rst'
    isone = .true.
  end if
  ...
  if (exists.EQV..true.) then
    ...
    if (iomess.ne.0) then
      ...
      open(unit=irst,file='N_'//nod(1:re_aux(10))//'_RESTART.tmp',status='replace',iostat=iomess2)
      ...
      do imol=1,nmol
        ...
        if ((dyn_mode.eq.3).OR.(dyn_mode.eq.7)) then
          if (fudge_mass.gt.0) then
            do k=1,intdim 
              write(irst,23) dc_di(imol)%v(k),dc_di(imol)%im(k),dc_di(imol)%fudge(k)
          ...
          if (fudge_mass.gt.0) then
            do k=1,intdim
              write(irst,14) dc_di(imol)%v(k),dc_di(imol)%im(k),dc_di(imol)%olddat(k,2),dc_di(imol)%fudge(k)
            end do
          else
            do k=1,intdim 
...
#endif
  character(60) dumpfile
  logical exists,ismember,atrue,havw
!
  azero = 0
  atrue = .true.
  havw = .false.
...
#ifdef ENABLE_MPI
  call int2str(myrank,nod,re_aux(10))
  dumpfile = 'N_'//nod(1:re_aux(10))//'_'//basename(1:bleng)//'.rst'
  call strlims(dumpfile,t1,t2)
  inquire(file=dumpfile(t1:t2),exist=exists)
...
!   if hybrid, read-in current cycle
      ...
      do imol=1,nmol
        ...
        if ((dyn_mode.eq.3).OR.(dyn_mode.eq.7)) then
          if (fudge_mass.gt.0) then
            do k=1,intdim
              ftmp = -1.0
              read(irst,23) dc_di(imol)%v(k),dc_di(imol)%im(k),ftmp
              if (ftmp.gt.0.) then
                dc_di(imol)%fudge(k) = ftmp
              else if (havw.EQV..false.) then
                write(ilog,*) 'Warning. Failed to read fudge factor for inertial mass in at least one instance. This could mean &
 &that a run was restarted while changing FMCSC_FUDGE_DYN. This means that fudge factors are determined exactly as they would &
 &be at the beginning of a new run. Further warnings of this type are omitted.'
                havw = .true.
              end if
 ...
 &be at the beginning of a new run. Further warnings of this type are omitted.'
          ...
          if (fudge_mass.gt.0) then
            ...
            do k=1,intdim
              ftmp = -1.0
              read(irst,14) dc_di(imol)%v(k),dc_di(imol)%im(k),dc_di(imol)%olddat(k,2),ftmp
              if (dc_di(imol)%olddat(k,2).le.0.0) then ! only (at most) two entries
                dc_di(imol)%olddat(k,2) = dc_di(imol)%im(k)
                if (havw.EQV..false.) then
                  write(ilog,*) 'Warning. Failed to read fudge factor for inertial mass in at least one instance. This could mean &
 &that a run was restarted while changing FMCSC_FUDGE_DYN. This means that fudge factors are determined exactly as they would &
 &be at the beginning of a new run. Further warnings of this type are omitted.'
                  havw = .true.
                end if
              else if (ftmp.le.0.0) then
                write(ilog,*) 'Fatal. In reading inertial mass data for IMD, only 3/4 expected values were found. This is &
 &ambiguous and could be related to changes in either FMCSC_FUDGE_DYN or FMCSC_TMD_INT2UP across restarts. It is &
 &recommended to manually edit the restart file or to use keyword FMCSC_RST_MC2MD.'
                call fexit()
              else
                dc_di(imol)%fudge(k) = ftmp
              end if
