...
! hsl_ma48_ma50_internal_single (based on ma50)
! hsl_ma48_ma48_internal_single (based on ma48)
! hsl_ma48_ma51_internal_single (based on ma51)
! hsl_ma48_single (the user facing module)
!

module hsl_ma48_ma50_internal_single
   use hsl_zb01_single, zb01_real_info => zb01_info
...
!      space allocated.
   ...
   private
   ! public for user
   public :: ma50a, ma50b, ma50c
   ! public for unit tests
   public :: ma50e, ma50f, ma50g, ma50h, ma50i
...
SUBROUTINE ma50a(m,n,ne,la,a,irn,ljcn,jcn,iq,cntl,icntl,ip,np, &
    jfirst,lenr,lastr, &
...
!    floating-point operations needed for the factorization.
  ...
  integer(long) :: eye, i, idummy, iend, ifill, ifir, ii, ij, &
    ...
    jnew, jpiv, jpos, j1, j2, l, lc, lenpiv, lp, lr
...
! if (m<1 .or. n<1) go to 690
  ...
  if (la<ne) go to 710
!!    la = multiplier*la
!!    call ZB01_resize1(a,la,zb01info)
!!      if (zb01info%flag .lt. 0) then
!!        info(1) = -2*n
!!        return
!!      endif
!!      call ZB01_integer(irn,la,zb01intinfo)
...
! Check to see if reduced matrix should be considered as full.
    if (nered>=(min(cntl(1),one)*real(n-nord))*real(m-mord)) go to 640
...
! NEPR is number of entries in strictly U part of pivot row.
      nepr = lenr(ipiv) - 1
      nefact = nefact + nepr
      rinfo(1) = rinfo(1) + cpiv*2 + real(lenr(ipiv))
      j1 = ip(ipiv)
...
! Call compress and reset pointers
        call ma50d(a,irn,iq,n,dispc,.true.)
        jbeg = iq(j)
        jend = jbeg + lenc(j) - 1
        pivbeg = iq(jpiv)
        pivend = pivbeg + lenc(jpiv) - 1
...
! Compress row file array.
            info(2) = info(2) + 1
            call ma50d(a,jcn,ip,m,dispr,.false.)
!!!! Call to HSL_ZB01
...
640 info(5) = mord + min(m-mord-nulli,n-nord-nullj)
  do 650 l = 1, min(m-mord,n-nord)
    rinfo(1) = rinfo(1) + real(m - mord - l + 1,kind=wp) +  &
                          real(m-mord-l,kind=wp)*real(n-nord-l,kind=wp)*2.0e0
650 continue
  np = nord
...
! Test for rank deficiency
  ...
  if (mp>0 .and. icntl(3)>2) then
    write (mp,'(A,I6,A,F12.1/A,7I8)') ' Leaving MA50A with NP =', np, &
...
!690 info(1) = -1
!  if (lp>0) write (lp,'(/A/(2(A,I8)))') ' **** Error return from MA50A ****', &
!    ' M =', m, ' N =', n
!  go to 750
700 info(1) = -2
  if (lp>0) write (lp,'(/A/(A,I10))') ' **** Error return from MA50A ****', &
    ' NE =', ne
  go to 750
...
!720 info(1) = -4
!  if (lp>0) write (lp,'(/A/(3(A,I9)))') ' **** Error return from MA50A ****', &
...
750 end  subroutine ma50a


SUBROUTINE ma50b(m,n,ne,job,aa,irna,iptra,cntl,icntl,ip,iq,np,lfact,fact, &
...
!! the blocking (for BTF) in ma48b to work.  We may change this later.
  real(wp) :: fact(lfact)
...
!      holds the vector IPIV output by MA50G.
...
! end if
  if (ne<=0) then
    info(1) = -2
    if (lp>0) write (lp,'(/A/A,I6)') ' **** Error return from MA50B ****', &
      ' NE =', ne
    go to 550
  end if
  ...
  if (job==1) then
    ...
    if (irnf(1)/=0) then
      info(1) = -6
      if (lp>0) write (lp,'(/A/A,I1,A)') ' **** Error return from MA50B ***', &
        ' Call with JOB=', job, &
    ...
    info(1) = -5
    if (lp>0) write (lp,'(/A/A,I2)') ' **** Error return from MA50B ****', &
      ' JOB =', job
    go to 550
  end if
...
! Print input data
  if (mp>0) then
    if (icntl(3)>2) write (mp, &
      ...
      ' Entering MA50B with M =', m, ' N =', n, ' NE =', ne, ' JOB =', job, &
...
140   continue
      rinfo(1) = rinfo(1) + one + 2*real(iptrl(j)-eye1)
150 continue
...
! Factorize full block
    if (icntl(5)>1) call ma50g(mf,nf,fact(if1),mf,icntl(5),cntl(4), &
      irnf(if1+mf),rank)
    if (icntl(5)==1) call ma50f(mf,nf,fact(if1),mf,cntl(4),irnf(if1+mf),rank)
    if (icntl(5)<=0) call ma50e(mf,nf,fact(if1),mf,cntl(4),irnf(if1+mf),rank)
    info(5) = info(5) + rank
    do 450 i = 1, min(mf,nf)
      rinfo(1) = rinfo(1) + real(mf - i + 1,kind=wp) + &
                            real(mf-i,kind=wp)*real(nf-i,kind=wp)*2.0e0
450 continue
  end if
  if (info(5)<min(m,n)) info(1) = 1
  if (mp>0 .and. icntl(3)>2) then
    write (mp,'(A,I6,A,F12.1/A,I3,A,4I8)') ' Leaving MA50B with IRNF(2) =', &
...
520 info(1) = -3
  ...
  go to 550
530 info(1) = -(7+k)
  if (lp>0) write (lp,'(/A/A,I6,A)') ' **** Error return from MA50B **** ', &
    ' Small pivot found in column', k, ' of the permuted matrix.'
!  go to 550
!!!
! Again this error can only happen if MA50 is tested separately
!540 info(1) = -4
!  if (lp>0) write (lp,'(/A/(3(A,I9)))') ' **** Error return from MA50B ****', &
!    ' Entry in row', i, ' and column', j, ' duplicated'
550 end subroutine ma50b

SUBROUTINE ma50c(m,n,icntl,iq,np,trans,lfact,fact,irnf,iptrl,iptru,b,x,w)
...
! if (m<1 .or. n<1) go to 250
  ...
  if (mp>0 .and. icntl(3)>2) write (mp,'(/2(A,I6),A,I4,A,L2)') &
    ' Entering MA50C with M=', m, ' N =', n, ' NP =', np, ' TRANS =', trans
  if1 = iptrl(n) + 1
  mf = irnf(2)
  nf = n - np
...
120 continue
    if (mp>0 .and. icntl(3)>3) write (mp,'(A/(4X,5F10.4))') &
      ' Leaving MA50C with X =', (x(i),i=1,m)
...
240   continue
    ...
    if (mp>0 .and. icntl(3)>3) write (mp,'(A/(4X,5F10.4))') &
      ' Leaving MA50C with X =', (x(i),i=1,n)
  end if
  return
!!!
! Error condition.
!250 info(1) = -1
!  if (lp>0) write (lp,'(/A/2(A,I8))') ' **** Error return from MA50C ****', &
!    ' M =', m, ' N =', n
end subroutine ma50c

SUBROUTINE ma50d(a,ind,iptr,n,disp,reals)
...
!        Update elements in column J.
    do 10 i = 1, j - 1
      if (m>i) call saxpy(m-i,-a(i,j),a(i+1,i),1,a(i+1,j),1)
10  continue
...
!           Apply row interchange to columns 1:N+J-K.
      if (jp/=j) call sswap(n+j-k,a(j,1),lda,a(jp,1),lda)
...
!           Apply column interchange and record it.
      if (k<n) call sswap(m,a(1,j),1,a(1,n-k+j),1)
      ipiv(n-k+j) = -j
...
!           Update diagonal and subdiagonal elements in column J.
      call sgemv('No transpose',m-j+1,j-1,-one,a(j,1),lda,a(1,j),1,one,a(j,j), &
        1)
!          Find pivot.
      jp = j - 1 + isamax(m-j+1,a(j,j),1)
      ipiv(j) = jp
...
!             Compute block row of U.
        call sgemv('Transpose',j-1,n-j,-one,a(1,j+1),lda,a(j,1),lda,one, &
          a(j,j+1),lda)
      end if
...
!          Update diagonal and subdiagonal elements in column J.
      call sgemv('No transpose',m-j+1,j-j1,-one,a(j,j1),lda,a(j1,j),1,one, &
        a(j,j),1)
...
!           Apply row interchange to columns J1:J2
      if (jp/=j) call sswap(j2-j1+1,a(j,j1),lda,a(jp,j1),lda)
...
!             Compute row of U within current block
        call sgemv('Transpose',j-j1,j2-j,-one,a(j1,j+1),lda,a(j,j1),lda,one, &
          a(j,j+1),lda)
      end if
...
!              Apply operations to new column.
        do 20 i = j1, j - 1
          ...
          a(jp,j) = temp
20      continue
        if (j>j1) call strsv('Lower','No transpose','Unit',j-j1,a(j1,j1),lda, &
          a(j1,j),1)
      else
        j2 = j2 - 1
      end if
...
!              Update the Schur complement
        call strsm('Left','Lower','No transpose','Unit',j2-j1+1,n-k,one, &
          a(j1,j1),lda,a(j1,j2+1),lda)
        if (m>j2) call sgemm('No transpose','No transpose',m-j2,n-k,j2-j1+1, &
...
!  B       (input/output) REAL array, size max(M,N)
...
!        Solve L*X = B, overwriting B with X.
    if (icntl5>0) then
      if (rank>0) call strsv('L','NoTrans','Unit',rank,a,lda,b,1)
    else
      do 40 k = 1, rank - 1
        if (b(k)/=zero) call saxpy(rank-k,-b(k),a(k+1,k),1,b(k+1),1)
40    continue
    end if

!        Solve U*X = B, overwriting B with X.
    if (icntl5>0) then
      if (rank>0) call strsv('U','NoTrans','NonUnit',rank,a,lda,b,1)
    else
      do 50 k = rank, 2, -1
        if (b(k)/=zero) then
          b(k) = b(k)/a(k,k)
...
!        Solve U'*X = B, overwriting B with X.
    ...
    if (icntl5>0) then
      if (rank>0) call strsv('U','Trans','NonUnit',rank,a,lda,b,1)
    else
      if (rank>0) b(1) = b(1)/a(1,1)
      do 90 i = 2, rank
...
!        Solve L'*X = B, overwriting B with X.
    if (icntl5>0) then
      if (rank>0) call strsv('L','Trans','Unit',rank,a,lda,b,1)
    else
      do 100 i = rank - 1, 1, -1
...
end subroutine ma50i

end module hsl_ma48_ma50_internal_single

module hsl_ma48_ma48_internal_single
   use hsl_ma48_ma50_internal_single
   implicit none
...
!      space allocated.
   ...
   private
   public :: ma48a, ma48b, ma48c
   public :: ma48i ! public for unit tests
...
contains
    ...
    SUBROUTINE ma48a(m,n,ne,job,la,a,irn,jcn,keep,cntl,icntl,info,rinfo, &
                      endcol)
...
!       LASTC is also used to get workspace in KEEP for MA50A.
...
!       LENC is also used to get workspace in KEEP for MA50A.
...
50    IF (mp>0 .AND. icntl(3)>2) THEN
        WRITE (mp,'(/A/A,I7,A,I6,A,I7,A,I2,A,I7/A,1P,4E12.4/A,4I8/A,3I8)') &
...
! Repartition IW (none of previous data is needed any more)
      ...
      nextc = 4*m + n
      ptro = nextc
! Use scratch space in KEEP for work arrays in MA50A.
...
!         CALL ma50a(nr,nc,nzb,la-ne-k,a(ne+k+1),irn(ne+k+1),jcn(ne+1), &
          CALL ma50a(nr,nc,nzb,la,fact,ifact,ljfact,jfact, &
...
! Action if failure in allocate in ma50a
...
! LENCOL(KB) holds the actual storage forecast for MA50B entry.
          kb = kb + 1
...
9160  FORMAT (' Message from MA48A/AD ..',I8,' out-of-range indices fo','und')
    END subroutine ma48a


    SUBROUTINE ma48b(m,n,ne,job,la,a,irn,jcn,keep,cntl,icntl,info,rinfo)
...
! Partition KEEP and A
      ...
      IF (mp>0 .AND. icntl(3)>2) THEN
        WRITE (mp,'(/A/3(A,I8),A,I2,A,I8/A,1P,3E12.4/A,3I8/A,I8/A,I8)') &
...
120       CONTINUE
          np = keep(mblock+3*jb)
!!!!!!!!!!!!!!!!!!! MA50B !!!!!!!!!!!!!!!!!!!!!!!
          CALL ma50b(nr,nc,nzb,job5,a(k+1),irn(k+1),keep(iptrd+j1),cntl5, &
...
150   CONTINUE
      ...
      IF (mp>0 .AND. icntl(3)>2) THEN
        ...
        DO 170 jb = 1, kb
          ...
          DO 160 j = j1, j3
            WRITE (mp,'(A,I5,(T13,3(1PE12.4,I5)))') ' Column', j, &
...
9190  FORMAT (' Error return from MA48B/BD because ','JOB = ',I10)
    END  subroutine ma48b


    SUBROUTINE ma48c(m,n,trans,job,la,a,irn,keep,cntl,icntl,rhs,x,error, &
        info)
...
! Solve system using MA48D/DD or MA50C/CD.
        IF (nb==1 .AND. keep(mblock+3)>=0) THEN
          ...
          CALL ma50c(m,n,icntl5,iqb,iblock,trans,la-ne,a(ne+1), &
            irn(ne+1),keep(iptrl+1),keep(iptru+1),w,x,w(neq+1))
        ELSE
          CALL ma48d(n,ne,la-ne,a(ne+1),a,irn(ne+1),irn,keep(iptrd+1), &
...
!            i.  infinity      max     i
        ...
        omega(2) = zero
        DO 230 i = 1, neq
          tau = (w(2*neq+i)*dxmax+abs(rhs(i)))*real(nvar)*ctau
...
! Solve system using MA48D/DD or MA50C/CD.
              IF (nb==1 .AND. keep(mblock+3)>=0) THEN
          iblock = keep(mblock+3)
                CALL ma50c(m,n,icntl5,iqb,iblock, .NOT. trans,la-ne, &
...
9170  FORMAT (' Error return from MA48C/CD because of ','nonconvergenc', &
        ...
        '(9) = ',I10)
    END subroutine ma48c


    SUBROUTINE ma48d(n,ne,la,a,aa,irn,irna,iptrd,iptro,nb,iblock,iptrl,iptru, &
...
!!!
!     integer(long) iflag(15)
! I     row index
! IFLAG error flag from MA50C .. will never be set
! IQB   is passed to MA50C/CD to indicate that no column permutation
!       is used.
! J     column index
! JB    block index.
...
20          CONTINUE
          ELSE
            ...
            np = iblock(2,jb)
            CALL ma50c(nc,nc,icntl5,iqb,np,trans,la+1-j1,a(j1), &
...
! PUT PERMUTATION IN THE REQUIRED FORM.
  ...
  140 CONTINUE
      ...
      END subroutine
end module hsl_ma48_ma48_internal_single

module hsl_ma48_ma51_internal_single
   implicit none
...
! This module is based on ma51 1.0.0 (12 July 2004)
   ...
   private
   public :: ma51a, ma51c
   public :: ma51b, ma51d ! public for unit tests
...
contains
    ...
    SUBROUTINE ma51a(m,n,la,irn,keep,rank,rows,cols,w)
...
10        CONTINUE
        ...
        ELSE
          ...
          CALL ma51z(nr,nc,iqb,np,la+1-ne-j1,irn(ne+j1),keep(iptrl+k1), &
...
60    CONTINUE
    ...
    END subroutine ma51a

    SUBROUTINE ma51b(m,n,iq,np,lfact,irnf,iptrl,iptru,rank,rows,cols,w)
...
! K2 Position of next row or column of the insignificant part.
      ...
      CALL ma51z(m,n,iq,np,lfact,irnf,iptrl,iptru,rank,rows,cols,w)
...
30    CONTINUE
      ...
      DO 40 i = 1, n
        ...
        END IF
40    CONTINUE
    END subroutine ma51b

    SUBROUTINE ma51c(m,n,la,a,irn,keep,sgndet,logdet,w)
!     This is for use in conjunction with the MA48 routines
...
! LOGDET is a real output variable that returns the
...
70        CONTINUE
        ELSE
          j1 = 1
          IF (jb>1) j1 = keep(kblock+3*jb)
          CALL ma51d(nr,nc,iqb,np,la+1-j1-ne,a(ne+j1),irn(ne+j1), &
...
80    CONTINUE
    ...
    END subroutine ma51c

    SUBROUTINE ma51d(m,n,iq,np,lfact,fact,irnf,iptrl,iptru,sgndet,logdet,w)
!  Computes the determinant of a factorized n-by-n matrix.
      INTEGER k, l, m, n, iq(*), np
...
! Check for rank-deficient case
        IF (m/=n .OR. mf/=nf .OR. irnf(if1+mf+nf-1)<0) THEN
          ...
          RETURN
        END IF
        CALL ma51x(mf,fact(if1),mf,irnf(if1+mf),sgndet,logdet)
      END IF
...
60    CONTINUE
    ...
    END subroutine ma51d



    SUBROUTINE ma51x(n,a,lda,ipiv,sgndet,logdet)
      IMPLICIT NONE
      INTEGER n
      integer :: lda
      real(wp) a(lda,n)
...
30    CONTINUE
    ...
    END subroutine ma51x



    SUBROUTINE ma51y(m,n,ipiv,rank,rows,cols)
...
60    CONTINUE
    ...
    END subroutine ma51y


    SUBROUTINE ma51z(m,n,iq,np,lfact,irnf,iptrl,iptru,rank,rows,cols,w)
...
! NF Number of columns held in full format.
      ...
      IF (mf>0 .AND. nf>0) THEN
        CALL ma51y(mf,nf,irnf(if1+mf),rank,w,cols(np+1))
      ELSE
        rank = 0
        DO 10 i = 1, mf
          w(i) = 0
...
240     CONTINUE
    ...
    END subroutine ma51z

end module hsl_ma48_ma51_internal_single


module hsl_ma48_single
   use hsl_zd11_single
   use hsl_ma48_ma48_internal_single
   use hsl_ma48_ma51_internal_single
   implicit none
...
contains
   ...
   SUBROUTINE ma48_initialize_single(factors,control)
      ...
      if (present(control)) then
          control%switch = 0.5e0
          control%u      = 0.01e0
          control%drop   = 0.0e0
          control%tolerance = 0.0e0
          control%cgce      = 0.5e0
          control%lp = 6
          control%wp = 6
          control%mp = 6
...
!!!!!!!!!!!!!!!  Call to MA48A !!!!!!!!!!!!!!!!!!!!!!
      if (present(endcol)) then
        call ma48a(m,n,ne,job,la,factors%val,factors%irn,factors%jcn, &
...
! Storage must be increased if ma48b fails so effective value of
...
! Copy irn
      ...
      cntl(3) = control%drop
      cntl(4) = control%tolerance
! Switch off error message printing from ma48b
      icntl(1) = -1
      icntl(2) = -1
      if (control%ldiag.gt.2) icntl(2) = control%mp
      icntl(3) = control%ldiag
...
!!!!!!!!!!!!!!!  Call to MA48B !!!!!!!!!!!!!!!!!!!!!!
      call ma48b(factors%m,factors%n,ne,job,la,factors%val, &
...
! Check whether this has been preceded by a call to ma48_factorize
      ...
      if(matrix%ne .eq. 0) then
        x = 0.0e0
        if (present(resid)) resid = 0.0e0
        if (present(error)) error = 0.0e0
        return
      endif
...
!!! Problem with kind = long is size statement
      ...
      call ma48c(factors%m,factors%n,trans48,job,  &
                  lasolve,factors%val, &
...
!!!
! Only invoked if error return -8 or -9 from ma48c
...
!     Nonzer should be called after MA48B ..
...
!     la = size(factors%val,kind=long)
      la = size(factors%val)*lone
      call ma51a(factors%m,factors%n,la,factors%irn,factors%keep,rank, &
                  rows,cols,iwork)
...
!     la = size(factors%val,kind=long)
      la = size(factors%val)*lone
      call ma51c(factors%m,factors%n,la,factors%val,factors%irn, &
