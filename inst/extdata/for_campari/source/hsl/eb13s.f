...
C
      ...
      END
C**************************************************************
      SUBROUTINE EB13A(IND,N,NUMEIG,NBLOCK,NSTEPS,ER,EI,LN,X,U,W,IKEEP,
...
C     ..
C     .. Array Arguments ..
      REAL CNTL(1),EI(NSTEPS*NBLOCK),ER(NSTEPS*NBLOCK),
     +     RKEEP(NSTEPS*NBLOCK* (10+2*NSTEPS*NBLOCK)+13),RINFO(5),
     +     U(LN,NSTEPS*NBLOCK),W(LN,NSTEPS*NBLOCK),X(LN,NSTEPS*NBLOCK)
...
C Optional printing of input parameters.
     ...
     +        NSTEPS,'  LN =',LN
            WRITE (MP,FMT='(/A,2I3,2I4,I7,5I4,I5/A,1PE12.4)')
...
C
      ...
      NLOW = IKEEP(2)
      NUP = IKEEP(3)
      CALL EB13C(IPOS,IND,N,NUMEIG,NBLOCK,NUMCOL,X,U,W,LN,NLOW,NUP,
...
C     ..
C     .. Array Arguments ..
      REAL EI(NSTEPS*NBLOCK),ER(NSTEPS*NBLOCK),
     +     RKEEP(12+ (NSTEPS*NBLOCK)**2),RES(NEV),U(LN,NSTEPS*NBLOCK),
     +     W(LN,NSTEPS*NBLOCK),X(LN,NSTEPS*NBLOCK),Y(LN,NEV)
      INTEGER IKEEP(4)
C     ..
C     .. Local Scalars ..
      REAL ANORM
...
C     ..
C     .. External Subroutines ..
      EXTERNAL EB13Q,EB13R
C     ..
      IKEEP(4) = 0
      NUMCOL = NSTEPS*NBLOCK
      IF (IKEEP(1).EQ.0) THEN
         CALL EB13R(N,NUMCOL,NEV,ER,EI,Y,X,W,LN,RKEEP(13))
         IKEEP(1) = 1
         IKEEP(2) = 1
         IKEEP(3) = NEV
      ELSE IF (IKEEP(1).EQ.1) THEN
         ANORM = RKEEP(1)
         CALL EB13Q(N,NEV,LN,Y,U,RES,ER,EI,IKEEP(4),ANORM)
         IKEEP(1) = 0
      END IF
C
      RETURN
      END
C*************************************************************
      SUBROUTINE EB13C(IPOS,IND,N,NUMEIG,NBLOCK,NUMCOL,X,U,W,LN,NLOW,
...
C
C     .. Parameters ..
      REAL HALF,ONE,ZERO,TWO,TOL
      PARAMETER (HALF=0.5,ONE=1.0,ZERO=0.0,TWO=2.0,TOL=0.01)
C     ..
C     .. Scalar Arguments ..
      INTEGER IND,IPOS,LN,N,NBLOCK,NLOW,NUMCOL,NUMEIG,NUP
C     ..
C     .. Array Arguments ..
      REAL CH(2,2*NUMCOL),EI(NUMCOL),EIOLD(NUMCOL),ER(NUMCOL),
     +     EROLD(NUMCOL),EV(2,2*NUMCOL),KEEP(12),QTAQ(NUMCOL,NUMCOL),
     +     WORK(NUMCOL,NUMCOL),RINFO(5),U(LN,NUMCOL),W(LN,NUMCOL),
     +     X(LN,NUMCOL),CNTL(1)
      INTEGER ICNTL(11),IKEEP(11),INFO(6),ITYPE(NUMCOL)
C     ..
C     .. Local Scalars ..
      REAL A1,A2,C2,D,RATIO,T,T1,T2,T3,WRT1,XI,XR,NETA
      INTEGER I,I1,IEV,IFOUND,IPNTR,J,J1,J2,JFOUND,JJ,JPNTR,JPOWER,K,
     +        ICNTL9,KSTOP,MHULL,MP,NEV,ITS
C     ..
C
C     .. External Functions ..
      REAL SDOT,SNRM2,FA14A
      EXTERNAL SDOT,SNRM2,FA14A
C     ..
C     .. External Subroutines ..
      EXTERNAL SAXPY,SCOPY,SGEMM,SGEMV,SSCAL,EB13D,EB13E,EB13F,
     +         EB13G,EB13H,EB13J,EB14A,EB14B,EB14C,EB14D
C     ..
C     .. Intrinsic Functions ..
      INTRINSIC ABS,INT,LOG10,MAX,MIN,MOD,REAL,SQRT
C     ..
   10 CONTINUE
      IFOUND = INFO(4)
...
C Check that the norm of A (in KEEP(1)) is large enough
         IF (ICNTL(7).NE.2 .AND. KEEP(1).LT.100.0*KEEP(7)) THEN
            INFO(1) = -6
            GO TO 290
         END IF
         IKEEP(11) = 0
...
C Generate a random array X()
            DO 50 J = 1,NBLOCK
               DO 40 I = 1,N
                  X(I,J) = FA14A(IKEEP(9),-1)
   40          CONTINUE
   50       CONTINUE
         END IF
         IF (NBLOCK.GT.1) THEN
            CALL EB13J(X,LN,N,NBLOCK,0,U)
            CALL EB13E(N,X,LN,W,LN,NBLOCK)
         ELSE
            T = SNRM2(N,X,1)
            CALL SSCAL(N,ONE/T,X,1)
...
C for block Hessenberg form
   ...
   70       CONTINUE
         ...
         J = J1 - NBLOCK
         IF (NBLOCK.GT.1) THEN
            CALL EB13E(N,U(1,J),LN,X(1,J1),LN,NBLOCK)
            NETA = ONE/ (SQRT(TWO))

            DO 80 I = 1,NBLOCK
               W(I,1) = SNRM2(N,X(1,J1+I-1),1)
   80       CONTINUE
            ITS = 0

   90       CALL SCOPY(NBLOCK,W(1,1),1,W(1,2),1)
            ITS = ITS + 1

            CALL SGEMM('T','N',IPNTR,NBLOCK,N,ONE,X,LN,X(1,J1),LN,ZERO,
     +                 U(1,J),LN)
            CALL SGEMM('N','N',N,NBLOCK,IPNTR,-ONE,X,LN,U(1,J),LN,ONE,
     +                 X(1,J1),LN)
            DO 100 I = 1,NBLOCK
               IF (ITS.EQ.1) CALL SCOPY(IPNTR,U(1,J+I-1),1,
     +                                  QTAQ(1,J+I-1),1)
               IF (ITS.GT.1) CALL SAXPY(IPNTR,ONE,U(1,J+I-1),1,
     +                                  QTAQ(1,J+I-1),1)
  100       CONTINUE
...
c            CALL SGEMM('T','N',IPNTR,NBLOCK,N,ONE,X,LN,X(1,J1),LN,
c     +                 ZERO,QTAQ(1,J),NUMCOL)
c            CALL SGEMM('N','N',N,NBLOCK,IPNTR,-ONE,X,LN,QTAQ(1,J),
c     +                 NUMCOL,ONE,X(1,J1),LN)
c            CALL SGEMM('T','N',IPNTR,NBLOCK,N,ONE,X,LN,X(1,J1),LN,
c     +                 ZERO,U(1,J),LN)
c            CALL SGEMM('N','N',N,NBLOCK,IPNTR,-ONE,X,LN,U(1,J),LN,
c     +                 ONE,X(1,J1),LN)
c            DO 80 I = 1,NBLOCK
c               CALL SAXPY(IPNTR,ONE,U(1,J+I-1),1,QTAQ(1,J+I-1),1)
c   80       CONTINUE
            DO 140 I = 1,NBLOCK
               T = SNRM2(N,X(1,J1+I-1),1)
               IF (T.LT.KEEP(7)) THEN
     ...
     +                INFO(1).NE.3) WRITE (MP,FMT=9000)
               ...
               END IF
               QTAQ(J1+I-1,J+I-1) = T
               CALL SSCAL(N,ONE/T,X(1,J1+I-1),1)
               DO 130 I1 = I + 1,NBLOCK
                  T = SDOT(N,X(1,J1+I1-1),1,X(1,J1+I-1),1)
                  QTAQ(J1+I-1,J+I1-1) = T
                  CALL SAXPY(N,-T,X(1,J1+I-1),1,X(1,J1+I1-1),1)
  130          CONTINUE
  140       CONTINUE
            CALL EB13E(N,X(1,J1),LN,W(1,J1),LN,NBLOCK)
            NLOW = J1
            NUP = J1 + NBLOCK - 1
            INFO(2) = INFO(2) + NBLOCK
            IKEEP(11) = IPNTR
            IF (IPNTR+NBLOCK.EQ.NUMCOL) THEN
               IPOS = 6
               IKEEP(11) = 0
               CALL EB13J(X,LN,N,NUMCOL,JFOUND*NBLOCK,U)
            END IF
            GO TO 290

         ELSE

            CALL SCOPY(N,U(1,J),1,X(1,J1),1)
            NETA = ONE/ (SQRT(TWO))
            T = SNRM2(N,X(1,J1),1)
            ITS = 0
  150       T1 = T
...
C Use classical Gram Schmidt
            CALL SGEMV('T',N,J,ONE,X,LN,X(1,J1),1,ZERO,U(1,J),1)
            CALL SGEMV('N',N,J,-ONE,X,LN,U(1,J),1,ONE,X(1,J1),1)
            IF (ITS.EQ.1) CALL SCOPY(J,U(1,J),1,QTAQ(1,J),1)
            IF (ITS.GT.1) CALL SAXPY(J,ONE,U(1,J),1,QTAQ(1,J),1)
            T = SNRM2(N,X(1,J1),1)
C Test to see if we need to iterate GS process
     ...
     +             INFO(1).NE.3) WRITE (MP,FMT=9000)
            ...
            CALL SSCAL(N,ONE/T,X(1,J1),1)

C Return for matrix-vector product
            CALL SCOPY(N,X(1,J1),1,W(1,J1),1)
            NLOW = J1
            NUP = J1
            INFO(2) = INFO(2) + 1
            IKEEP(11) = IPNTR
...
C Arnoldi iteration with Chebychev acceleration
            IF (IKEEP(11).EQ.0) THEN
               IF (NBLOCK.GT.1) CALL EB13E(N,X(1,IFOUND+1),LN,
     +                               W(1,IFOUND+1),LN,J1)
               IF (NBLOCK.EQ.1) CALL SCOPY(N,X(1,IFOUND+1),1,
     +                                     W(1,IFOUND+1),1)
               KEEP(9) = TWO/KEEP(8)
               KEEP(10) = ZERO
...
C Recurrence relation
            CALL EB13H(N,T1,T2,T3,U(1,IFOUND+1),LN,W(1,IFOUND+1),LN,
     +                  X(1,IFOUND+1),LN,J1)
            IKEEP(11) = IKEEP(11) + 1
            IF (IKEEP(11).LT.INFO(3)) THEN
               IF (NBLOCK.GT.1) THEN
                  CALL EB13E(N,W(1,IFOUND+1),LN,X(1,IFOUND+1),LN,J1)
                  CALL EB13E(N,U(1,IFOUND+1),LN,W(1,IFOUND+1),LN,J1)
               ELSE
                  CALL SCOPY(N,W(1,IFOUND+1),1,X(1,IFOUND+1),1)
                  CALL SCOPY(N,U(1,IFOUND+1),1,W(1,IFOUND+1),1)
               END IF
               NLOW = IFOUND + 1
               NUP = (JFOUND+1)*NBLOCK
            ...
            ELSE
               IF (NBLOCK.GT.1) THEN
                  CALL EB13E(N,U(1,IFOUND+1),LN,X(1,IFOUND+1),LN,J1)
                  CALL EB13J(X,LN,N, (JFOUND+1)*NBLOCK,IFOUND,U)
                  CALL EB13J(X,LN,N, (JFOUND+1)*NBLOCK,IFOUND,U)
                  CALL EB13E(N,X(1,JFOUND*NBLOCK+1),LN,
     +                        W(1,JFOUND*NBLOCK+1),LN,NBLOCK)
               ELSE
                  CALL SCOPY(N,U(1,IFOUND+1),1,X(1,IFOUND+1),1)
                  CALL EB13J(X,LN,N,IFOUND+1,IFOUND,U)
                  CALL EB13J(X,LN,N,IFOUND+1,IFOUND,U)
                  CALL SCOPY(N,X(1,IFOUND+1),1,W(1,IFOUND+1),1)
               END IF
...
C  JJ = J if NBLOCK=1)
         IF (I.EQ.1) THEN
            IF (NBLOCK.GT.1) THEN
               CALL EB13E(N,X(1,J),LN,W(1,JJ+1),LN,NBLOCK)
               CALL EB13E(N,U(1,J),LN,U(1,JJ+1),LN,NBLOCK)
            ELSE
               CALL SCOPY(N,X(1,J),1,W(1,JJ+1),1)
               CALL SCOPY(N,U(1,J),1,U(1,JJ+1),1)
            END IF
            KEEP(9) = TWO/KEEP(8)
            KEEP(10) = ZERO
            T1 = TWO*KEEP(9)/KEEP(12)
...
C
C Recurrence relation
         CALL EB13H(N,T1,T2,T3,U(1,JJ+1),LN,W(1,JJ+1),LN,X(1,JJ+1),LN,
     +               NBLOCK)
C
         IPNTR = IPNTR + 1
         IF (I.LT.INFO(3)) THEN
            IF (NBLOCK.GT.1) THEN
               CALL EB13E(N,W(1,JJ+1),LN,X(1,JJ+1),LN,NBLOCK)
               CALL EB13E(N,U(1,JJ+1),LN,W(1,JJ+1),LN,NBLOCK)
            ELSE
               CALL SCOPY(N,W(1,JJ+1),1,X(1,JJ+1),1)
               CALL SCOPY(N,U(1,JJ+1),1,W(1,JJ+1),1)
            END IF
            NLOW = JJ + 1
            NUP = JJ + NBLOCK
            INFO(2) = INFO(2) + NBLOCK
         ...
         ELSE
            IF (NBLOCK.GT.1) THEN
               NETA = ONE/ (SQRT(TWO))
               CALL EB13E(N,U(1,JJ+1),LN,X(1,JJ+1),LN,NBLOCK)
               CALL EB13J(X,LN,N,JJ+NBLOCK,JJ,U)

               DO 160 I = 1,NBLOCK
                  W(I,1) = SNRM2(N,X(1,JJ+I-1),1)
  160          CONTINUE
               ITS = 0

  170          CALL SCOPY(NBLOCK,W(1,1),1,W(1,2),1)
               ITS = ITS + 1

               CALL SGEMM('T','N',JJ,NBLOCK,N,ONE,X,LN,X(1,JJ+1),LN,
     +                    ZERO,U,LN)
               CALL SGEMM('N','N',N,NBLOCK,JJ,-ONE,X,LN,U,LN,ONE,
     +                    X(1,JJ+1),LN)
C Test to see if we need to iterate GS process
               DO 180 I = 1,NBLOCK
                  W(I,1) = SNRM2(N,X(1,JJ+I),1)
  180          CONTINUE
               DO 190 I = 1,NBLOCK
                  IF (W(I,1).LT.NETA*W(I,2) .AND. ITS.LE.3) THEN
                     GO TO 170
...
C QR factorisation.
     ...
     +                   INFO(1).NE.3) WRITE (MP,FMT=9000)
                     ...
                     T = KEEP(7)
                  END IF
                  CALL SSCAL(N,ONE/T,X(1,JJ+I),1)
                  DO 200 I1 = I + 1,NBLOCK
                     T = SDOT(N,X(1,JJ+I1),1,X(1,JJ+I),1)
                     CALL SAXPY(N,-T,X(1,JJ+I),1,X(1,JJ+I1),1)
  200             CONTINUE
  210          CONTINUE
               CALL EB13E(N,X(1,JJ+1),LN,W(1,JJ+1),LN,NBLOCK)

            ELSE
               NETA = ONE/ (SQRT(TWO))
               CALL SCOPY(N,U(1,JJ+1),1,X(1,JJ+1),1)
               CALL EB13J(X,LN,N,JJ+1,JJ,U)
               CALL SSCAL(N,ZERO,W,1)
               T = SNRM2(N,X(1,JJ+1),1)
               ITS = 0

  220          T1 = T
               ITS = ITS + 1
               CALL SGEMV('T',N,JJ,ONE,X,LN,X(1,JJ+1),1,ZERO,U,1)
               CALL SGEMV('N',N,JJ,-ONE,X,LN,U,1,ONE,X(1,JJ+1),1)
               T = SNRM2(N,X(1,JJ+1),1)
     ...
     +                INFO(1).NE.3) WRITE (MP,FMT=9000)
               ...
               CALL SSCAL(N,ONE/T,X(1,JJ+1),1)
               CALL SCOPY(N,X(1,JJ+1),1,W(1,JJ+1),1)
            ...
            ELSE
               J1 = JFOUND*NBLOCK
               CALL EB13J(X,LN,N,NUMCOL,J1,U)
               CALL EB13J(X,LN,N,NUMCOL,J1,U)
               CALL EB13E(N,X(1,J1+1),LN,W(1,J1+1),LN,NUMCOL-J1)
               NLOW = J1 + 1
               NUP = NUMCOL
...
C
         CALL SGEMM('T','N',NUMCOL,NUMCOL-J1,N,ONE,X,LN,U(1,J1+1),LN,
     +              ZERO,QTAQ(1,J1+1),NUMCOL)
C
C Convert B into upper Hessenberg form and
C perform a Ritz acceleration step.
C
         CALL SCOPY(NUMCOL,ER,1,EROLD,1)
         CALL SCOPY(NUMCOL,EI,1,EIOLD,1)
         CALL EB13F(2,IND,N,NUMCOL,J1,QTAQ,WORK,U,X,LN,KEEP(7),ER,EI,
     +               ITYPE)
C
...
C
         ...
         CALL EB13E(N,X(1,J1+1),LN,W(1,J1+1),LN,J2-J1)
         NLOW = J1 + 1
         NUP = J2
         INFO(2) = INFO(2) + J2 - J1
         IPOS = 7
...
C
         IF (NBLOCK.GT.1) THEN
            CALL SGEMM('T','N',NUMCOL,NBLOCK,N,ONE,X,LN,
     +                 U(1,NUMCOL-NBLOCK+1),LN,ZERO,
     +                 QTAQ(1,NUMCOL-NBLOCK+1),NUMCOL)
            CALL EB13F(2,IND,N,NUMCOL,J1,QTAQ,WORK,U,X,LN,KEEP(7),ER,
     +                  EI,ITYPE)
         ELSE
            CALL SGEMV('T',N,NUMCOL,ONE,X,LN,U(1,NUMCOL),1,ZERO,
     +                 QTAQ(1,NUMCOL),1)
C QTAQ is already in Hessenberg form
            CALL EB13F(1,IND,N,NUMCOL,J1,QTAQ,WORK,U,X,LN,KEEP(7),ER,
     +                  EI,ITYPE)
         END IF
C
C Schur vectors held in X.
...
C We need to keep U = A*X for the next iteration.
         CALL EB13E(N,U(1,J1+1),LN,W(1,J1+1),LN,J2-J1)
C Test for convergence.
         CALL EB13D(N,NUMCOL,NBLOCK,NEV,ITYPE,QTAQ,W,X,LN,ER,EI,EROLD,
...
C Braconnier ellipse
               CALL EB14C(MHULL,XR,CH,D,C2,A2,A1)
            ELSE IF (ICNTL(8).EQ.3) THEN
C Saad ellipse
               CALL EB14D(MHULL,NUMEIG,EV,CH,ER,EI,W,D,C2,A2)
            ELSE
C Ho ellipse
               CALL EB14B(MHULL,XR,XI,CH,D,C2,A2,A1)
            END IF
            RINFO(1) = D
            RINFO(2) = C2
            RINFO(3) = A2
            CALL EB13G(NUMEIG,ER,EI,W,RATIO,WRT1,D,C2,A2)
...
C
            ...
            JPOWER = MAX(JPOWER,1)
            INFO(3) = INT(REAL(INFO(3))* (ONE+LOG10(REAL(INFO(5)))))
...
C
 9000 FORMAT (/'  Warning from EB13A/AD. INFO(1) = 3',/'  Arnoldi ',
     ...
     +       'returned.')
      END
C*****************************************************************
      SUBROUTINE EB13D(N,NUMCOL,NBLOCK,NUMEIG,ITYPE,T,AQ,Q,LN,ER,EI,
...
C     ..
C     .. Array Arguments ..
      REAL AQ(LN,NUMCOL),EI(NUMCOL),EIOLD(NUMCOL),ER(NUMCOL),
     +     EROLD(NUMCOL),Q(LN,NUMCOL),R(5),RINFO(5),T(NUMCOL,NUMCOL),
     +     WORK(NUMCOL),CNTL(1)
      INTEGER ICNTL(11),INFO(6),ITYPE(NUMCOL)
C     ..
C     .. Local Scalars ..
      REAL ANORM,EPS,R1,R2,RMACH
      INTEGER I,IFOUND,IPOWER,J,JFOUND,KFOUND,MP
C     ..
C     .. External Functions ..
      REAL SNRM2,FD15A
      EXTERNAL SNRM2,FD15A
C     ..
C     .. External Subroutines ..
      EXTERNAL SGEMV
C     ..
...
C denominator.
            WORK(J+1) = SNRM2(N,AQ(1,J+1),1)
            WORK(J+2) = SNRM2(N,AQ(1,J+2),1)
         END IF
         CALL SGEMV('N',N,J+2,ONE,Q,LN,T(1,J+1),1,-ONE,AQ(1,J+1),1)
         R1 = SNRM2(N,AQ(1,J+1),1)
         CALL SGEMV('N',N,J+2,ONE,Q,LN,T(1,J+2),1,-ONE,AQ(1,J+2),1)
         R2 = SNRM2(N,AQ(1,J+2),1)
...
C denominator.
            WORK(J+1) = SNRM2(N,AQ(1,J+1),1)
         END IF
         CALL SGEMV('N',N,J+1,ONE,Q,LN,T(1,J+1),1,-ONE,AQ(1,J+1),1)
         R1 = SNRM2(N,AQ(1,J+1),1)
         IF (ICNTL(7).NE.2) THEN
            R(1) = R1/ANORM
...
C denominator.
     ...
     +            R(3).LT.R(4) .AND. R(4).GT.R(5)) THEN
         ...
         ELSE IF (R(1).LE.EPS*10**2 .AND. R(1).LT.R(2)) THEN
            IPOWER = MIN(IPOWER,INT(40.0* (ONE+LOG10(R(1)/EPS))))
         END IF
...
C Optional printing at end of iteration.
      IF (MP.GT.0 .AND. ICNTL(6).GE.3) THEN
         IF (IFOUND.GE.NUMEIG) THEN
            WRITE (MP,FMT='(/A/A,6I7/A,1P,5E12.4)')
     ...
     +        ' RINFO(1:5) = ', (RINFO(I),I=1,5)
         ...
         ELSE IF (ICNTL(6).GE.5) THEN
            WRITE (MP,FMT='(/A,I3,A/A,6I7/A,1P,5E12.4/A,1PE12.4)')
 ...
 9020 FORMAT (1P,2E12.4,4(' '),1P,2E12.4)
      END
C*****************************************************************
      SUBROUTINE EB13E(N,W,LDW,V,LDV,M)
C
...
C     ..
      DO 10 J = 1,M
         CALL SCOPY(N,W(1,J),1,V(1,J),1)
   10 CONTINUE
      RETURN
      END
C*****************************************************************
      SUBROUTINE EB13F(JND,IND,N,NUMCOL,IFOUND,QTAQ,Z,QWORK,Q,LN,EPS,
     +                  ER,EI,ITYPE)
C
...
C     ..
C     .. Array Arguments ..
      REAL EI(NUMCOL),ER(NUMCOL),Q(LN,NUMCOL),QTAQ(NUMCOL,NUMCOL),
     +     QWORK(N,NUMCOL),Z(NUMCOL,NUMCOL)
      INTEGER ITYPE(NUMCOL)
C     ..
C     .. External Subroutines ..
      EXTERNAL SGEMM,EB13E,EB13K,EB13L
C     ..
C     .. Local Scalars ..
      INTEGER I,J
C     ..
...
C
         CALL EB13L(NUMCOL,IFOUND+1,QTAQ,QWORK(1,IFOUND+1),Z)
C
...
C
      END IF
C
      CALL EB13K(IND,QTAQ,Z,NUMCOL,IFOUND+1,NUMCOL,EPS,ER,EI,ITYPE,
     +            NUMCOL,NUMCOL)
C
C We now have T = V(T)*H*V. We now wish to form
C                   Q = Q+ * (Z*V)
C
      CALL EB13E(N,Q(1,IFOUND+1),LN,QWORK(1,IFOUND+1),N,NUMCOL-IFOUND)

C Q+    is stored in QWORK()  (N-by-NUMCOL)
C Z*V    is stored in Z()  (NUMCOL-by-NUMCOL)
C Use BLAS-3 routine SGEMM to form matrix product.

      CALL SGEMM('N','N',N,NUMCOL-IFOUND,NUMCOL,ONE,QWORK,N,
     +           Z(1,IFOUND+1),NUMCOL,ZERO,Q(1,IFOUND+1),LN)
      RETURN
      END
C*******************************************************************
      SUBROUTINE EB13G(NEV,ER,EI,VEC,RATIO,WRT1,D,C2,A2)
C
...
C     .. Parameters ..
      REAL ONE,ZERO,HALF
      PARAMETER (ONE=1.0,ZERO=0.0,HALF=0.5)
C     ..
C     .. Scalar Arguments ..
      REAL A2,C2,D,RATIO,WRT1
      INTEGER NEV
C     ..
C     .. Array Arguments ..
      REAL EI(NEV),ER(NEV),VEC(NEV)
C     ..
C     .. Local Scalars ..
      REAL A,EE,SMALL,T,X,Y
      INTEGER I
C     ..
C     .. External Functions ..
      REAL FD15A
...
C Order ratios (descending order).
      ...
      END
C*****************************************************************
      SUBROUTINE EB13H(N,T1,T2,T3,X,LX,Y,LY,Z,LZ,M)
C
...
C     +          J) = SIGN(EMIN,Z(I,J))
   ...
   40 CONTINUE
      ...
      END
C******************************************************************
      SUBROUTINE EB13J(A,ADIM,N,P,R,WORK)
C
...
C
         DO 10 J = R + 1,P
            CALL SGEMV('T',N,R,ONE,A(1,1),ADIM,A(1,J),1,ZERO,WORK,1)
            CALL SGEMV('N',N,R,-ONE,A(1,1),ADIM,WORK,1,ONE,A(1,J),1)
   10    CONTINUE
      END IF
      K = R + 1
      RHO = SNRM2(N,A(1,K),1)
...
C Normalize A(:,K) to get Q(:,K)
         CALL SSCAL(N,ONE/RHO,A(1,K),1)
C
         IF (K.LT.P) THEN
C Compute the K-th row of R, i.e., R(K,K+1:P)^T = A(:,K+1:P)^T Q(:,K)
            CALL SGEMV('T',N,P-K,ONE,A(1,K+1),ADIM,A(1,K),1,ZERO,
     +                 WORK(K+1),1)
C
C Rank-1 update: A(:,K+1:P) = A(:,K+1:P) - Q(:,K)R(K,K+1:P)
            CALL SGER(N,P-K,-ONE,A(1,K),1,WORK(K+1),1,A(1,K+1),ADIM)
            RHO = SNRM2(N,A(1,K+1),1)
         END IF
         K = K + 1
         GO TO 20
      END IF
C
      ...
      END
C****************************************************************
      SUBROUTINE EB13K(IND,A,V,N,NLOW,NUP,EPS,ER,EI,TYPE,NA,NV)
C
...
C     ..
C     .. Array Arguments ..
      REAL A(NA,N),EI(N),ER(N),V(NV,N)
      INTEGER TYPE(N)
C     ..
C     .. Local Scalars ..
      REAL E1,E2,P,Q,R,RMACH,S,T,W,X,Y,Z
      INTEGER I,IT,L,MU,NL,NU
      LOGICAL FAIL
C     ..
C     .. External Subroutines ..
      EXTERNAL EB13M,EB13N,EB13O
C     ..
C     .. External Functions ..
      REAL FD15A
      EXTERNAL FD15A
...
C Perform a QR step between NL and NU.
         CALL EB13N(A,V,P,Q,R,NL,NU,N,NA,NV)
         GO TO 30
      END IF
C 2x2 block found.
...
C Loop to position  2x2 block.
      NL = MU - 1
C Attempt  to EB13O the block into two real eigenvalues
      CALL EB13O(A,V,N,NL,E1,E2,NA,NV,RMACH)
C If the EB13O was successful, go and order the
C real eigenvalues.
...
C The next block is 2X2.
            IF (IND.EQ.2) THEN
               IF (ABS((A(MU-1,MU-1)-A(MU,MU))**2+4.0*A(MU,MU-1)*A(MU-1,
     +             MU)).GE.ABS((A(MU+1,MU+1)-A(MU+2,
     +             MU+2))**2+4.0*A(MU+1,MU+2)*A(MU+2,MU+1))) GO TO 120
     ...
     +               A(MU+1,MU+1)+A(MU+2,MU+2)) THEN
               GO TO 120
            END IF
            CALL EB13M(A,V,N,NL,2,2,EPS,FAIL,NA,NV,RMACH)
            IF (FAIL) GO TO 130
            MU = MU + 2
            GO TO 70
         END IF
...
C The next block is 1X1.
     ...
     +       A(MU+1,MU+1)**2) GO TO 120
      ELSE IF ((A(MU-1,MU-1)+A(MU,MU))/2.0.GE.A(MU+1,MU+1)) THEN
         GO TO 120
      END IF
      CALL EB13M(A,V,N,NL,2,1,EPS,FAIL,NA,NV,RMACH)
      IF (FAIL) GO TO 140
      MU = MU + 1
      GO TO 70
...
C The next block is 2X2.
     ...
     +                2.0) GO TO 110
               END IF
               CALL EB13M(A,V,N,MU,1,2,EPS,FAIL,NA,NV,RMACH)
               IF (FAIL) GO TO 150
               MU = MU + 2
               GO TO 100
            END IF
...
C The next block is 1X1.
         ...
         ELSE IF (A(MU,MU).GE.A(MU+1,MU+1)) THEN
            GO TO 110
         END IF
         CALL EB13M(A,V,N,MU,1,1,EPS,FAIL,NA,NV,RMACH)
         MU = MU + 1
         GO TO 100
      END IF
  110 MU = NL
...
C 2X2 block.
               CALL EB13O(A,V,N,NU-1,E1,E2,NA,NV,RMACH)
               IF (A(NU,NU-1).NE.ZERO) THEN
                  ER(NU) = E1
                  EI(NU-1) = E2
                  ER(NU-1) = ER(NU)
...
C Single root.
  ...
  190 IF (NU.GE.NLOW) GO TO 180
      ...
      END
C*********************************************************************
      SUBROUTINE EB13L(N,LOW,A,ORT,Z)
C
...
C
      ...
      END
C******************************************************************
      SUBROUTINE EB13M(A,V,N,L,B1,B2,EPS,FAIL,NA,NV,RMACH)
C
...
C Interchange 2X2 and B2XB2 blocks.
         ...
         Q = ONE
         R = ONE
         CALL EB13N(A,V,P,Q,R,L,M,N,NA,NV)
         IT = 0
   10    CONTINUE
         IT = IT + 1
         IF (IT.GT.30) THEN
...
C Interchange 1X1 and 2X2 blocks.
         ...
         Q = ONE
         R = ONE
         CALL EB13N(A,V,P,Q,R,L,L+2,N,NA,NV)
         IT = 0
   20    CONTINUE
         IT = IT + 1
         IF (IT.GT.30) THEN
...
C Interchange 1X1 and 1X1 blocks.
   ...
   50       CONTINUE
      ...
      END
C******************************************************************
      SUBROUTINE EB13N(A,V,P,Q,R,NL,NU,N,NA,NV)
C
...
C Accumulate the transformation in V.
  ...
  100 CONTINUE
      ...
      END
C*******************************************************************
      SUBROUTINE EB13O(A,V,N,L,E1,E2,NA,NV,RMACH)
C
...
C     ..
      ...
      END
C**************************************************************
      SUBROUTINE EB13Q(N,NUMEIG,LN,Y,AY,RES,ER,EI,JNFO,ANORM)
C
...
C     ..
C     .. Array Arguments ..
      REAL AY(LN,NUMEIG),EI(NUMEIG),ER(NUMEIG),RES(NUMEIG),Y(LN,NUMEIG)
C     ..
C     .. Local Scalars ..
      REAL RMACH,T,T1,TETA,XMU
      INTEGER J,J1,K
C     ..
C     .. External Functions ..
...
C Case of a complex eigenvalue
            ...
            XMU = ER(J)
            TETA = EI(J)
            T1 = SDOT(N,AY(1,J),1,AY(1,J),1) +
     +           SDOT(N,AY(1,J1),1,AY(1,J1),1)
            T1 = SQRT(T1)
            DO 30 K = 1,N
               AY(K,J) = AY(K,J) - XMU*Y(K,J) + TETA*Y(K,J1)
               AY(K,J1) = AY(K,J1) - XMU*Y(K,J1) - TETA*Y(K,J)
   30       CONTINUE
            T = SDOT(N,AY(1,J),1,AY(1,J),1) +
     +          SDOT(N,AY(1,J1),1,AY(1,J1),1)
            T = SQRT(T)
            IF (T1.LT.RMACH) THEN
               RES(J) = T
...
C Case of a complex eigenvalue
   ...
   60       CONTINUE
     ...
     +          SDOT(N,AY(1,J1),1,AY(1,J1),1)
      ...
      END
C*******************************************************************
      SUBROUTINE EB13R(N,NUMCOL,NUMEIG,ER,EI,Y,X,W,LN,TW)
C
C     This routine computes the (right) eigenvectors corresponding
C     to the eigenvalues computed by EB13A/AD
C
...
C     ..
C     .. Array Arguments ..
      REAL EI(NUMCOL),ER(NUMCOL),TW(NUMCOL,NUMCOL),W(LN,NUMEIG),
     +     X(LN,NUMCOL),Y(LN,NUMEIG)
C     ..
C     .. Local Scalars ..
      REAL E,F,P,Q,R,RA,RMACH,S,SA,T1,U1,VI,VR,WI,WR,WW,X1,XN,Y1,Z
...
C     ..
C     .. External Subroutines ..
      EXTERNAL SGEMM,SSCAL,EB13E,EB13P
C     ..
C     .. Intrinsic Functions ..
      INTRINSIC ABS,SQRT
C     ..
...
C Complex vector associated with lambda = P - I * Q
            ...
            IF (ABS(TW(NE,NA)).GT.ABS(TW(NA,NE))) THEN
               ...
               TW(NA,NE) = -Q/TW(NE,NA)
            ELSE
               CALL EB13P(-TW(NA,NE),ZERO,TW(NA,NA)-P,Q,E,F)
               TW(NA,NA) = E
               TW(NA,NE) = F
            END IF
            TW(NE,NA) = ONE
            ...
            DO 30 II = 2,NA
               ...
               IF (WI.EQ.ZERO) THEN
                  U1 = TW(I,I) - P
                  RA = TW(I,NE) + SDOT(NA+1-M,TW(I,M),NUMCOL,TW(M,NA),1)
                  SA = SDOT(NA+1-M,TW(I,M),NUMCOL,TW(M,NE),1)
                  CALL EB13P(-RA,-SA,U1,Q,E,F)
                  TW(I,NA) = E
                  TW(I,NE) = F
                  M = I
               ELSE IF (WI.GT.ZERO) THEN
                  Z = TW(I+1,I+1) - P
                  U1 = TW(I,I) - P
                  R = TW(I+1,NE) + SDOT(NA+1-M,TW(I+1,M),NUMCOL,
     +                TW(M,NA),1)
                  S = SDOT(NA+1-M,TW(I+1,M),NUMCOL,TW(M,NE),1)
                  RA = TW(I,NE) + SDOT(NA+1-M,TW(I,M),NUMCOL,TW(M,NA),1)
                  SA = SDOT(NA+1-M,TW(I,M),NUMCOL,TW(M,NE),1)
                  X1 = TW(I,I+1)
                  Y1 = TW(I+1,I)
     ...
     +                (ABS(U1)+ABS(Q)+ABS(X1)+ABS(Y1)+ABS(Z))
                  CALL EB13P(X1*R-Z*RA+Q*SA,X1*S-Z*SA-Q*RA,VR,VI,E,F)
                  TW(I,NA) = E
                  TW(I,NE) = F
                  ...
                  ELSE
                     CALL EB13P(-R-Y1*TW(I,NA),-S-Y1*TW(I,NE),Z,Q,E,F)
                     TW(I+1,NA) = E
                     TW(I+1,NE) = F
                  END IF
                  M = I
...
C Real vector
            ...
            DO 40 II = 1,NA
               ...
               IF (WI.EQ.ZERO) THEN
                  WW = TW(I,I) - P
                  R = TW(I,NE) + SDOT(NA+1-M,TW(I,M),NUMCOL,TW(M,NE),1)
               ...
               ELSE IF (WI.GT.ZERO) THEN
                  U1 = TW(I,I) - P
                  Z = TW(I+1,I+1) - P
                  S = TW(I+1,NE) + SDOT(NA+1-M,TW(I+1,M),NUMCOL,
     +                TW(M,NE),1)
                  R = TW(I,NE) + SDOT(NA+1-M,TW(I,M),NUMCOL,TW(M,NE),1)
                  Y1 = TW(I+1,I)
                  X1 = TW(I,I+1)
...
C full matrix
      CALL SGEMM('N','N',N,NUMEIG,NUMEIG,ONE,X,LN,TW,NUMCOL,ZERO,Y,LN)
C
C Normalise eigenvectors
   ...
   60 IF (I.LE.NUMEIG) THEN
         IF (EI(I).EQ.ZERO) THEN
            Z = SNRM2(N,Y(1,I),1)
            IF (Z.GT.RMACH) CALL SSCAL(N,ONE/Z,Y(1,I),1)
            I = I + 1
         ELSE
            Z = SDOT(N,Y(1,I),1,Y(1,I),1) +
     +          SDOT(N,Y(1,I+1),1,Y(1,I+1),1)
            Z = SQRT(Z)
            IF (Z.GT.RMACH) THEN
               CALL SSCAL(N,ONE/Z,Y(1,I),1)
               CALL SSCAL(N,ONE/Z,Y(1,I+1),1)
            END IF
            I = I + 2
         END IF
         GO TO 60
...
C compute A*Y.
      CALL EB13E(N,Y,LN,W,LN,NUMEIG)
C
      RETURN
      END
C January 2002: PA02 (from archive) included inline
      SUBROUTINE EB14A(N,M,Z,ZOUT)
C
...
C Determine second  point
      ...
      DO 20 K = 1,N
         IF (Z(1,K).GT.RMIN) THEN
            CALL EB14E(ZTMP,Z(1,I1),Z(1,K),CS)
            IF (CS.LE.CM) THEN
...
C Determine all other points.
   30 CM = 2.0
      ...
      DO 40 K = 1,N
         IF (Z(1,K).GT.RMIN) THEN
            CALL EB14E(Z(1,I1),Z(1,I2),Z(1,K),CS)
            IF (CS.LE.CM) THEN
...
C     ..
C     .. Local Scalars ..
      REAL A,A1S,AS,B,B1,B1S,B2,BS,C2S,DS,EPSIL,R,RS,S1,S2,T1,T2,TEST,
     +     TEST1,X1I,X1R,X2I,X2R,X3I,X3R,Y,Z
...
C Fit 3-point ellipse through (1R,X1I), (X2R,X2I), (X3R,X3I).
   20 CALL EB14J(X1R,X1I,X2R,X2I,X3R,X3I,XR,XI,D,A,B,A1,B1,R,C2,IER)
C
      IF (IER.NE.1) THEN
...
C and (X2R,X2I) relative to (XR,XI)
   40 IOPT1 = 0
      ...
      IOPT3 = 0
C      BS = B
      CALL EB14K(X1R,X1I,X2R,X2I,XR,XI,RS,AS,BS,A1S,B1S,DS,C2S)
      TEST1 = EB14I(X3R,X3I,DS,AS,BS)
C
...
C and (X3R,X3I) relative to (XR,XI)
         CALL EB14K(S1,T1,X3R,X3I,XR,XI,RS,AS,BS,A1S,B1S,DS,C2S)
         TEST1 = EB14I(S2,T2,DS,AS,BS)
         IF (TEST1.LE.EPSIL) THEN
...
C Compute new (feasible) 3-point ellipse
            CALL EB14J(X1R,X1I,X2R,X2I,X3R,X3I,XR,XI,D,A,B,A1,B1,R,C2,
     +                  IER)
            GO TO 40
         END IF
      END IF
...
C
C****************************************************************
      SUBROUTINE EB14C(IVP,XR,EV,D,C2,A2,A1)
C
...
C     ..
C     .. Local Scalars ..
      REAL A,B,BB,DB,EPS,PP,RR,TB,TEST
      INTEGER I
C     ..
C     .. External Functions ..
      REAL EB14I
...
C     ..
      ...
      A = EV(1,IVP) - D
   10 PP = PP + 1.
      IF (PP.GT.EPS) A = A + ABS((EV(1,IVP)- (PP*XR))/ (PP+1.))
      B = 0.
C  Determine B
      DO 20 I = 1,IVP
         RR = EV(1,I) - D
   ...
   20 CONTINUE
      DO 30 I = 1,IVP
         TEST = EB14I(EV(1,I),EV(2,I),D,A,B)
   ...
   30 CONTINUE
      ...
      END
C********************************************************************
      SUBROUTINE EB14D(MHULL,NUMEIG,EV,CH,ER,EI,WORK,D,C2,A2)
C
...
C     ..
C     .. Array Arguments ..
      REAL CH(2,MHULL),EI(NUMEIG),ER(NUMEIG),EV(2,MHULL),WORK(NUMEIG)
C     ..
C     .. Local Scalars ..
      REAL A2OLD,C2OLD,DOLD,EPSL,RATIO,RC,TA2,TC2,TD,TEST,TRC,WRT1,XREF
...
C     ..
      ...
      IF (C2.NE.ZERO .AND. EI(NUMEIG).NE.ZERO) THEN
         CALL EB14F(NUMEIG,ER,EI,WORK,RATIO,WRT1,D,C2,A2)
...
C
      DO 40 I = 1,MHULL
         DO 30 J = 1,MHULL
            IF (I.EQ.J) GO TO 30
            CALL EB14G(MHULL,EV,I,J,D,C2,A2,IFLAG)
            IF (IFLAG.EQ.0) THEN
               DO 20 K = 1,MHULL
...
C  Find the three point ellipse passing by points J,K,L
               CALL EB14H(MHULL,EV,J,K,L,D,C2,A2,IFLAG,RC)
               IF (IFLAG.EQ.0) THEN
...
C     ..
C     .. Local Scalars ..
      REAL DEN,DEN21,DEN23,RMACH,X21,X23,Y21,Y23
C     ..
C     .. External Functions ..
      REAL FD15A
      EXTERNAL FD15A
...
C     ..
      ...
      END
C******************************************************************
      SUBROUTINE EB14F(NEV,ER,EI,VEC,RATIO,WRT1,D,C2,A2)
C
C     Returns convergence ratios for the eigenvalues ER,EI, relative to
C     the ellipse defined by D,C2.
C
...
C Order ratios (descending order).
      ...
      END
C***************************************************************
      SUBROUTINE EB14G(M,CH,J,K,D,C2,A2,IFLAG)
C
...
C     ..
C     .. Local Scalars ..
      REAL A,AA,B,BB,EPS,S,T,TT,X,Y,Y1,Y2,Z
C     ..
C     .. External Subroutines ..
      EXTERNAL EB14M
C     ..
...
C See Abramowitz and Stegun p17 (the real root of a third degree poly.)
         ...
         Y1 = X + SQRT(X**2+Z**3)
         Y2 = X - SQRT(X**2+Z**3)
         Y = SIGN(ABS(Y1)** (1.0/3.0),Y1) + SIGN(ABS(Y2)** (1.0/3.0),Y2)
         A2 = Y + (BB*AA)/ (BB+TT)
         C2 = (A2* (A2-TT-AA))/ (A2-AA)
      ELSE
C Case iv: general case
         CALL EB14M(A,B,S,T,D,C2,A2,IFLAG)
      END IF
      RETURN
      END
C*****************************************************************
      SUBROUTINE EB14H(M,CH,J,K,L,D,C2,A2,IFLAG,RC)
C
...
C     ..
C     .. Local Scalars ..
      REAL DET,DIFF,DKJ,DLJ,DLK,EPS,W,Z
C     ..
C     .. External Functions ..
      REAL FD15A
      EXTERNAL FD15A
...
C
   ...
   20 RETURN
      END
C*************************************************************
      REAL FUNCTION EB14I(X,Y,D,A,B)
C
C     .. Parameters ..
      REAL EPS,ZERO
      PARAMETER (EPS=1.0E-4,ZERO=0.0)
C     ..
C     .. Scalar Arguments ..
      REAL A,B,D,X,Y
C     ..
      IF (A.EQ.ZERO) A = EPS
      IF (B.EQ.ZERO) B = EPS
      EB14I = ((X-D)* (X-D)/ (A*A)) + (Y*Y/ (B*B)) - 1.0
C
      RETURN
      END
C**************************************************************
      SUBROUTINE EB14J(X1,Y1,X2,Y2,X3,Y3,XR,YR,D,A,B,A1,B1,R,C2,IER)
C
...
C
C     .. Scalar Arguments ..
      REAL A,A1,B,B1,C2,D,R,X1,X2,X3,XR,Y1,Y2,Y3,YR
      INTEGER IER
C     ..
C     .. Local Scalars ..
      REAL A12,A2,AAA,ASB2,B2,BETA,X12D,X12M,X13D,X13M,X1D,XD,Y212,Y312
C     ..
C     .. Intrinsic Functions ..
      INTRINSIC SQRT
C     ..
...
C     ..
      ...
      X12M = (X1+X2)/2.0
      X13M = (X1+X3)/2.0
      Y212 = (Y2-Y1)* (Y2+Y1)/2.0
      Y312 = (Y3-Y1)* (Y3+Y1)/2.0
      X12D = X1 - X2
      X13D = X1 - X3
...
C
C**************************************************************
      SUBROUTINE EB14K(X1,Y1,X2,Y2,X,Y,R,A,B,A1,B1,D,C2)
C
...
C
C     .. Scalar Arguments ..
      REAL A,A1,B,B1,C2,D,R,X,X1,X2,Y,Y1,Y2
C     ..
C     .. Local Scalars ..
      REAL A12,A1K1,A1K2,A1PR,A1SB1,A2,A3,ALFA,AM,APR,ASB2,B1D,B1K1,
     +     B1K2,B1PR,B2,B3,BETA,BM,DR,X1D,X2D,XD,XX,Y1SX1,YY,YY1,Z11A,
     +     ZZ,ZZ1
      INTEGER II,INV
C     ..
C     .. External Subroutines ..
...
C     ..
      ...
      IF (Y2.EQ.Y1) THEN
         ...
         IF (Y1.EQ.ZERO) THEN
            ...
            R = A/ (A1+B1)
         ELSE
            CALL EB14L(X2,Y2,X,Y,D,A,B,A1,B1,R,C2)
         END IF
         GO TO 20
      END IF
      B = 10.0*Y1
      IF (Y2.GT.Y1) B = 10.0*Y2
      AM = X2 - X1
      BM = (Y2-Y1)* (Y2+Y1)/AM
C
      DO 10 II = 1,400
...
C
         ...
         A1K2 = (ZZ1*B/ (A1*B1D))
         A1PR = (ZZ*A1K1) - A1K2
         DR = (1.0+APR)/ (A1PR+B1PR)
         IF (ABS(R-DR).LE.1.E-4) GO TO 20
         ZZ = (1.0+R* (A1K2-B1K2))/ (R* (A1K1+B1K1)-ALFA)
         IF (ZZ.LT.ZERO) ZZ = -ZZ
         IF (ZZ.EQ.ZERO) ZZ = 1.E-3
         ASB2 = ZZ** (2.0/3.0)
         B2 = ((X1D*X1D)/ASB2) + (Y1*Y1)
...
C
      ...
      END
C**************************************************************
      SUBROUTINE EB14L(X,Y,X1,Y1,D,A,B,A1,B1,R,C2)
C
...
C
C     .. Scalar Arguments ..
      REAL A,A1,B,B1,C2,D,R,X,X1,Y,Y1
C     ..
C     .. Local Scalars ..
      REAL A12,A2,ASB2,AX,AX1,B2,BETA,DR,R1,R2,RP,XSY2,YSX12
      INTEGER I,NUM
C     ..
C     .. Intrinsic Functions ..
      INTRINSIC ABS,SQRT
...
C
         ...
         AX = ((A/B)**3.0)/XSY2
         AX1 = ((A1/B1)**3.0)*YSX12
         IF ((AX1.EQ.1.0) .AND. (AX.EQ.1.0)) THEN
            R = (A+B)/ (A1+B1)
            GO TO 20
         END IF
         R1 = (1.0-AX)/ (A*AX+B)
         R2 = (A1*AX1+B1)/ (1.0-AX1)
         DR = R1*R2
         R = (A+B)/ (A1+B1)
         IF ((ABS(R-DR).LE.1.E-3) .OR. (ABS(R-RP).LE.1.E-3)) GO TO 20
         RP = R
         R = R/R2
         AX = (1.0-B*R)/ (1.0+R*A)
   ...
   20 CONTINUE
      ...
      END
C********************************************************************
      SUBROUTINE EB14M(A,B,S,T,D,C2,A2,IFLAG)
C
...
C
C     .. Parameters ..
      REAL ZERO,TWO,THREE,FOUR,FIVE
      PARAMETER (ZERO=0.0,TWO=2.0,THREE=3.0,FOUR=4.0,FIVE=5.0)
C     ..
C     .. Scalar Arguments ..
      REAL A,A2,B,C2,D,S,T
      INTEGER IFLAG
C     ..
C     .. Local Scalars ..
      REAL E1,E2,PSCAL,R1,R2,R3,R4,Y,Z
      INTEGER N,NA,NB,NR
C     ..
C     .. Local Arrays ..
      REAL E(5),P(6),PP(6),ROOT(5)
C     ..
C     .. External Functions ..
      REAL FD15A
      EXTERNAL FD15A
...
C Reverse coefficient order for PA02B/BD.
      ...
      PP(5) = P(2)
      PP(6) = P(1)
      CALL PA02B(PP,E1,E2,NB,NR,NA,N,ROOT,E)
      IF (NR.NE.1) THEN
         IFLAG = 1
         RETURN
      ELSE
...
C Compute iteration parameters
      ...
      RETURN
      END
      SUBROUTINE PA02B(A,BX1,BX2,NB,NR,NA,N,ROOT,E)
...
C     ..
C     .. Local Scalars ..
      REAL C1,C2,D1,DZ,E1,E2,E3,EPS,F1,FP,FZ,G1,P1,P2,P3,SGN,STO,
     +     WLOG,WMAX,X,X1,X2,XXL,XXU,ZM,ZP,ZZ
      INTEGER I,ICR,IK,IR1,IS1,IS2,IS6,ISC,ISC1,ISW,J,M,M1,N2,NL,
     +        NR1,NRC,NRP
C     ..
C     .. Local Arrays ..
      REAL ALPHA(50),BETA(50),W1(50),W2(50)
C     ..
C     .. External Functions ..
      REAL FD15A
      EXTERNAL FD15A
C     ..
C     .. Intrinsic Functions ..
      INTRINSIC ABS,LOG,MAX,EXP,SIGN,REAL
C     ..
