...
do
  ...
  DEPS=`grep -H -l1 "  use ${i}" ${SOURCES} | awk '{print "${LIB_DIR}/${ARCH}/" $1 ":"}' | sed s/.f90:/.o/`
  DEPS2=`grep -H -l1 "  use ${i}" ${SOURCES} | awk '{print "${LIB_DIR}/${ARCH}/mpi/" $1 ":"}' | sed s/.f90:/.o/`
  DEPS5=`grep -H -l1 "  use ${i}" ${SOURCES} | awk '{print "${LIB_DIR}/${ARCH}/threads/" $1 ":"}' | sed s/.f90:/.o/`
  DEPS7=`grep -H -l1 "  use ${i}" ${SOURCES} | awk '{print "${LIB_DIR}/${ARCH}/mpi_threads/" $1 ":"}' | sed s/.f90:/.o/`
  DEPS3=`grep -H -l1 "  use ${i}" ${MODOS} | awk '{print "${LIB_DIR}/${ARCH}/" $1 ":"}' | sed s/.f90:/.o/`
  DEPS4=`grep -H -l1 "  use ${i}" ${MODOS} | awk '{print "${LIB_DIR}/${ARCH}/mpi/" $1 ":"}' | sed s/.f90:/.o/`
  DEPS6=`grep -H -l1 "  use ${i}" ${MODOS} | awk '{print "${LIB_DIR}/${ARCH}/threads/" $1 ":"}' | sed s/.f90:/.o/`
  DEPS8=`grep -H -l1 "  use ${i}" ${MODOS} | awk '{print "${LIB_DIR}/${ARCH}/mpi_threads/" $1 ":"}' | sed s/.f90:/.o/`
...
MDEPS=`grep -H -l1 "#include \"macros.i\"" ${SOURCES} | awk '{print "${LIB_DIR}/${ARCH}/" $1}' | sed s/.f90/.o/`
MDEPS2=`grep -H -l1 "#include \"macros.i\"" ${SOURCES} | awk '{print "${LIB_DIR}/${ARCH}/mpi/" $1}' | sed s/.f90/.o/`
MDEPS3=`grep -H -l1 "#include \"macros.i\"" ${SOURCES} | awk '{print "${LIB_DIR}/${ARCH}/threads/" $1}' | sed s/.f90/.o/`
MDEPS4=`grep -H -l1 "#include \"macros.i\"" ${SOURCES} | awk '{print "${LIB_DIR}/${ARCH}/mpi_threads/" $1}' | sed s/.f90/.o/`
echo ${MDEPS} ${MDEPS2} ${MDEPS3} ${MDEPS4}: \${SRC_DIR}/macros.i >> ${SRC_DIR}/DEPENDENCIES

MDEPS=`grep -H -l1 "#include \"macros.i\"" ${MODOS} | awk '{print "${LIB_DIR}/${ARCH}/" $1}' | sed s/.f90/.o/`
MDEPS2=`grep -H -l1 "#include \"macros.i\"" ${MODOS} | awk '{print "${LIB_DIR}/${ARCH}/mpi/" $1}' | sed s/.f90/.o/`
MDEPS3=`grep -H -l1 "#include \"macros.i\"" ${MODOS} | awk '{print "${LIB_DIR}/${ARCH}/threads/" $1}' | sed s/.f90/.o/`
MDEPS4=`grep -H -l1 "#include \"macros.i\"" ${MODOS} | awk '{print "${LIB_DIR}/${ARCH}/mpi_threads/" $1}' | sed s/.f90/.o/`
echo ${MDEPS} ${MDEPS2} ${MDEPS3} ${MDEPS4}: \${SRC_DIR}/macros.i >> ${SRC_DIR}/DEPENDENCIES
