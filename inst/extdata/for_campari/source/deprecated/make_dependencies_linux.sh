...
echo "Found the following source files in the specified directory: ${SOURCES}"
...
do
  ...
  DEPS5=`grep -H -m 1 "  use ${i}" ${SOURCES} | awk '{print "${LIB_DIR}/${ARCH}/threads/" $1}' | sed s/.f90:/.o/`
  DEPS7=`grep -H -m 1 "  use ${i}" ${SOURCES} | awk '{print "${LIB_DIR}/${ARCH}/mpi_threads/" $1}' | sed s/.f90:/.o/`
  ...
  DEPS6=`grep -H -m 1 "  use ${i}" ${MODOS} | awk '{print "${LIB_DIR}/${ARCH}/threads/" $1}' | sed s/.f90:/.o/`
  DEPS8=`grep -H -m 1 "  use ${i}" ${MODOS} | awk '{print "${LIB_DIR}/${ARCH}/mpi_threads/" $1}' | sed s/.f90:/.o/`
...
MDEPS3=`grep -H -m 1 "#include \"macros.i\"" ${SOURCES} | awk '{print "${LIB_DIR}/${ARCH}/threads/" $1}' | sed s/.f90:#include/.o/`
MDEPS4=`grep -H -m 1 "#include \"macros.i\"" ${SOURCES} | awk '{print "${LIB_DIR}/${ARCH}/mpi_threads/" $1}' | sed s/.f90:#include/.o/`
echo ${MDEPS} ${MDEPS2} ${MDEPS3} ${MDEPS4}: \${SRC_DIR}/macros.i >> ${SRC_DIR}/DEPENDENCIES

MDEPS=`grep -H -m 1 "#include \"macros.i\"" ${MODOS} | awk '{print "${LIB_DIR}/${ARCH}/" $1}' | sed s/.f90:#include/.o/`
MDEPS2=`grep -H -m 1 "#include \"macros.i\"" ${MODOS} | awk '{print "${LIB_DIR}/${ARCH}/mpi/" $1}' | sed s/.f90:#include/.o/`
MDEPS3=`grep -H -m 1 "#include \"macros.i\"" ${MODOS} | awk '{print "${LIB_DIR}/${ARCH}/threads/" $1}' | sed s/.f90:#include/.o/`
MDEPS4=`grep -H -m 1 "#include \"macros.i\"" ${MODOS} | awk '{print "${LIB_DIR}/${ARCH}/mpi_threads/" $1}' | sed s/.f90:#include/.o/`
echo ${MDEPS} ${MDEPS2} ${MDEPS3} ${MDEPS4}: \${SRC_DIR}/macros.i >> ${SRC_DIR}/DEPENDENCIES
