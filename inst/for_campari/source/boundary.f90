...
!
  ...
  use system
  use atoms
  use movesets, ONLY: trans_xyzbuf
!
  implicit none
!
  integer i,imol
  RTYPE dis2,como(3),random,tvec(3),hlp
  logical done
!
  do i=1,3
    como(i) = com(imol,i)
...
!
  if (bnd_shape.eq.1) then
    if (bnd_type.eq.1) then
      ...
      do i=1,3
        com(imol,i) = (random()-0.5)*bnd_params(i)*trans_xyzbuf + 0.5*bnd_params(i)
        com(imol,i) = com(imol,i) + bnd_params(3+i)
      end do
    end if
  else if (bnd_shape.eq.2) then ! sphere
    done = .false.
    hlp = trans_xyzbuf*trans_xyzbuf*bnd_params(5)
    do while (done.EQV..false.)
      do i=1,3
        com(imol,i) = (random()*2.0 - 1.0)*bnd_params(4)*trans_xyzbuf
      end do
      dis2 = 0.0
      do i=1,3
        dis2 = dis2 + com(imol,i)*com(imol,i)
      end do
      if (dis2.le.hlp) then
        done = .true.
      end if
    ...
    hlp = trans_xyzbuf*trans_xyzbuf*bnd_params(5)
    do while (done.EQV..false.)
      com(imol,1) = trans_xyzbuf*bnd_params(4)*(2.0*random()-1.0)
      com(imol,2) = trans_xyzbuf*bnd_params(4)*(2.0*random()-1.0)
      dis2 = 0.0
      do i=1,2
        dis2 = dis2 + com(imol,i)*com(imol,i)
      end do
    ...
    if (bnd_type.eq.1) then
      ...
      com(imol,3) = trans_xyzbuf*bnd_params(6)*(random()-0.5)
    end if
    com(imol,:) = com(imol,:) + bnd_params(1:3)
  else
...
!
  integer i
  RTYPE dis2,como(3),comv(3),random,tvec(3),hlp
  logical done
!
  do i=1,3
    como(i) = comv(i)
...
! rectangular box
  if (bnd_shape.eq.1) then
    if (bnd_type.eq.1) then
      ...
      do i=1,3
        comv(i) = (random()-0.5)*bnd_params(i)*trans_xyzbuf + 0.5*bnd_params(i)
        comv(i) = comv(i) + bnd_params(3+i)
      end do
    end if
! spherical system
    ...
    do while (done.EQV..false.)
      do i=1,3
        comv(i) =  (random()*2.0 - 1.0)*bnd_params(4)*trans_xyzbuf
      end do
      dis2 = 0.0
      do i=1,3
        dis2 = dis2 + comv(i)*comv(i)
      end do
      if (dis2.le.hlp) then
        done = .true.
      end if
...
! cylinder
    ...
    hlp = trans_xyzbuf*trans_xyzbuf*bnd_params(5)
    do while (done.EQV..false.)
      comv(1) = trans_xyzbuf*bnd_params(4)*(2.0*random()-1.0)
      comv(2) = trans_xyzbuf*bnd_params(4)*(2.0*random()-1.0)
      dis2 = 0.0
      do i=1,2
        dis2 = dis2 + comv(i)*comv(i)
      end do
    ...
    if (bnd_type.eq.1) then
      ...
      comv(3) = trans_xyzbuf*bnd_params(6)*(random()-0.5)
    end if
    comv(:) = comv(:) + bnd_params(1:3)
  else
