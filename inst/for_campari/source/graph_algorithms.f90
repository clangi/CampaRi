...
! CONTRIBUTIONS: Marco Bacci, Nicolas Bloechliger                          !
!                                                                          !
!--------------------------------------------------------------------------!
!
#include "macros.i"
...
!
    ...
    if (use_REMC.EQV..true.) then
      ...
      fn = 'N_'//nod(1:re_aux(10))//'_STRUCT_CLUSTERING.clu'
...
#endif
!
  if ((tpi.le.1).AND.((cequil.gt.0).OR.(dopfold.EQV..true.).OR.(synmode.ne.0).OR.(eigvalmd.ne.0).OR.(ccfepmode.gt.0).OR.&
 &                    (caddlkmd.ne.0))) then
...
!
! if requested, augment network: implies calling tarjan_SCC -> adds links and computes strongly connected components
  call augment_network(it,nbasins,caddlkmd,maxtime_eqmsm,tpi)
!
...
!$OMP BARRIER
#endif
  if ((dopfold.EQV..true.).OR.(synmode.ne.0).OR.(eigvalmd.ne.0).OR.(ccfepmode.gt.0).OR.(caddlkmd.ne.0)) then
!   set refscc, inissnap, and endssnap to legal values 
    if (tmat_report.EQV..true.) then
      i = 1
      j = 1
      call edit_tmat(it,nbasins,i,j,tpi)
      i = refscc
#ifdef ENABLE_THREADS
!$OMP MASTER
#endif
...
! Eigenvalues and eigenvectors (timescales and projections) 
  if (eigvalmd.ne.0) then
    i = 1 ! work with fewtsnb(:,1:2)
    k = 1 ! zero out SCC-spanning transitions and normalize to resultant sub-matrices
#ifdef ENABLE_THREADS
!$OMP BARRIER
    call edit_tmat(it,nbasins,i,k,tpi)
!$OMP MASTER
#else
    call edit_tmat(it,nbasins,i,k,azero)
#endif
...
#ifdef ENABLE_THREADS
!$OMP BARRIER
    call edit_tmat(it,nbasins,j,k,tpi)
!$OMP MASTER
#else
    call edit_tmat(it,nbasins,j,k,azero)
#endif
...
#endif
!   Fix the initial and end snapshots first and reference component
    if (it(sconnect(inissnap,4))%nb.le.0) then
      if (tpi.le.1) write(ilog,*) 'Warning. Initial snapshot for the generation of synthetic trajectories belongs to a cluster &
 &that is cut off from the network. Disabled.'
      synmode = 0
    end if
 ...
 &the search is limited to that component the initial and target cluster belong to.'
    ...
    else if ((maxval(it(1:nbasins)%inscc).ne.minval(it(1:nbasins)%inscc)).AND.(synmode.gt.0)) then
...
!     populate fewtsnb(:,3:4) with the cumulative sums for picking via binary_search
      call edit_tmat(it,nbasins,athree,aone,tpi)
    end if
    if (tpi.le.1) call System_Clock(count=tcnt(7))
    if (synmode.ne.0) then
      if (tpi.le.1) then
        write(ilog,*)
...
!$OMP BARRIER
      ...
      if (((which_timeflow.eq.2).OR.(which_timeflow.eq.3)).AND.(synmode.ne.0)) then
        dofwr = .false.
        call synth_traj_MSM(it,nbasins,sconnect(1:cstored,4),inissnap,endssnap,dofwr,tpi)
      end if
    end if
...
!$OMP BARRIER
    ...
    if (size(csccwts).gt.1) then
      if ((inissnap_usrslct.gt.0).OR.((ccfepmode.gt.7).AND.(ccfepmode.le.10))) then
        if (tpi.le.1) write(ilog,*) 'Warning. The computed cut-based free energy profiles will only include that strongly &
 &connected component containing the chosen reference snapshot (FMCSC_INISYNSNAP).'
      else if (inissnap_usrslct.eq.0) then
        if (tpi.le.1) write(ilog,*) 'Warning. The computed cut-based free energy profiles will only include that strongly &
 &connected component containing the cluster with the largest sampling weight.'
      else if (inissnap_usrslct.eq.-1) then
        if (tpi.le.1) write(ilog,*) 'Remark. Potentially computing separate cut-based free energy profiles for individual strongly&
 & connected components from the respective clusters with the largest sampling weights.'
      else if (inissnap_usrslct.eq.-2) then
        if (tpi.le.1) write(ilog,*) 'Warning. The computed cut-based free energy profiles will only include the largest strongly &
 &connected component.'
      end if
    end if
    do kk=1,size(csccwts) ! loop over components
!     use only target component unless requested otherwise
      if (((inissnap_usrslct.ne.-1).OR.((ccfepmode.gt.7).AND.(ccfepmode.le.10))).AND.(refscc.ne.kk)) cycle
      foundit = .false.
      do j=1,2
...
!       always use raw sampling weight as reference probability vector (even though this may not truly be the "largest" in ss)
        ssix = 1 !index of raw sampling weight in %nodewt
        if (j.eq.1) then
          dofwr = .true.
        else
          dofwr = .false.
...
!       for the weight in ssix, find the largest cluster for the component in question
        ...
        if (ll.gt.1) then
          ...
          if ((inissnap_usrslct.eq.-2).AND.(sconnect(inissnap,4).ne.k)) then
            write(ilog,*) 'Fatal. An error in sel_ssnap that reflects in gen_graph_from_clusters? This is a bug.'
            call fexit()
          end if 
          if ((ccfepmode.le.7).OR.(ccfepmode.gt.10)) then
            call cfep(it,nbasins,k,dval,ccfepmode,dofwr,tpi)
          else
            if ((ccfepmode.eq.8).OR.(ccfepmode.eq.10)) then ! (+) committor
              ii = 8
              call cfep(it,nbasins,k,dval,ii,dofwr,tpi)
            end if
          ...
          if (tpi.le.1) write(ilog,*) 'Warning. Skipping calculation of cut-based free energy profile(s) for component ',kk,' &
 &as it only comprises a single cluster.'
          foundit = .true.
        end if
      end do
    end do
...
!
  ...
  call int2str(myrank,nod,re_aux(10))
  if (use_REMC.EQV..true.) then
    fn = 'N_'//nod(1:re_aux(10))//'_STRUCT_CLUSTERING.graphml'
  else
...
! 33 format(a,i<ilen>,a)
  ...
  do i=1,nclusters
    ...
    write(istr,*) ilen
    do j=1,it(i)%nb
      if ((it(i)%wghtsnb(j,1).le.0).AND.(it(i)%fewtsnb(j,1).le.0.0)) cycle
      if (k.le.0) then
        klen = 1
      else
        klen = log10(1.0*k) + 1
...
! this may edit it(:)%wghtsnb(:,1:2) and it(:)%fewtsnb(:,1:2)
!
! it can create inevitable inconsistencies between the two, which means that edit_tmat and similar routines
! should not subsequently utilize wghtsnb, at least for those cases
!
subroutine augment_network(it,nbasins,mode,maxtime,tpi)
!
  use clusters
  use iounit
  use threads, ONLY: thr_rutil,thrdat
  use, INTRINSIC:: ISO_FORTRAN_ENV
!
  implicit none
!
  integer, INTENT(IN):: nbasins,mode,tpi
  type(t_scluster):: it(nbasins)
  integer(KIND=8), INTENT(IN):: maxtime
!
  integer i,j,k,mred,nsccs,iters,sts(4),cbnd(2),isbad(2),tonext,lastt
  RTYPE pms(5),eps,naddr(3),loglk(10),maxdev(2),thresh,kr,ptmp(2)
  logical haveself
...
!
  if ((tpi.le.0).AND.(OMP_IN_PARALLEL().EQV..true.)) then
    write(ilog,*) 'Fatal. When using multi-threaded code, augment_network(...) must always be called &
 &by all threads of the binding region (instead got a thread identifier of zero). This is a bug.'
  ...
  if (tpi.gt.0) then
    ...
    call threads_bounds(i,j,tpi,k,cbnd) ! linear in clusters
    j = 4
    call get_thread_loop_bounds(i,j,sts(1:2),sts(3:4),tpi)! 1,4 means clusters by numbers of neighbors
    sts(2) = sts(3)
  else
    cbnd(1) = 1
    cbnd(2) = nbasins
...
!
  if (mode.eq.0) return
!
  if (tpi.le.1) write(ilog,*) 'Now augmenting/editing network ...'
!
  loglk(:) = 0.0
  i = 1
  j = 1
  ...
  do i=sts(1),sts(2)
    do j=1,it(i)%nb
      if (it(i)%fewtsnb(j,3).gt.0) loglk(1) = loglk(1) + it(i)%wghtsnb(j,1)*log(it(i)%fewtsnb(j,3))
      if (it(i)%fewtsnb(j,4).gt.0) loglk(2) = loglk(2) + it(i)%wghtsnb(j,2)*log(it(i)%fewtsnb(j,4))
    end do
  end do
#ifdef ENABLE_THREADS
!$OMP BARRIER
...
!   fix missing self transitions and reverse edges
    do i=sts(1),sts(2)
      haveself = .false.
      do j=1,it(i)%nb
        it(i)%fewtsnb(j,3:4) = 1.0*it(i)%wghtsnb(j,1:2) ! hi-jack (briefly) to backup
        if (it(i)%lstnb(j).eq.i) then
          haveself = .true.
        end if
        if (it(i)%inscc.ne.it(it(i)%lstnb(j))%inscc) then
          if ((mode.eq.1).OR.(mode.eq.3)) then ! supplement
          ...
          else if ((mode.eq.-1).OR.(mode.eq.-3).OR.(mode.eq.-4).OR.(mode.eq.-5).OR.(mode.eq.-6)) then ! deplete
            if (it(i)%wghtsnb(j,1).gt.0) then
              naddr(3) = naddr(3) + 0.5*it(i)%wghtsnb(j,1)
              it(i)%wghtsnb(j,1) = 0
              it(i)%fewtsnb(j,1) = 0.0
            end if
            if (it(i)%wghtsnb(j,2).gt.0) then
              naddr(3) = naddr(3) + 0.5*it(i)%wghtsnb(j,2)
              it(i)%wghtsnb(j,2) = 0
              it(i)%fewtsnb(j,2) = 0.0
            end if
          end if
        end if
        if ((mode.eq.4).OR.(mode.eq.5).OR.(mode.eq.-4).OR.(mode.eq.-5)) then
          if (it(i)%wghtsnb(j,2).gt.it(i)%wghtsnb(j,1)) then
            k = it(i)%wghtsnb(j,2) - it(i)%wghtsnb(j,1)
            naddr(2) = naddr(2) + 0.5*k
      ...
      if ((haveself.EQV..false.).AND.&
 &        ((mode.eq.2).OR.(mode.eq.3).OR.(mode.eq.5).OR.(mode.eq.-2).OR.(mode.eq.-3).OR.(mode.eq.-5))) then
        naddr(1) = naddr(1) + 1.0
        it(i)%nb = it(i)%nb + 1
...
! Prinz et al. max-prob estimator
  if ((mode.eq.6).OR.(mode.eq.-6)) then
#ifdef ENABLE_THREADS
!$OMP MASTER
#endif
    ...
    do i=sts(1),sts(2)
      ...
      do j=1,it(i)%nb
        if (it(i)%lstnb(j).eq.i) then
          haveself = .true.
          k = it(i)%wghtsnb(j,1)
          it(i)%fewtsnb(j,3) = 2.0*k
        else
          it(i)%fewtsnb(j,3) = 1.0*(it(i)%wghtsnb(j,1) + it(it(i)%lstnb(j))%wghtsnb(it(i)%map(j),1))
        end if
      end do
      if ((it(i)%nb.eq.0).OR.((haveself.EQV..true.).AND.(k.le.0).AND.(it(i)%nb.eq.1))) then ! ill-defined
        it(i)%rflw = 0
        it(i)%active = -1
      else if ((it(i)%nb.eq.1).AND.(haveself.EQV..true.)) then ! cut off explicitly
        it(i)%rflw = 0
        it(i)%active = 0
      else if ((haveself.EQV..true.).AND.(nint(sum(it(i)%fewtsnb(1:it(i)%nb,3))).eq.2*k)) then ! effectively cut off
        it(i)%rflw = 0
        it(i)%active = 0
      else
        it(i)%rflw = nint(sum(it(i)%fewtsnb(1:it(i)%nb,3))) ! hi-jack
        it(i)%active = it(i)%rflw - k ! hi-jack
      end if
      it(i)%ldis = k ! hi-jack
      it(i)%fewtsnb(1,4) = 2.0*k ! hi-jack, relies on default alsz of 2
      it(i)%fewtsnb(2,4) = 1.0*it(i)%rflw
    end do
#ifdef ENABLE_THREADS
!$OMP BARRIER
#endif
    ...
    do while (1.eq.1)
      maxdev(1) = 0.0
      do i=cbnd(1),cbnd(2)
        if (it(i)%active.le.0) cycle
        pms(5) = it(i)%fewtsnb(1,4)
        it(i)%fewtsnb(2,4) = it(i)%fewtsnb(2,4) - it(i)%fewtsnb(1,4)
        it(i)%fewtsnb(1,4) = (it(i)%ldis*it(i)%fewtsnb(2,4))/(1.0*it(i)%active)
        maxdev(1) = max(maxdev(1),abs(it(i)%fewtsnb(1,4)-pms(5)))
        it(i)%fewtsnb(2,4) = it(i)%fewtsnb(2,4) + it(i)%fewtsnb(1,4)
      end do
#ifdef ENABLE_THREADS
!$OMP BARRIER
      ...
      do i=sts(1),sts(2)
        do j=1,it(i)%nb
          if (it(i)%lstnb(j).le.i) cycle
          k = it(i)%wghtsnb(j,1) + it(it(i)%lstnb(j))%wghtsnb(it(i)%map(j),1)
          if (k.eq.0) cycle ! write(*,*) i,it(i)%lstnb(j)
          pms(5) = it(i)%fewtsnb(j,3)
          pms(1) = 2.0*(it(i)%rflw + it(it(i)%lstnb(j))%rflw - k)
          pms(2) = it(i)%rflw*( it(it(i)%lstnb(j))%fewtsnb(2,4) - it(i)%fewtsnb(j,3) ) + &
 &                 it(it(i)%lstnb(j))%rflw*( it(i)%fewtsnb(2,4) - it(i)%fewtsnb(j,3) ) - &
 &                 k*( it(i)%fewtsnb(2,4) + it(it(i)%lstnb(j))%fewtsnb(2,4) - 2.0*it(i)%fewtsnb(j,3) )
          pms(3) = ( it(it(i)%lstnb(j))%fewtsnb(2,4) - it(i)%fewtsnb(j,3) )*&
 &                 ( it(i)%fewtsnb(2,4) - it(i)%fewtsnb(j,3) )*(-1.0*k)
          pms(4) = pms(2)*pms(2) - 2.0*pms(1)*pms(3)
          if ((pms(4).ge.0.0).AND.(pms(1).gt.0.0)) then
            it(i)%fewtsnb(j,3) = (-pms(2) + sqrt(pms(4)))/pms(1)
            it(it(i)%lstnb(j))%fewtsnb(it(i)%map(j),1) = it(i)%fewtsnb(j,3) ! these values are unused in this loop (i < it(i)%lstnb)
            maxdev(1) = max(maxdev(1),abs(it(i)%fewtsnb(j,3)-pms(5)))
          else if ((pms(4).gt.eps).AND.(pms(1).gt.0.0)) then
            it(i)%fewtsnb(j,3) = -pms(2)/pms(1)
            it(it(i)%lstnb(j))%fewtsnb(it(i)%map(j),1) = it(i)%fewtsnb(j,3)
            maxdev(1) = max(maxdev(1),abs(it(i)%fewtsnb(j,3)-pms(5)))
          else if (isbad(1).le.0) then
            write(ilog,*) 'Warning. Constrained maximum likelihood algorithm (CADDLINKMODE 6) was not successful. Reverting to &
 &direct ML estimate.'
            isbad(1) = 1  
          end if
        end do
...
#endif
      do i=sts(1),sts(2)
        if (it(i)%active.gt.0) then
!         contains self-transition for nodes who originally had one
          it(i)%fewtsnb(2,4) = sum(it(i)%fewtsnb(1:it(i)%nb,3))
!         for nodes missing the self-transition, it is redundant to add the extra field since it can never become nonzero
!          if (it(i)%ldis.le.0) it(i)%fewtsnb(2,4) = it(i)%fewtsnb(2,4) + it(i)%fewtsnb(1,4)
        end if
      end do
      iters = iters + 1
#ifdef ENABLE_THREADS
...
!$OMP BARRIER
      if (mod(iters,20).eq.0) then
        ...
        i = 1
        j = 50
        call thr_freduction(mred,maxdev(1:1),i,j,tpi)
        if (maxdev(1).le.thresh) exit
        mred = -1
        j = 10
        call thr_ireduction(mred,isbad(1:1),i,j,tpi)
        if (isbad(1).gt.0) exit
      end if
#else
      if (mod(iters,20).eq.0) then
        if (maxdev(1).le.thresh) exit
...
!$OMP MASTER
#endif
      if (iters.eq.tonext) then
        call System_Clock(count=tts(2))
        if ((tts(2)-tts(1)).gt.maxtime) then
          write(ilog,*) 'Aborting after ',iters,' iterations due to maximum time of ',(1.0*maxtime)/thrdat%rate,&
   &' [s] being exceeded.'
#ifdef   ENABLE_THREADS
!         set a termination signal
          ...
          kr = (1.0*iters)/(max(1_8,(tts(2)-tts(1)))/thrdat%rate) ! iterations per second
#else
          kr = (1.0*iters)/(max(1_INT64,(tts(2)-tts(1)))/thrdat%rate) ! iterations per second
#endif
          lastt = tonext
          i = nint(kr*0.1*maxtime/thrdat%rate) ! 10% into the future of maximum allowed time
          tonext = iters + max(1,i)
        end if
      end if
#ifdef ENABLE_THREADS
!$OMP END MASTER
!$OMP BARRIER
      if (thr_rutil(49,1).lt.0.0) exit
#endif
    end do
#ifdef ENABLE_THREADS
!$OMP BARRIER
#endif
    if (isbad(1).gt.0) then
      do i=sts(1),sts(2)
        do j=1,it(i)%nb
          it(i)%fewtsnb(j,1:2) = 1.0*it(i)%wghtsnb(j,1:2)
        end do
      end do
      return
    else
      do i=sts(1),sts(2)
        ...
        do j=1,it(i)%nb
          if (it(i)%lstnb(j).eq.i) then
            naddr(1) = naddr(1) + it(i)%fewtsnb(j,1)-it(i)%fewtsnb(1,4)
            it(i)%fewtsnb(j,1:2) = it(i)%fewtsnb(1,4)
          else if (it(i)%lstnb(j).gt.i) then
            naddr(2) = naddr(2) + sum(it(i)%fewtsnb(j,1:2)-it(i)%fewtsnb(j,3))
            if (it(i)%wghtsnb(j,1)+it(i)%wghtsnb(j,2).gt.0) then
              pms(1) = (it(i)%wghtsnb(j,2)*it(i)%lensnb(j,2) + it(i)%wghtsnb(j,1)*it(i)%lensnb(j,1))/&
 &                   (1.0*(it(i)%wghtsnb(j,1)+it(i)%wghtsnb(j,2))) ! original weighting
            else
              pms(1) = 0.0
            end if
            it(i)%fewtsnb(j,1:2) = it(i)%fewtsnb(j,3)
            it(i)%lensnb(j,1:2) = pms(1)
            it(it(i)%lstnb(j))%fewtsnb(it(i)%map(j),1:2) = it(i)%fewtsnb(j,3)
            it(it(i)%lstnb(j))%lensnb(it(i)%map(j),1:2) = pms(1)
          end if
        end do
#ifdef ENABLE_THREADS
      end do
!$OMP BARRIER
      do i=sts(1),sts(2)
#endif
        if ((it(i)%ldis.eq.0).AND.(it(i)%fewtsnb(1,4).gt.0.0)) then
          naddr(1) = naddr(1) + it(i)%fewtsnb(1,4)
          it(i)%nb = it(i)%nb + 1
          if (it(i)%nb.gt.it(i)%nbalsz) then
            call scluster_resizenb(it(i))
          end if
          ...
          it(i)%wghtsnb(it(i)%nb,1:2) = 1
          it(i)%lensnb(it(i)%nb,1:2) = (0.5*cradius)**2
          it(i)%fewtsnb(it(i)%nb,1:2) = it(i)%fewtsnb(1,4)
        end if
      end do
    end if
  end if
...
!$OMP BARRIER
  ...
  do i=sts(1),sts(2)
    ...
    do j=1,it(i)%nb
      ...
      ptmp(:) = ptmp(:) + it(i)%fewtsnb(j,1:2)
    end do
    if (ptmp(1).gt.0.0) ptmp(1) = 1.0/ptmp(1)
    if (ptmp(2).gt.0.0) ptmp(2) = 1.0/ptmp(2)
    do j=1,it(i)%nb
      if (it(i)%inscc.ne.it(it(i)%lstnb(j))%inscc) cycle
      if ((mode.ne.6).AND.(mode.ne.-6)) then
        if (it(i)%fewtsnb(j,1).gt.0) loglk(3) = loglk(3) + nint(it(i)%fewtsnb(j,3))*log(it(i)%fewtsnb(j,1)*ptmp(1))
        if (it(i)%fewtsnb(j,2).gt.0) loglk(4) = loglk(4) + nint(it(i)%fewtsnb(j,4))*log(it(i)%fewtsnb(j,2)*ptmp(2))
      else
        if (it(i)%fewtsnb(j,1).gt.0) loglk(3) = loglk(3) + it(i)%wghtsnb(j,1)*log(it(i)%fewtsnb(j,1)*ptmp(1))
        if (it(i)%fewtsnb(j,2).gt.0) loglk(4) = loglk(4) + it(i)%wghtsnb(j,2)*log(it(i)%fewtsnb(j,2)*ptmp(2))
      end if
    end do
  end do
  i = 1
...
!$OMP BARRIER
 ...
 i = 4
 j = 51
 call thr_freduction(mred,loglk(1:4),i,j,tpi)
 j = 55
 i = 3
 call thr_freduction(mred,naddr(1:3),i,j,tpi)
!$OMP MASTER
#endif
  k = 2
  call tarjan_SCC(it,nbasins,k)
  k = maxval(it(1:nbasins)%inscc)
 561 format(' Done. Added ',i7,' self transitions and ',i7,' missing reverse links for connectedness.',/,&
 &        ' The recalculated number of strongly connected components is ',i7,' (from ',i7,').')
 562 format(' Done. Imposition of detailed balance added a total link weight of ',i7,', and ',i7,' self transitions were added.'&
 &        ,/,' The recalculated number of strongly connected components is ',i7,' (from ',i7,').')
 563 format(' Done after ',i9,' iterations. Imposition of detailed balance modified the total, non-self link weight by &
 &',g12.5,', and self transitions (loops) by ',g12.5,'.',/,' The recalculated number of strongly connected components &
 &is ',i7,' (from ',i7,').')
 565 format(' Done. Added ',i7,' self transitions and removed ',i7,' links connecting different SCCs.',/,&
 &        ' The recalculated number of strongly connected components is still ',i7,' (from ',i7,').')
 566 format(' Done. Removed ',i7,' links connecting different SCCs. Subsequently, the imposition of detailed balance per SCC &
 &added a total link weight of ',i7,', and ',i7,' self transitions were added.'&
 &        ,/,' The recalculated number of strongly connected components is still ',i7,' (from ',i7,').')
 567 format(' Done after ',i9,' iterations following the removal of ',i7,' links connecting different SCCs. &
 &SCC-local imposition of detailed balance subsequently modified the total, non-self link weight by &
 &',g12.5,', and self transitions (loops) by ',g12.5,'.',/,' The recalculated number of strongly connected components &
 &is still ',i7,' (from ',i7,').')
 564 format(' The log-likelihood values for the direct count-based and the augmented/edited forward time transition matrices &
 &are ',g14.7,' and ',g14.7,', respectively.',/,' The same for the backward time matrices are ',g14.7,' and ',g14.7,'.')
 569 format(' All connections bridging different SCCs in the original network are ignored, and the results can represent a &
 &composite log-likelihood across multiple disjoint transition matrices.')

  if (mode.gt.0) then
    if (mode.le.3) then
      write(ilog,561) nint(naddr(1)),nint(naddr(2)),k,nsccs
    else if (mode.le.5) then
      write(ilog,562) nint(naddr(2)),nint(naddr(1)),k,nsccs
    else if (mode.eq.6) then
      write(ilog,563) iters,naddr(2),naddr(1),k,nsccs
    end if
  else if (mode.lt.0) then
    if (mode.ge.-3) then
      write(ilog,565) nint(naddr(1)),nint(naddr(3)),k,nsccs
    else if (mode.ge.-5) then
      write(ilog,566) nint(naddr(3)),nint(naddr(2)),nint(naddr(1)),k,nsccs
    else if (mode.eq.-6) then
      write(ilog,567) iters,nint(naddr(3)),naddr(2),naddr(1),k,nsccs
    end if
  end if
  write(ilog,564) loglk(1),loglk(3),loglk(2),loglk(4)
  write(ilog,569)
  nsccs = k
#ifdef ENABLE_THREADS
!$OMP END MASTER
...
!
subroutine cfep(it,nbasins,snode,thresher,nmode,dofwr,tpi)
!
  use clusters
  use iounit
  use mpistuff
  use interfaces
  use system
  use threads, ONLY: thrdat,thr_rhlper
!
  implicit none
!
...
!
  integer nbasins,snode,i,j,k,ipdbh,ii,jj,freeunit,tx,nmode,t1,t2,ssix,fbix,azero,aone,cbnd(2),sts(4)
  RTYPE thresher,totew,progger,rhlp(1)
  type(t_scluster) it(nbasins)
...
!
  if ((tpi.le.0).AND.(OMP_IN_PARALLEL().EQV..true.)) then
    write(ilog,*) 'Fatal. When using multi-threaded code, cfep(...) must always be called &
 &by all threads of the binding region (instead got a thread identifier of zero). This is a bug.'
    call fexit()
  end if
!$OMP BARRIER
!$OMP SINGLE
  if (allocated(thr_rhlper).EQV..true.) then
    deallocate(thr_rhlper)
  end if
  allocate(thr_rhlper(nbasins,1))
!$OMP END SINGLE
  i = 1
  j = nbasins
  if (tpi.gt.0) then
...
! for the cut function, we use the raw link weights, which generally are integer, but may be not 
  reqreal = .false.
  if ((caddlkmd.eq.6).OR.(caddlkmd.eq.-6).OR.&
 &    ((caddlinkwt.ne.1.0).AND.(caddlkmd.ne.0).AND.(caddlkmd.ne.4).AND.(caddlkmd.ne.-4))) reqreal = .true.
...
! first determine or copy the ordering parameter
  ...
  if (nmode.eq.1) then
#ifdef ENABLE_THREADS
    call iterative_mfpt(it,nbasins,snode,thr_rhlper(:,1),thresher,dofwr,maxtime_eqmsm,tpi)
#else
    call iterative_mfpt(it,nbasins,snode,mfptv,thresher,dofwr,maxtime_eqmsm,tpi)
#endif
...
#ifdef ENABLE_THREADS
!$OMP SINGLE
    call dijkstra_sp(it,nbasins,snode,thr_rhlper(:,1),k)
!$OMP END SINGLE
#else
...
#ifdef ENABLE_THREADS
!$OMP SINGLE
    call network_order_by_flux(it,nbasins,snode,thr_rhlper(:,1),dofwr,ldum)
!$OMP END SINGLE
#else
...
!
  if ((nmode.eq.8).OR.(nmode.eq.9)) then
#ifdef ENABLE_THREADS
    rhlp(1) = maxval(pfolds(cbnd(1):cbnd(2),k))
    i = 1
    j = 70
    call thr_freduction(aone,rhlp(1:1),i,j,tpi)
    if (rhlp(1).le.0) then
      if (tpi.le.1) then
        write(ilog,*) 'Warning. Aborting generation of pfold cut profile because the required committor &
 &probabilities are missing or corrupt.'
      end if
      return
    end if
    thr_rhlper(cbnd(1):cbnd(2),1) = pfolds(cbnd(1):cbnd(2),k)
#else
    if (maxval(pfolds(:,k)).le.0) then
      write(ilog,*) 'Warning. Aborting generation of pfold cut profile because the required committor &
 &probabilities are missing or corrupt.'
      return
    end if
    mfptv(:) = pfolds(1:nbasins,k)
#endif
  end if
!
#ifdef ENABLE_THREADS
...
!
  ...
  call int2str(myrank,nod,re_aux(10))
  if (use_REMC.EQV..true.) then
    fn = 'N_'//nod(1:re_aux(10))//'_'//bitt(t1:t2)//'_'//tstr//'_CFEP_'//nor(1:tx)//'.dat'
  else
...
!             the mean square distance across the cut as a linear average over all snap-to-snap transitions 
              hlp1(k) = hlp1(k) + it(iv4(i))%lensnb(j,fbix)*it(iv4(i))%fewtsnb(j,fbix)
            else
!             the mean square distance across the cut as a linear average over all snap-to-snap transitions 
...
!  logical, ALLOCATABLE:: hit(:)
  logical staterr,did_augment,write_weights,dofwr
...
! simple MSM reweighting
  if ((cequil.eq.1).OR.(cequil.eq.2)) then
    k = 2
    if ((cequil.eq.2).OR.(did_augment.EQV..true.)) k = 3
    if ((which_timeflow.eq.1).OR.(which_timeflow.eq.3)) then
#ifdef ENABLE_THREADS
!$OMP BARRIER
!$OMP MASTER
...
!$OMP BARRIER
    ...
    do i=sts(1),sts(2)
      do j=1,it(i)%nb
        if ((it(it(i)%lstnb(j))%inscc.eq.it(i)%inscc).AND.(it(i)%fewtsnb(j,fbix-2).gt.0.0)) then
...
! measure network imbalance
  ...
  do i=sts(1),sts(2)
    do j=1,it(i)%nb
      ...
      else if (it(i)%fewtsnb(j,fbix-2).gt.0.0) then
...
!$OMP SINGLE
#endif
  if ((1.0-normeo).gt.0.0) write(ilog,459) normerr/(1.0-normeo)
#ifdef ENABLE_THREADS
!$OMP END SINGLE NOWAIT
#endif
  deallocate(pbase)
...
!
subroutine iterative_mfpt(it,nbasins,snode,mfptv,thresher,dofwr,maxtime,tpi)
!
  use clusters
  use iounit
  use threads, ONLY: thrdat,thr_rutil
  use, INTRINSIC:: ISO_FORTRAN_ENV
!
  implicit none
!
  integer, INTENT(IN):: nbasins,snode,tpi
  logical, INTENT(IN):: dofwr
...
!
  ...
  RTYPE mfptv(nbasins)
!
  integer iterations,i,j,ii,nsccs,fbix,tonext,lastt,azero,cbnd(2),sts(4),chunksz
  RTYPE normerr(1),kr
  logical notdone
...
!
  if ((tpi.le.0).AND.(OMP_IN_PARALLEL().EQV..true.)) then
    write(ilog,*) 'Fatal. When using multi-threaded code, iterative_mfpt(...) must always be called &
 &by all threads of the binding region (instead got a thread identifier of zero). This is a bug.'
    call fexit()
  end if
  i = 1
...
!
  ...
  if (tpi.le.1) then
    write(ilog,'(1x,a,i8,a,a,a)') 'Now iteratively computing mean first passage time to node ',snode,' from ',timename(i:j),&
 &' time matrix ...'
  end if
  nsccs = size(csccwts)
  if (chunksz.gt.0) then
    allocate(incr(chunksz))
    allocate(hit(chunksz))
    hit(:) = .false.
  end if
!
  notdone = .true.
  iterations = 0
  mfptv(cbnd(1):cbnd(2)) = 1.0
  if ((snode.ge.cbnd(1)).AND.(snode.le.cbnd(2))) mfptv(snode) = 0.0
#ifdef ENABLE_THREADS
!$OMP BARRIER
#endif
  tonext = 10
    ...
    do i=sts(1),sts(2)
      ...
      do j=1,it(i)%nb
        ...
        if (it(i)%fewtsnb(j,fbix).gt.0.) then
          incr(ii) = incr(ii) + it(i)%fewtsnb(j,fbix)*mfptv(it(i)%lstnb(j))
          hit(ii) = .true.
        end if
      end do
    end do
    if ((snode.ge.sts(1)).AND.(snode.le.sts(2))) incr(snode-sts(1)+1) = 0.0
#ifdef ENABLE_THREADS
!$OMP MASTER
    thr_rutil(61,1) = 1.0
!$OMP END MASTER
...
#endif
    if (chunksz.gt.0) then
      normerr(1) = sum(abs(incr(:)-mfptv(sts(1):sts(2)))) ! /(1.0*chunksz)
      mfptv(sts(1):sts(2)) = incr(:)
    else
      normerr(1) = 0.0
    end if
    if (mod(iterations,20).eq.0) then
#ifdef ENABLE_THREADS
      ii = 1
      j = 60
      call thr_freduction(azero,normerr(1:1),ii,j,tpi)
#endif
      normerr(1) = normerr(1)/(1.0*nbasins)
      if (normerr(1).lt.thresher) exit
    end if
#ifdef ENABLE_THREADS
!$OMP MASTER
#endif
...
#ifdef ORACLE_FORTRAN
        kr = (1.0*iterations)/(max(1_8,(tts(2)-tts(1)))/thrdat%rate) ! iterations per second
#else
        kr = (1.0*iterations)/(max(1_INT64,(tts(2)-tts(1)))/thrdat%rate) ! iterations per second
#endif
        lastt = tonext
        i = nint(kr*0.1*maxtime/thrdat%rate) ! 10% into the future of maximum allowed time
...
!
  ...
  do i=sts(1),sts(2)
    ii = i - sts(1) + 1
    if ((hit(ii).EQV..false.).AND.(i.ne.snode)) then
      mfptv(i) = kr + 0.1*i
    end if
  end do
!
...
!
  if (hasany.EQV..false.) then ! completely cut off
    it(which)%inscc = cursccid
    idxcnt = idxcnt - 1
    it(which)%rflw = -1
    cursccid = cursccid + 1
  else if (it(which)%ldis.eq.it(which)%active) then
    do i=idxcnt,1,-1
...
!
  if (sum(it(iv1(inissnap))%fewtsnb(1:it(iv1(inissnap))%nb,fbix)).le.0.0) then
    if (tpi.le.1) then
      write(ilog,*) 'Warning. Initial snapshot for the generation of synthetic trajectories belongs to a cluster &
 &that is cut off from the network. Disabled.'
      synmode = 0
    end if
#ifdef ENABLE_THREADS
...
! few checks
  if (synmode.eq.1) then                     !Target (end cluster) is defined only in mode 1
    ...
    if (iv1(inissnap).eq.iv1(endssnap)) then
      if (tpi.le.1) then
        write(ilog,*)
        write(ilog,*) 'Warning. Initial and target snapshots for the generation of synthetic &
 &trajectories belong to the same cluster. Disabled.'
        write(ilog,*)
      end if
#ifdef ENABLE_THREADS
!$OMP BARRIER
...
!
    if ((nsskip.gt.0).AND.(((synmode.eq.1).AND.(straj(iiter).eq.endclu)).OR.(synmode.ne.1))) then ! write 
      ...
      if (use_REMC.EQV..true.) then
        ...
        filename = 'N_'//xpont(1:re_aux(10))//'_MSM_SYN_TRAJ_'//nod(1:tl)//'_'//ttype//'.frames'
      else
...
!  what if 0: set to centroid of largest cluster (always true regardless settings for framesfile, ccollect, ...)
  ...
  else if (inissnap_usrslct.eq.-2) then !set to centroid of largest cluster in largest strongly connected component
    refscc = maxloc(csccwts,dim=1)
    do j=1,nbasins
      if (it(j)%inscc.eq.refscc) then !since clusters are ordered, we exit when we find the first one of the ref component
        inissnap = it(j)%center
        exit
      end if
    end do
...
!
  if ((synmode.eq.1).OR.(dopfold.EQV..true.)) then  !cases where end snapshot is important
    if (endssnap_usrslct.eq.0) then   !default
      endssnap = cstored              !last stored snapshot (this is rudimentary, we should do better)
    else ! otherwise, we have already remapped the input to 1:cstored (now in inissnap_usrslct)
...
! print final selection (this may be redundant)
  ...
  if (inissnap_usrslct.eq.-2) then
    if (refscc.ne.maxloc(csccwts,dim=1)) then
      write(ilog,*) 'Fatal. Gotten refscc.ne.maxloc(csccwts,dim=1) with inissnap_usrslct.eq.-2. This is a bug.'
      call fexit()
    end if
  end if
  write(ilog,*) 
...
! first sanity check and read
 ...
 &FMCSC_RE_TRAJSKIP, FMCSC_RE_TRAJOUT, and FMCSC_REPLICAS must match the trace file. Assuming no reseeding occurred.'
    ...
    do k=1,re_conditions
      kkk = k
      jj = 0
      do while ((lmap(kkk).ne.kkk).AND.(jj.le.re_conditions))
        jj = jj + 1
        kkk = lmap(kkk)
        if (kkk.eq.k) then
          write(ilog,*) 'Warning. While reading PIGS trace file (...), line ',kk,' was found to contain a &
 &reciprocal or cyclical reseeding (swap(s)). This is not supported and cannot result from a standard PIGS calculation. &
 &Reseeding of replica ',k,' with ',lmap(k),' was ignored. If this is a manually added trace line, consider &
 &splitting it in two.'
          lmap(k) = k
          exit
        end if
        if (kkk.eq.lmap(kkk)) exit
      end do
    end do
  end do
  rewind(unit=iu)
  ...
  do i=1,klines
    ...
    do k=1,re_conditions
      ...
      kkk = k
      jj = 0
      do while ((lmapmat(kkk+1,i).ne.kkk).AND.(jj.le.re_conditions))
        jj = jj + 1
        kkk = lmapmat(kkk+1,i)
        if (kkk.eq.k) then
          lmapmat(k+1,i) = k
        ...
        if (kkk.eq.lmapmat(kkk+1,i)) exit
      end do
    end do
  end do
  kk = 1
...
! lastly, remap additional input snapshots where needed
  if (cmode.eq.4) then
!   correct bad default (this is slightly misplaced)
    if ((cprogindex.eq.1).AND.(cprogindstart.eq.-1)) cprogindstart = 0
!   Initial snapshot
    if (cprogindstart.gt.0) then !what if we have a frames file or ccollect is not 1 or both: remapping through sconnect(:,3)
      bhlp = csttmp
      j = cprogindstart
      call binary_search(bhlp,sconnect(1:bhlp,3),j,k)
      if (sconnect(max(1,min(bhlp,k)),3).eq.cprogindstart) then
        write(ilog,*) 'Identified initial snapshot selected by FMCSC_CPROGINDSTART (',cprogindstart,') as ',max(1,min(bhlp,k)),&
 &'th stored snapshot.'
        cprogindstart = max(1,min(bhlp,k))
      else
        write(ilog,*) 'Fatal. Initial snapshot selected by FMCSC_CPROGINDSTART could not be found amongst stored snapshots.'
        call fexit() ! to help the user
      end if
    end if
  end if
  if ((eigvalmd.ne.0).OR.(dopfold.EQV..true.).OR.(synmode.ne.0).OR.(ccfepmode.gt.0)) then
!   Initial snapshot 
    if (inissnap_usrslct.gt.0) then !what if we have a frames file or ccollect is not 1 or both: remapping through sconnect(:,3)
      ...
      if (sconnect(max(1,min(bhlp,k)),3).eq.inissnap_usrslct) then
        write(ilog,*) 'Identified initial snapshot selected by FMCSC_INISYNSNAP (',inissnap_usrslct,') as ',max(1,min(bhlp,k)),&
 &'th stored snapshot.'
...
!   Initial snapshot 
    if (endssnap_usrslct.gt.0) then !what if we have a frames file or ccollect is not 1 or both: remapping through sconnect(:,3)
      ...
      if (sconnect(max(1,min(bhlp,k)),3).eq.endssnap_usrslct) then
        write(ilog,*) 'Identified target snapshot selected by FMCSC_ENDSYNSNAP (',endssnap_usrslct,') as ',max(1,min(bhlp,k)),&
 &'th stored snapshot.'
...
!
subroutine edit_tmat(it,nbasins,mode,mode2,tpi)
!
  use clusters
  use iounit
!
  implicit none
!
  integer, INTENT(IN):: nbasins,mode,mode2,tpi
!
  type(t_scluster) it(nbasins)
!
  integer i,j,jk,sts(4)
  RTYPE ptmp
...
!
  if ((tpi.le.0).AND.(OMP_IN_PARALLEL().EQV..true.)) then
    write(ilog,*) 'Fatal. When using multi-threaded code, edit_tmat(...) must always be called &
 &by all threads of the binding region (instead got a thread identifier of zero). This is a bug.'
    call fexit()
  end if
  i = 1
  ...
  do i=sts(1),sts(2)
    do jk=1,2
      ...
      if ((mode.eq.0).OR.(mode.eq.2)) then
        ...
        else if (it(i)%nb.gt.0) then
          it(i)%fewtsnb(1:it(i)%nb,jk+2) = 0.0
        end if
...
!
  if (sccix.gt.0) then
    ...
    if (use_REMC.EQV..true.) then
      call int2str(myrank,nod,re_aux(10))
      fn = 'N_'//nod(1:re_aux(10))//'TMAT_'//compo//'_'//suffix//'.dat'
    else if (use_MPIAVG.EQV..true.) then
    ...
    if (use_REMC.EQV..true.) then
      call int2str(myrank,nod,re_aux(10))
      fn = 'N_'//nod(1:re_aux(10))//'TMAT_'//suffix//'.dat'
    else if (use_MPIAVG.EQV..true.) then
      if (myrank.ne.0) return
      fn = 'TMAT_'//suffix//'.dat'
    end if
...
! based on check/return structure above, we know that eigvec_msm is positive definite at least
  ...
  if (counter.gt.0) then
    write(ilog,*) 'The maximum difference between the assumed steady state vector and available steady state &
 &probabilities from a different algorithm is [%]: ',maxval(tmpvr(:))
  end if
  if (allocated(tmpvr).EQV..true.) deallocate(tmpvr)
!
...
!
  ...
  if (nz.le.0) then
    write(ilog,*) 'Warning. Aborting attempt to compute spectral properties for a component that is completely cut off &
 &(check FMCSC_INISYNSNAP).'
    return
  end if
  if (k.eq.1) then
    write(ilog,*) 'Warning. Aborting attempt to compute spectral properties for a component of size 1 &
 &(check FMCSC_INISYNSNAP).'
    return
  end if  
...
! write to file the results
  ...
  if (use_REMC.EQV..true.) then
    call int2str(myrank,xpont,re_aux(10))
    filename = 'N_'//xpont(1:re_aux(10))//'_EIGENS_'//fnsffx//'.dat'
  else
  ...
  if (actedim.lt.numeig) then
    ...
    if (use_REMC.EQV..true.) then
      call int2str(myrank,xpont,re_aux(10))
      filename = 'N_'//xpont(1:re_aux(10))//'_EIGVALS_NOTCONVRGD_'//fnsffx//'.dat'
    else
...
!
  ...
  else if ( (eigvalmd.eq.2).AND.(doeigvec_msm.EQV..true.) ) then
...
!!! First solve eigenvalue problem !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 &Restarting attempt: ',iarn,' on',nmaxitr,'. Tolerance:',CNTL(1)
        ...
        notdone = .false.
        write(ilog,'(A32,2x,G16.8)') 'Accepted convergence tolerance: ',RINFO(4)
...
!   To compute the corresponding eigenvectors
    IKEEP(1) = 0 ! essential if convergence failure for some eigenvalues before
...
!
  ...
  integer iu,freeunit,i,j,k,ii,kk,iii,kkk,info,counter,nextras,refsccbu,fprfxl
  integer exstat
...
! initialize 
  ...
  if (use_REMC.EQV..true.) then
    call int2str(myrank,nod,re_aux(10))
    fprfx(1:(re_aux(10)+3)) = 'N_'//nod(1:re_aux(10))//'_'
    fprfxl = re_aux(10)+3
  else
    fprfxl = 0
  end if
...
!!! First solve the problem for the forward (+) committors !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  if (pfold_report.EQV..true.) then
    iu = freeunit()
    if (fprfxl.gt.0) then
      open(unit=iu,file=fprfx//"fold_clus.out",status='unknown')
    else
    ...
    iu = freeunit()
    if (fprfxl.gt.0) then
      open(unit=iu,file=fprfx//"unfold_clus.out",status='unknown')
    else
...
! outputting
  if (pfold_report.EQV..true.) then
    iu = freeunit()
    if (fprfxl.gt.0) then
      filename = fprfx//'mat_pfold_'//fnsffx//'_plus.dat'
    else
...
! outputting
  if (pfold_report.EQV..true.) then
    iu = freeunit()
    if (fprfxl.gt.0) then
      filename = fprfx//'rhs_pfold_'//fnsffx//'_plus.dat'
    else
...
! Solve with iterative refinement
  ...
  if (dofwr.EQV..true.) then
    ...
    if (fprfxl.gt.0) then
      open(unit=iu,file=fprfx//'PFOLD_PLUS_FWD.dat')
    else
    ...
    if (fprfxl.gt.0) then
      open(unit=iu,file=fprfx//'PFOLD_PLUS_BWD.dat')
    else
...
!     check it
 ...
 &tev(:,1),tev(:,2),tmpvr2(:,1:2),actedim,etol,tmpvr2(:,1),hsl_verbosity,exstat)
      ...
      if (pfold_report.EQV..true.) then
        iu = freeunit()
        if (fprfxl.gt.0) then
          filename = fprfx//'ss_pfold_'//fnsffx//'.dat'
        else
...
!   outputting
    if (pfold_report.EQV..true.) then
      iu = freeunit()
      if (fprfxl.gt.0) then
        filename = fprfx//'mat_pfold_'//fnsffx//'_minus.dat'
      else
...
!   outputting
    if (pfold_report.EQV..true.) then
      iu = freeunit()
      if (fprfxl.gt.0) then
        filename = fprfx//'rhs_pfold_'//fnsffx//'_minus.dat'
      else
...
!   Solve with iterative refinement
    ...
    if (dofwr.EQV..true.) then
      ...
      if (fprfxl.gt.0) then
        open(unit=iu,file=fprfx//'PFOLD_MINUS_FWD.dat')
      else
      ...
      if (fprfxl.gt.0) then
        open(unit=iu,file=fprfx//'PFOLD_MINUS_BWD.dat')
      else
