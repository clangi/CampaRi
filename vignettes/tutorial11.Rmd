...
run_campari(FMCSC_SEQFILE="nbu.in", # you must have it defined according to CAMPARI's rules
            ...
            PARAMETERS="olsaal.prm", # if this variable it is not supplied will be automatically assigned to <full path to folder>/campari/params/abs3.2_opls.prm
            FMCSC_SC_IPP=0.0,
            FMCSC_SC_BONDED_T=1.0,
            FMCSC_DYNAMICS=3,
            FMCSC_FRICTION=3.0,
...
Please note that the above calculations were possible thanks to the possibility to automatically set a PARAMETERS file from the campari exe. Let's rename some of the results:
```{bash, eval=FALSE}
...
```{r, eval=FALSE}
un_campari(data_file = "NBU.dcd", key_file_input = "nbu_clustering.key", base_name="nbu_clustering_rmsd",
...
Anyhow we advise to use the function sapphire_plot() which come with the CampaRi package because it is built around the ggplot2.
